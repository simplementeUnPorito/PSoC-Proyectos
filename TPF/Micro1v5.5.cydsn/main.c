...
#define MODO_GUARDAR_SELECCION_ 13
#define REINICIAR 12
...
uint8_t interpretarKey(uint16_t key){
    switch(ventana)
    {
        ...
        case MODO_GUARDAR_SELECCION:
            switch(key){
                case KEY_1:
                    ventana = MODO_GUARDAR_SELECCION_;
            ...
            return MAL;
        case MODO_GUARDAR_SELECCION_:
             switch(key){
                case KEY_1:
                    ventana = MODO_GUARDAR;
                    flag.config.bit.modoMuestreo = 0;
                    return BIEN;
                case KEY_2:
                    ventana = MODO_GUARDAR;
                    flag.config.bit.modoMuestreo = 1;
                    TimerGuardar_Enable();
                    resetGuardar_Write(0);
                    return BIEN;
                case ESC_KEY:
                    ventana = MENU;
                    return BIEN;
            ...
            return MAL;
        case MODO_GUARDAR:
            if(flag.config.bit.modoMuestreo == 0 && key == ENTER_KEY)
            {
                guardarPosicionActual();
                flag.cambio.bit.updateGuardar = 1;
                return BIEN;
            }else if(key == ESC_KEY)
            {
                ventana = MENU;
...
CY_ISR(guardarMedida){
    resetGuardar_Write(1);
    guardarDone_ClearPending();
    if(ventana == MODO_GUARDAR && flag.config.bit.modoMuestreo == 1){
        //resetGuardar_Write(1);
        if(guardarPosicionActual()){
            //TimerGuardar_Enable();
            resetGuardar_Write(0);
            PC_PutStringConst("Guardado\n\r");
        }
        else{
            ventana = MENU;
            flag.reset = TRUE;
...
void myStart()
{
    ...
    TimerGuardar_Start();
    guardarDone_StartEx(guardarMedida);
    resetGuardar_Write(1);
...
void updateDisplay(){
    ...
    switch(ventana){
        ...
        case MODO_GUARDAR_SELECCION_:
            flag.reset = FALSE;
            printLCD("Seleccione modo:","1M. ENT 2M. Fijo");
            //LCD_setRGB(255, 255, 255);  // Cambiar el color a azul
            changeAnimation(MODO_GUARDAR_SELECCION);
            break;
        case MODO_GUARDAR:
            //if(flag.reset == TRUE){
            flag.reset = FALSE;
            if(flag.config.bit.modoMuestreo == 0){
                LCD_setCursor(14,0);
                LCD_print("   ",3);
                ...
                printLCD(bufferGuardar,"ESC para salir  ");
                 ...
                 printLCD("Guardando...    ","ESC para salir  ");
            }
...
void printNumber(uint8_t modo){
    ...
    if(modo)
        ...
        snprintf(bufferAux,3,"%i",guardado[flag.config.bit.vector].lenLista);
    ...
    for(;;)
    {
        if(updatePantalla_Read() == 1){
            ...
            if(flag.key != NADA&& RGB_Read() == 1){
                resetRGB_Write(TRUE);
                changeRGB();
            }if((ventana == MODO_GUARDAR && flag.config.bit.modoMuestreo == 0 && flag.cambio.bit.updateGuardar == 1)){
                flag.cambio.bit.updateGuardar = 0;
                printNumber(0);
            }
