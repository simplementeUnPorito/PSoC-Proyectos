...
declare all_registers {
 ...
 declare create_clock {
  ...
  {port_pin_list        List       		}
  {-add                 Flag       		}
  {-nominal_period      Float {$par > 0}		}
 } {param(-period) && (param(-name) || param(port_pin_list))}
 # -nominal_period added by Neil Tuttle (neil@cypress.com)
...
# -nominal_period added by Ambica Prasad (ambica@softjin.com)
# -nominal_period is used to specify the nominal period of the clock. In some cases, the clock division
# will result in a complex waveforms. This option is required by Cypress Semiconductor to capture this 
# difference in actual and the specified frequency.
...
#sdc1.7
 ...
 declare set_false_path {
  ...
  {-fall_through               List {dup}    }
  {-rise_through               List {dup}    }
 } {(param(-from) || param(-to) || param(-through) || param(-rise_from) || param(-fall_from) || param(-rise_to) || param(-fall_to)) && \
 !(param(-rise) && param(-fall)) && !(param(-setup) && param(-hold))}
# Modified by Ambica Prasad, SoftJin Technologies. ambica@softjin.com
# Changed the check to include atleast one of -from, -to, -through, -rise_from, -rise_to, -fall_from. -fall_to as a mandatory parameter
