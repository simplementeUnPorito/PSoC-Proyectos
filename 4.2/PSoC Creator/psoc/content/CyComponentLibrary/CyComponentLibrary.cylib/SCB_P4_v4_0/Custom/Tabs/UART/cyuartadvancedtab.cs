...
namespace SCB_P4_v4_0
{
    public partial class CyUARTAdvancedTab : CyTabControlWrapper
    {
            ...
            : base(parameters, editor)
        {
            ...
            m_chbInterruptRXBreakDetected.CheckedChanged += m_chb_CheckedChanged;
            ...
            m_numRXBreakWidth.TextChanged += m_numRXBreakWidth_TextChanged;
            ...
            m_numRXBreakWidth.Maximum = decimal.MaxValue;
            m_numRXBreakWidth.Minimum = 0;
            #endregion
        }
        #endregion
        ...
        public override void UpdateUI()
        {
            ...
            m_chbInterruptRXBreakDetected.Checked = m_params.UART_InterruptOnRXBreakDetected;

            m_cbRXTriggerLevel.SelectedIndex = m_params.UART_RXTriggerLevel;
            m_numRXBreakWidth.Text = m_params.UART_RXBreakWidth.ToString();
        ...
        private void UpdateRXNotEmptyInterrupt()
        {
            if (m_params.GlobalEditMode && 
                m_params.UART_EnableByteMode && m_params.UART_RxBufferSize == m_params.UartRxBufferMinSize)
            {
        ...
        public void UpdateBreakDetectedState()
        {
            // Break detected checkbox
            bool breakDetectEnabled = m_params.UartSubMode.Value == CyEUARTSubModeType.STANDARD;
            m_chbInterruptRXBreakDetected.Enabled = breakDetectEnabled;

            if (breakDetectEnabled == false && m_chbInterruptRXBreakDetected.Checked)
            {
                m_chbInterruptRXBreakDetected.Checked = false;
            }
            
            // Break width
            bool breakWidthEnabled = m_params.UART_InterruptOnRXBreakDetected;
           
            m_lblRXBreakWidth.Enabled = breakWidthEnabled;
            m_numRXBreakWidth.Enabled = breakWidthEnabled;

            ValidateRXBreakWidth();
        }
        ...
        private void m_numRXBreakWidth_TextChanged(object sender, EventArgs e)
        {
            byte breakWidth;
            if (Byte.TryParse(m_numRXBreakWidth.Text, out breakWidth))
            {
                if (breakWidth >= CyParamRanges.UART_RX_BREAK_WIDTH_MIN &&
                    breakWidth <= CyParamRanges.UART_RX_BREAK_WIDTH_MAX)
                {
                    m_params.UART_RXBreakWidth = breakWidth;
                }
            }
            ValidateRXBreakWidth();
        }

        private void ValidateRXBreakWidth()
        {
            string errorMessage = String.Empty;

            if (m_params.UART_InterruptOnRXBreakDetected)
            {
                byte breakWidth;
                bool breakWidthValid = Byte.TryParse(m_numRXBreakWidth.Text, out breakWidth) &&
                                       (breakWidth >= CyParamRanges.UART_RX_BREAK_WIDTH_MIN &&
                                        breakWidth <= CyParamRanges.UART_RX_BREAK_WIDTH_MAX);

                if (breakWidthValid == false)
                {
                    errorMessage = String.Format(Resources.UARTBreakWidthError,
                        CyParamRanges.UART_RX_BREAK_WIDTH_MIN, CyParamRanges.UART_RX_BREAK_WIDTH_MAX);
                }
            }

            // Don't show error message if control is disabled
            if (m_numRXBreakWidth.Enabled == false)
                errorMessage = String.Empty;

            m_errorProvider.SetError(m_numRXBreakWidth, errorMessage);
        }
        ...
        void m_chb_CheckedChanged(object sender, EventArgs e)
        {
            if (m_params.GlobalEditMode)
            {
                ...
                else if (sender == m_chbInterruptRXBreakDetected)
                {
                    m_params.UART_InterruptOnRXBreakDetected = m_chbInterruptRXBreakDetected.Checked;
                    UpdateBreakDetectedState();
                }
