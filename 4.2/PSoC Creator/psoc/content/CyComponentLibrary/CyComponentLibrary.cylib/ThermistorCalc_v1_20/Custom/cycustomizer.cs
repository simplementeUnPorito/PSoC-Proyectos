...
namespace ThermistorCalc_v1_20
{
    public class CyCustomizer : ICyParamEditHook_v1, ICyAPICustomize_v1
    {
        ...
        private const char ARRAY_ITEMS_SPLITTER = ',';
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            ...
            else
            {
                ...
                string lookUpTableStr = "static const uint32 CYCODE " + instanceName + "_LUT[] = { ";
                ...
                lookUpTableStr += " };" + Environment.NewLine + Environment.NewLine;
        ...
        private string WordWrap(string str)
        {
            ...
            string[] splitted = str.Split(ARRAY_ITEMS_SPLITTER);
            for (int i = 0; i < splitted.Length; i++)
            {
                if ((line.Length + splitted[i].Length + 1) <= MAX_LINE_LENGTH)
                {
                    line += splitted[i] + ARRAY_ITEMS_SPLITTER;
                }
                else
                {
