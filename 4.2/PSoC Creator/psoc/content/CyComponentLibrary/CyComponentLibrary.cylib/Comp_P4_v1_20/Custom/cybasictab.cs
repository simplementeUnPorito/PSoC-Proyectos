...
namespace Comp_P4_v1_20
{
    ...
    {
        private const string CTB_INT_NAME = "CTBlnt";
        private const string CTB_MINI_INT_NAME = "CTBmlnt";
            ...
            : base(param)
        {
            ...
            label_Note1.Text = string.Format(Resources.FootnoteText_Format1, param.IsCTBIpBlock ? 
                CTB_INT_NAME : CTB_MINI_INT_NAME);
        ...
        public void UpdateUI()
        {
            ...
            UpdatePolarityInvertVisibility();
        }

        private void UpdatePolarityInvertVisibility()
        {
            if (!rbPolarityInv.Checked && !m_params.IsUDB)
                rbPolarityInv.Enabled = false;
            else if (!rbPolarityInv.Checked && rb_DeepSleepEnabled.Checked)
                rbPolarityInv.Enabled = false;
            else
        ...
        private void UpdateDeepSleepVisibility()
        {
            ...
            {
                grB_DeepSleep.Enabled = m_params.DeepSleep;
                errorProvider.SetError(rb_DeepSleepEnabled, m_params.DeepSleep ?
                    String.Format(Resources.ErrMsgDeepSleep) : String.Empty);
            }
            else
            {
        ...
        {
            if (m_params.Polarity == CyEPolarityType.CMP_INVERTING && !m_params.IsUDB)
                errorProvider.SetError(rbPolarityInv, String.Format(Resources.ErrMsgPolarityNotUdb));
            else if (m_params.Polarity == CyEPolarityType.CMP_INVERTING && m_params.DeepSleep)
                errorProvider.SetError(rbPolarityInv, String.Format(Resources.ErrMsgPolarity));
            else
                errorProvider.SetError(rbPolarityInv, String.Empty);
        }
