...
namespace Scan_ADC_v2_10
{
    ...
    public class CyParamNames
    {
        ...
        public const string DEVICE_SERIES_NAME = "DeviceSeriesName";
    ...
    public enum CyEInputMode
    {
        ...
        SINGLE_ENDED_CHAN = 1,
        [Description("Filter (single ended)")]
        FILTER_CHAN = 2,
    }
    ...
    /// SAR type.
    /// </summary>
    [CyEnumCName("sar_type_enum")]
    public enum CyESarType
    {
        [Description("mxs40pass")]
        SAR_MXS40PASS = 0,
        [Description("m0s8pass4b")]
        SAR_M0S8PASS4B = 1,
    }
    ...
    public class CyParameters
    {
        ...
        private CyESarType m_sarType = CyESarType.SAR_M0S8PASS4B;
        ...
        /// <summary>
        /// Which SAR version is being used?
        /// </summary>
        public CyESarType SarType
        {
            get { return m_sarType; }
            set { m_sarType = value; }
        }
        ...
        /// <summary>
        /// Exposes DeviceSeriesName since there is no query support in the ICyDeviceQuery_v1 interface.
        /// </summary>
        public string DeviceSeriesName
        {
            get { return GetValue<string>(CyParamNames.DEVICE_SERIES_NAME); }
        }
        ...
        public CyConfigModel GetConfig(int i)
        {
            ...
            if (m_configs[i] == null)
            {
                ...
                double sampleRate_ksps = 0;
                bool hasExtVrefSource = true;

                switch (this.SarType)
                {
                    case CyESarType.SAR_MXS40PASS:
                        //Some parts do not have any way to connect an external Vref or Bypass cap
                        CySarExtVrefType_v1 pinType;
                        m_instQuery.DeviceQuery.PSoC6DeviceInfoQueryV1.GetSarAdcExtVrefType(out pinType);
                        hasExtVrefSource = (pinType == CySarExtVrefType_v1.NONE) ? false : true;
                        if(!hasExtVrefSource)
                        {
                            config.VrefBypass = false;
                        }
                        m_instQuery.DeviceQuery.PSoC6DeviceInfoQueryV1.GetSarAdcMaxSampleRateKSPS(out sampleRate_ksps);
                        config.Sar = new CySar_P6(config.VrefSource, config.VrefBypass, sampleRate_ksps, hasExtVrefSource);
                        break;
                    case CyESarType.SAR_M0S8PASS4B:
                        m_instQuery.DeviceQuery.PSoC4DeviceInfoQueryV1.GetSarAdcMaxSampleRateKSPS(out sampleRate_ksps);
                        config.Sar = new CySar_P4B(config.VrefSource, config.VrefBypass, sampleRate_ksps, true);
                        break;
                }

                // Configure the Scheduler
                CyScheduler.AdcClockDividerMin = config.Sar.MinAdcClockDivider;
                CyScheduler.SampleMin_ns = config.Sar.MinAcqTime_ns;
                CyScheduler.MaxSps = Convert.ToInt32(config.Sar.SampleRate_ksps) * 1000;
                CyScheduler.AdcClocksNotSampling = config.Sar.ApertureAdcClocksNotSampling;
                ...
                if (config.ChannelList.Count == 0)
                {
                    ...
                    channels.Add(new CyChannelModel(config.Sar.MinAcqTime_ns));
                    channels.Add(new CyChannelModel(config.Sar.MinAcqTime_ns));
                ...
                while (config.ApertureTimers.Count < CySarRanges.APERTURE_TIMER_COUNT)
                {
                    config.ApertureTimers.Add(CySarRanges.APERTURE_TIMER_MIN);
                }
                ...
                if (config.ApertureTimers.Count > CySarRanges.APERTURE_TIMER_COUNT)
                {
                    config.ApertureTimers.RemoveRange(
                        CySarRanges.APERTURE_TIMER_COUNT,
                        config.ApertureTimers.Count - CySarRanges.APERTURE_TIMER_COUNT);
                }
