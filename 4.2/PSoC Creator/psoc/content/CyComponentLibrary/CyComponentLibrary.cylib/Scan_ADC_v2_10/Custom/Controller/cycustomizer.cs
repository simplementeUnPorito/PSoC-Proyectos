...
namespace Scan_ADC_v2_10
{
        ...
        ICyShapeCustomize_v2
    {
            ...
            ICyInstEdit_v1 edit, ICyTerminalQuery_v1 termQuery, ICyExpressMgr_v1 mgr)
        {
            ...
            {
                m_editor.SetSelectedPage("Common");
            }
        ...
        public CyExprEvalFuncEx GetFunction(string exprFuncName)
        {
            switch (exprFuncName)
            {
                ...
                case "IsEdgeTrigger":
                    return new CyExprEvalFuncEx(IsEdgeTrigger);
        ...
        object IsEdgeTrigger(string exprFuncName, object[] callingArgs, ICyExprEvalArgs_v2 creatorArgs,
    ICyExprTypeConverter typeConverter)
        {
            ...
            bool result = CyExprEval.IsEdgeTrigger(m_params);
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            if (m_params == null)
            {
                ...
                for (int i = 0; i < m_params.NumberOfConfigs; i++)
                {
                    ...
                    if (config.Sar.IsVrefQueriable())
                    {
                        config.VrefVoltage = CyVoltage.GetDesignVrefValue(m_params, config.Sar.VrefSource);
        ...
        public IEnumerable<CyDRCInfo_v1> GetDRCs(ICyDRCProviderArgs_v1 args)
        {
            ...
            if (CyDrc.CheckVrefBypassNotice(m_params) == false)
            {
                yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Info,
                    global::Scan_ADC_v2_10.Controller.cydrcerrors.VrefBypassNotice);
            }
            if (CyDrc.CheckVrefExternalNotice(m_params) == false)
            {
                yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Error,
                    global::Scan_ADC_v2_10.Controller.cydrcerrors.VrefNoExtConnectNotice);
            }
            double maxVref = 0;
            if (CyDrc.CheckVrefNotTooHigh(m_params, out maxVref) == false)
            {
                yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Info,
                    string.Format(global::Scan_ADC_v2_10.View.cyguierrors.VrefTooHigh,
                                  maxVref.ToString("N3")));
            }
            double minVref = 0;
            ...
            {
                yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Info,
                    string.Format(global::Scan_ADC_v2_10.View.cyguierrors.VrefTooLow,
                                  minVref.ToString("N3")));
            }
            if (CyDrc.CheckVnegResultNotice(m_params) == false)
            {
                yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Info,
                    global::Scan_ADC_v2_10.Controller.cydrcerrors.VnegResultNotice);
            }
            if (CyDrc.CheckAverageOverflow(m_params) == false)
            {
                if (m_params.SarType == CyESarType.SAR_MXS40PASS)
                    yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Error,
                        global::Scan_ADC_v2_10.Controller.cydrcerrors.AverageOverflowError);
                else
                    yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Warning,
                        global::Scan_ADC_v2_10.Controller.cydrcerrors.AverageOverflowNotice);
            }
            if (CyDrc.CheckAverageAltRes(m_params) == false)
            {
                yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Warning,
                    global::Scan_ADC_v2_10.Controller.cydrcerrors.AltResolutionAverageNotice);
            }
            if (CyDrc.CheckSameSampleMode(m_params) == true)
            {
                yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Error,
                    global::Scan_ADC_v2_10.Controller.cydrcerrors.DifferentSampleMode);
            }
        ...
        private void LoadEditableParameters(ICyInstEdit_v1 instEdit, ICyTerminalQuery_v1 termQuery)
        {
            ...
            m_params.IsFilterAllowed = isFilterAllowed;
            var sarType = GetSarType(instEdit);
            m_params.SarType = sarType;
        }
        ...
        private void LoadReadOnlyParameters(ICyInstQuery_v1 instQuery)
        {
            ...
            m_params.IsFilterAllowed = isFilterAllowed;
            var sarType = GetSarType(instQuery);
            m_params.SarType = sarType;
        }
        ...
        private bool DeviceAllowsFilter(ICyInstQuery_v1 instQuery)
        {
            var deviceQuery = instQuery.DeviceQuery;
            var uabCount = deviceQuery.GetFeatureParameter("m0s8pass4b", "NR_UABS");
            string deviceSeries = this.m_params.DeviceSeriesName;

            // A filter is only available on devices that have UABs and non PSoC 4100PS devices.
            // PSoC 4100PS devices have UABs but the filter is intentionally not exposed.
            // See Marketing Segmentation Plan in JIAO-340.
            var filterOK = ((uabCount > 0) && (deviceSeries != "PSoC 4100PS")) ? true : false;

            return filterOK;
        }

        // Which SAR version are we using?
        static private CyESarType GetSarType(ICyInstQuery_v1 instQuery)
        {
            //Default to m0s8pass4b
            var deviceQuery = instQuery.DeviceQuery;
            var chanCount = deviceQuery.GetFeatureParameter("mxs40pass", "SAR.SAR_CHANNELS");
            var sarType = (chanCount > 0) ? CyESarType.SAR_MXS40PASS : CyESarType.SAR_M0S8PASS4B;
