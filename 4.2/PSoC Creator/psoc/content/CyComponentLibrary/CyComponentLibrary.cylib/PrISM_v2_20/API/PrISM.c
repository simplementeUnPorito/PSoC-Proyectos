...
void `$INSTANCE_NAME`_Init(void) `=ReentrantKeil($INSTANCE_NAME . "_Init")`
{
    ...
    `$CySetRegReplacementString`(`$INSTANCE_NAME`_AUX_CONTROL_PTR, 
                        `$CyGetRegReplacementString`(`$INSTANCE_NAME`_AUX_CONTROL_PTR) | `$INSTANCE_NAME`_FIFO0_CLR);
    CyExitCriticalSection(enableInterrupts);
    
    #if(!`$INSTANCE_NAME`_PULSE_TYPE_HARDCODED)
        /* Writes density type provided by customizer */
        #if(`$INSTANCE_NAME`_GREATERTHAN_OR_EQUAL == `$INSTANCE_NAME`_COMPARE0)
            `$INSTANCE_NAME`_CONTROL_REG |= `$INSTANCE_NAME`_CTRL_COMPARE_TYPE0_GREATER_THAN_OR_EQUAL;
        #else
            `$INSTANCE_NAME`_CONTROL_REG &= (uint8)~`$INSTANCE_NAME`_CTRL_COMPARE_TYPE0_GREATER_THAN_OR_EQUAL;
        #endif /* End `$INSTANCE_NAME`_COMPARE0 */    
    
        #if(`$INSTANCE_NAME`_GREATERTHAN_OR_EQUAL == `$INSTANCE_NAME`_COMPARE1)
            `$INSTANCE_NAME`_CONTROL_REG |= `$INSTANCE_NAME`_CTRL_COMPARE_TYPE1_GREATER_THAN_OR_EQUAL;
        #else
            `$INSTANCE_NAME`_CONTROL_REG &= (uint8)~`$INSTANCE_NAME`_CTRL_COMPARE_TYPE1_GREATER_THAN_OR_EQUAL;
        #endif /* End `$INSTANCE_NAME`_COMPARE1 */    
