...
namespace SPI_Slave_v2_70
{
    public partial class CySPISControl : UserControl, ICyParamEditingControl
    {
        ...
        public CySPISControl(CySPISParameters inst)
        {
            InitializeComponent();
        ...
        private void CySPISControl_Load(object sender, EventArgs e)
        {
            UpdateDrawing();
        }
        ...
        {
            m_params.ShiftDir = (E_B_SPI_SLAVE_SHIFT_DIRECTION)CyEnumConverter.GetEnumValue(
                cbShiftDir.Text, typeof(E_B_SPI_SLAVE_SHIFT_DIRECTION));
            UpdateDrawing();
            ep_Errors.SetError((Control)sender, string.Empty);
        }
        ...
        {
            char[] charsToTrim = { '0' };
            if (numBitRateHertz.Text.Contains(POINT))
        ...
        private bool NumUpDownValidated(object sender)
        {
            ...
            if (sender == numBitRateHertz)
            {
                ...
                message = Resources.FrequencyEPMsg;
            ...
            else if (sender == numDataBits)
            {
                ...
                message = string.Format(Resources.NumOfDataBitsEPMsg, min, max);
                multiplier = 1;
