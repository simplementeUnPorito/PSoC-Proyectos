...
namespace LUT_v1_50
{
    public class CyVerilogBuilder
    {
        ...
        public void DefineAlways(List<string> signals, EdgeTypeEnum edgeType)
        {
            string edge = string.Empty;
            if (edgeType == EdgeTypeEnum.POSITIVE)
                edge = "posedge ";
            else if (edgeType == EdgeTypeEnum.NEGATIVE)
                edge = "negedge ";

            string alwaysStmt = string.Format("always @({0}", edge);
            Write(alwaysStmt);
