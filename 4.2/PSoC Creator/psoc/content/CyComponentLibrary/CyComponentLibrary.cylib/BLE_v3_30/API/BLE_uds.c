...
CYBLE_API_RESULT_T CyBle_UdssSetCharacteristicValue(CYBLE_UDS_CHAR_INDEX_T charIndex, uint8 attrSize, uint8 *attrValue)
{
	...
	else if(cyBle_udss.charInfo[charIndex].charHandle == CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE)
    {
                                                         ...
                                                         uint8 attrSize, uint8 *attrValue)
{
    ...
    else if(cyBle_udss.charInfo[charIndex].descrHandle[CYBLE_UDS_CCCD] == CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE)
    {
        apiResult = CYBLE_ERROR_GATT_DB_INVALID_ATTR_HANDLE;
    }
    else
...
*  'Handle Value Notification' which results in CYBLE_EVT_UDSC_NOTIFICATION event
...
*  results in CYBLE_EVT_UDSC_INDICATION or CYBLE_EVT_GATTC_HANDLE_VALUE_IND (if 
...
*  Server side the CYBLE_EVT_UDSS_WRITE_CHAR events is generated.
                                                            ...
                                                            uint8 attrSize, uint8 * attrValue)
{
	...
	else if(cyBle_udsc.charInfo[charIndex].valueHandle == CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE)
    {
		apiResult = CYBLE_ERROR_GATT_DB_INVALID_ATTR_HANDLE;
	}
    else if((CYBLE_CHAR_PROP_WRITE & cyBle_udsc.charInfo[charIndex].properties) == 0u)
    {
...
*  sent to the GATT Server and on successful execution of the request on the 
*  Server side the CYBLE_EVT_UDSS_READ_CHAR events is generated.
...
CYBLE_API_RESULT_T CyBle_UdscGetCharacteristicValue(CYBLE_CONN_HANDLE_T connHandle, CYBLE_UDS_CHAR_INDEX_T charIndex)
{
    ...
    else if((CYBLE_CHAR_PROP_READ & cyBle_udsc.charInfo[charIndex].properties) == 0u)
    {
        apiResult = CYBLE_ERROR_INVALID_OPERATION;
    }
    else
    ...
    CYBLE_UDS_CHAR_INDEX_T charIndex, CYBLE_UDS_DESCR_INDEX_T descrIndex, uint8 attrSize, uint8 * attrValue)
{
    ...
    else if(cyBle_udsc.charInfo[charIndex].descrHandle[descrIndex] == CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE)
    {
		apiResult = CYBLE_ERROR_GATT_DB_INVALID_ATTR_HANDLE;
	}
    else
