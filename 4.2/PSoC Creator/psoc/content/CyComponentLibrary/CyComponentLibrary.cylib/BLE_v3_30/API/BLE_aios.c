...
*                           hat has been previously triggered is still in progress.
*   * CYBLE_GATTS_ERR_CCCD_IMPROPERLY_CONFIGURED - Client Characteristic Configuration 
...
*  'Handle Value Notification' which results in CYBLE_EVT_AIOSC_NOTIFICATION event
...
*  results in CYBLE_EVT_AIOSC_INDICATION or CYBLE_EVT_GATTC_HANDLE_VALUE_IND (if 
...
CYBLE_GATT_ERR_CODE_T CyBle_AiosSetAggregateChar(void)
{
    ...
    CYBLE_AIOS_CHAR_INDEX_T locCharIndex;
    uint8 locCharInstance;
    uint8 attrValue[CYBLE_AIOS_CHAR_PRESENTATION_FORMAT_LEN];
                        ...
                        locCharIndex++)
        {
            ...
            {   
                                ...
                                locCharInstance++)
                {
                    ...
                    if (gattErr == CYBLE_GATT_ERR_NONE)
                    {   
                        currentDescriptionValue = ((uint16)(((uint16)attrValue[CYBLE_AIOS_CHAR_PRESENTATION_FORMAT_LEN-1u]) << 8u)) | ((uint16)attrValue[CYBLE_AIOS_CHAR_PRESENTATION_FORMAT_LEN-2u]);
...
*   If the AIOS service-specific callback is registered 
*      (with CyBle_AiosRegisterAttrCallback):
*  * CYBLE_EVT_AIOSC_WRITE_DESCR_RESPONSE - in case if the requested attribute is
*                                successfully wrote on the peer device,
