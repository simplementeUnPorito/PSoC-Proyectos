/*******************************************************************************
*
* FILENAME:         B_FanTach_v4_0.v
* Component Name:   B_FanTach_v4_0
*
* DESCRIPTION:
...
*
*  DESCRIPTION:
*    The primary puprose of this datapath is to perform glitch filtering of
...
`ifdef B_FanTach_v4_0_ALREADY_INCLUDED
`else
`define B_FanTach_v4_0_ALREADY_INCLUDED

module B_FanTach_v4_0
(
    ...
    */
    ...
    localparam CLR_EVENTS       = 4'b1010;     /* Reset tach period counter (A0->0) and clear occured events */
    ...
    **************************************************************************/
    ...
    reg     [3:0]   tach_state;             /* State machine that drives the tachometer datapath */
    ...
    reg             glitch_filter_state;    /* Hodls state of a simple Glitch Filer state machine used to generate 
    ...
    reg             tach_cnt3;              /* Signal used to replace TACH_CNT3 state of tachometer SM */
    ...
    **************************************************************************/
    ...
    begin: DmpgFactor
        ...
        /* Contros store signals for damping factor counter datapath */
        ...
        cy_psoc3_dp8 #(.cy_dpconfig_a(
        {
            ...
            `CS_FEEDBACK_DSBL, `CS_CI_SEL_CFGA, `CS_SI_SEL_CFGA,
            `CS_CMP_SEL_CFGA, /*CFGRAM3:          IDLE: Retain register values*/
        ...
        begin
            ...
            begin            /* Starts new measurement sequence */
                ...
                end                                /* sequence is larger than a damping factor period. */
                else begin
                    damping_q_fb <= 1'b1;
                    damping_nq <= 1'b0;
                end
            ...
            begin      /* Indicates that measurement sequence is complete */
                ...
                /* damping_nq and damping_q_fb are holding theirs previous state */
            end
        end
    ...
    begin
        ...
        /* HW mode without damping factor */
        assign pid_int = 1'b0;
    end
        ...
        case (glitch_filter_state)
            ...
            FILT_ENABLE:                           /* Glitch filter is enabled so wait for terminal count */
                begin
                    if (glitch_filter_tc)
                    begin
                        glitch_filter_state <= FILT_LOAD;    /* TC occured so change the state */
                    end
                end
    ...
    * counter implemented in datapath. Also implements a divided for 16
    ...
    **************************************************************************/
    ...
    begin
        ...
        always @ (posedge synced_tach_clock)
        begin
            drq <= (tach_state[2:0] == 3'b111) | (tach_state[2:0] == 3'b101);
        end
        ...
        begin
            case (tach_state)
                ...
                CLR_EVENTS:                     /* Clear events and generate "next_fan" pulse */
                    begin
                        tach_state <= IDLE;
                    end
            ...
            case (tach_state)
                ...
                TACH_CNT2:                      /* Measure 2nd and 3rd tach pulses. Stall if we timeout */
                    ...
                    begin
                        ...
                        begin
                            tach_cnt3 <= 1'b1;  /* tach_cnt3 is used to emulate TACH_CNT3 state. The separate
                                                * state was not used for counting third tachometer pulse to 
                                                * save udb resources.
                                                */
                        end
                    end
                ...
                CLR_EVENTS:                      /* Clear events and generate "next_fan" pulse */
                    begin
                        tach_state <= IDLE;
						tach_cnt3 <= 1'b0;      /* Clear tach_cnt3 prior to next measurement cycle */
                    end
    ...
    **************************************************************************/
    ...
    begin
        ...
        begin
            ...
            next_fan  <= (tach_state == CLR_EVENTS);
            pulse_tc  <= (tach_state == COMPARE);
        end
    end
...
`endif /* B_FanTach_v4_0_ALREADY_INCLUDED */
