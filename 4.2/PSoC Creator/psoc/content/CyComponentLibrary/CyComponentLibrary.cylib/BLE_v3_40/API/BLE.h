...
#define CYBLE_HCI_TYPE                              (`$HciMode`u)
...
#define CYBLE_GAP_SECURITY_LEVEL_SECURE_CONN        (3u)
    ...
    #define CYBLE_AUTO_POPULATE_WHITELIST           (`$SecurityAutoPopulateWhitelist`)
    #if(CYBLE_M0S8BLESS_VERSION_2)
        #define CYBLE_MAX_RESOLVABLE_DEVICES        (`$SecurityMaxResolvableDevices`)
        #define CYBLE_ENABLE_LL_PRIVACY             (`$SecurityEnableLLPrivacy`)
    #else
        #define CYBLE_MAX_RESOLVABLE_DEVICES        (0u)
        #define CYBLE_ENABLE_LL_PRIVACY             (0u)
    #endif /* CYBLE_M0S8BLESS_VERSION_2 */
    #define CYBLE_GAP_SECURITY_LEVEL                (`$SecurityLevel`)
#else
    #if(CYBLE_M0S8BLESS_VERSION_2)
...
#define CYBLE_SHARING_MODE                          (`$SharingMode`u)  
...
#define CYBLE_STRICT_PAIRING                        (`$SecurityStrictPairing`)
...
#define CYBLE_SECURITY_MODE                         ((`$SecurityMode` == 0u) ? CYBLE_GAP_SEC_MODE_1 : CYBLE_GAP_SEC_MODE_2)
#define CYBLE_SECURITY_LEVEL                        (`$SecurityLevel`)
#define CYBLE_SECURITY_ENC_KEY_SIZE                 (`$SecurityEncryptionKeySize`)
...
#define CYBLE_GATT_MAX_ATTR_BUFF_COUNT       ((`$MaxAttrNoOfBuffer` > 0u) ? (`$MaxAttrNoOfBuffer` - 1u) : 0u)
...
#define CYBLE_GATT_RELIABLE_CHAR_COUNT      (`$ReliableWriteCharCount`)
/* The total length of characteristics with reliable write property */
#define CYBLE_GATT_RELIABLE_CHAR_LENGTH      (`$ReliableWriteCharLength`)
...
#define CYBLE_GATT_ENABLE_EXTERNAL_PREP_WRITE_BUFF  (`$EnableExternalPrepWriteBuff`u)
...
#define CYBLE_L2CAP_ENABLE                              (`$EnableL2capLogicalChannels`u)
...
#if(CYBLE_MODE_PROFILE)
    #if(CYBLE_M0S8BLESS_VERSION_2)
        #define CYBLE_LL_MAX_TX_PAYLOAD_SIZE            (`$MaxTxPayloadSize`)
        #define CYBLE_LL_MAX_RX_PAYLOAD_SIZE            (`$MaxRxPayloadSize`)
