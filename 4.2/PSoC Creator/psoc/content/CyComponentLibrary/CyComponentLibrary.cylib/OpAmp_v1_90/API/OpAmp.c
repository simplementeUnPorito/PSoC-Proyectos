...
void `$INSTANCE_NAME`_Stop(void) `=ReentrantKeil($INSTANCE_NAME . "_Stop")`
{
    ...
    if(`$INSTANCE_NAME`_PM_ACT_CFG_REG == 0u)
    {
...
void `$INSTANCE_NAME`_SetPower(uint8 power) `=ReentrantKeil($INSTANCE_NAME . "_SetPower")`
{
        ...
        `$INSTANCE_NAME`_CR_REG &= (uint8)(~`$INSTANCE_NAME`_PWR_MASK);
        `$INSTANCE_NAME`_CR_REG |= power & `$INSTANCE_NAME`_PWR_MASK;      /* Set device power */
    #else
        CYASSERT(`$INSTANCE_NAME`_HIGHPOWER == power);
