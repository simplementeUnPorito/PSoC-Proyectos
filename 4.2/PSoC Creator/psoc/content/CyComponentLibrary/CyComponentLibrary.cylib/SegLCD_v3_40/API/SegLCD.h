...
#define `$INSTANCE_NAME`_FIND_PIXEL(port, pin, row)          ((uint16)(((((uint16)(row)) << `$INSTANCE_NAME`_ROW_SHIFT) + \
                                                             (((uint16)(port)) << `$INSTANCE_NAME`_BYTE_SHIFT)) + ((uint16)(pin))))
...
***************************************/
#if(CY_PSOC3)
    #define `$INSTANCE_NAME`_DMA_ADDRESS_MASK          (0x0000FFFFul)
#endif /* (CY_PSOC3) */
...
#define `$INSTANCE_NAME`_ISR_NUMBER                ((uint8) `$INSTANCE_NAME`_TD_DoneInt__INTC_NUMBER)
#define `$INSTANCE_NAME`_ISR_PRIORITY              ((uint8) `$INSTANCE_NAME`_TD_DoneInt__INTC_PRIOR_NUM)

#if(CY_PSOC5LP)
    #define `$INSTANCE_NAME`_WAKEUP_ISR_NUMBER         ((uint8) `$INSTANCE_NAME`_Wakeup__INTC_NUMBER)
    #define `$INSTANCE_NAME`_WAKEUP_ISR_PRIORITY       ((uint8) `$INSTANCE_NAME`_Wakeup__INTC_PRIOR_NUM)
#endif /* CY_PSOC5LP */

`$writerHPixelDef`

#define `$INSTANCE_NAME`_LCD_TD_SIZE               ((uint8) `$INSTANCE_NAME`_SegLcdPort_DMA_TD_PROTO_COUNT)

#define `$INSTANCE_NAME`_COM_TD_COUNT              ((uint8) \
                                                   ((`$INSTANCE_NAME`_COMM_NUM * `$INSTANCE_NAME`_LCD_TD_SIZE) - 1u))
...
#define `$INSTANCE_NAME`_BIAS_VOLTAGE          (`$BiasVoltage`u)
...
#define `$INSTANCE_NAME`_LOWDRIVE_STRENGTH     (`$LowDriveStrength`u)
    ...
    #define `$INSTANCE_NAME`_DRIVE_TIME        ((`$INSTANCE_NAME`_LOW_DRIVE_TIME + `$INSTANCE_NAME`_HI_DRIVE_TIME) - 1u)
#else
    ...
    #define `$INSTANCE_NAME`_PWM_PERIOD_VAL        (0x00u)
    /* D0 is equal to HiDrive time, as it used as period reg for Low Power mode */
    #define `$INSTANCE_NAME`_PWM_DRIVE_VAL         (`$INSTANCE_NAME`_HI_DRIVE_TIME - 1u)
    /* Make sure D1 is 0 */
...
#define `$INSTANCE_NAME`_ALIASED_AREA_PTR         ((reg32) CYREG_PRT0_DR_ALIAS)
