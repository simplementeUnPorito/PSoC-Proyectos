...
namespace SegLCD_v3_40
{
    partial class CyAPIGenerator
    {
        ...
        void Write7SegDigit_n(TextWriter writer, TextWriter writer_h, int index, string varDecl)
        {
            ...
            writer.WriteLine("        {");
            writer.WriteLine("            /* Select pixel state for the digit base on value in look-up table */");
            writer.WriteLine("            {0}_WRITE_PIXEL({1}_disp{2}[position][i],", m_instanceName, m_instanceName, index);
            writer.WriteLine("            ((uint8)({0}_7SegDigits[digit] >> i)) & ((uint8){1}_PIXEL_STATE_ON));", m_instanceName, m_instanceName); 
            writer.WriteLine("        }"); 
            writer.WriteLine("    }");
            writer.WriteLine("}");
            writer.WriteLine("");
        ...
        void Write7SegNumber_n(TextWriter writer, TextWriter writer_h, int index)
        {
            ...
            writer.WriteLine("{ ");
            writer.WriteLine("    uint8 i;");
            writer.WriteLine("    uint8 num;");
            writer.WriteLine("");
            writer.WriteLine("    if(0u != {0}_DIGIT_NUM_{1})", m_instanceName, index);
            writer.WriteLine("    {");
            ...
            writer.WriteLine("    {");
            writer.WriteLine("        num = (uint8)(value % 10u);");
        ...
        void PutChar14seg_n(TextWriter writer, TextWriter writer_h, int index, string varDecl)
        {
            ...
            writer.WriteLine("             /* Select pixel state for the digit base on value in look-up table */");
            writer.WriteLine("            {0}_WRITE_PIXEL({1}_disp{2}[position][i], ", m_instanceName, m_instanceName, index);
            writer.WriteLine("            ((uint8)({0}_14SegChars[character] >> i)) & ((uint8) {1}_PIXEL_STATE_ON));", m_instanceName, m_instanceName);
            writer.WriteLine("        }");
            writer.WriteLine("    }");
            writer.WriteLine("}");
            writer.WriteLine("");
        ...
        {
            writer_h.WriteLine("void    {0}_WriteString14Seg_{1}(uint8 const character[], uint8 position) `=ReentrantKeil($INSTANCE_NAME . \"_WriteString14Seg_{1}\")`;", m_instanceName, index);
            ...
            writer.WriteLine("*");
            writer.WriteLine("*******************************************************************************/");
            writer.WriteLine("void {0}_WriteString14Seg_{1}(uint8 const character[], uint8 position) `=ReentrantKeil($INSTANCE_NAME . \"_WriteString14Seg_{1}\")`", m_instanceName, index);
            writer.WriteLine("{");
            writer.WriteLine("    uint8 ch;");
            writer.WriteLine("    uint8 i = 0u;");
            writer.WriteLine("");
        ...
        void PutChar16seg_n(TextWriter writer, TextWriter writer_h, int index, string varDecl)
        {
            ...
            writer.WriteLine("        {");
            writer.WriteLine("            {0}_WRITE_PIXEL({1}_disp{2}[position][i], ", m_instanceName, m_instanceName, index);
            writer.WriteLine("            ((uint8) ({0}_16SegChars[character] >> i)) & ((uint8) {1}_PIXEL_STATE_ON));", m_instanceName, m_instanceName);
            writer.WriteLine("        }");
            writer.WriteLine("    }");
            writer.WriteLine("}");
            writer.WriteLine("");
        ...
        {
            writer_h.WriteLine("void    {0}_WriteString16Seg_{1}(uint8 const character[], uint8 position) `=ReentrantKeil($INSTANCE_NAME . \"_WriteString16Seg_{1}\")`;", m_instanceName, index);
            writer.WriteLine("/*******************************************************************************");
            ...
            writer.WriteLine("*");
            writer.WriteLine("*******************************************************************************/");
            writer.WriteLine("void {0}_WriteString16Seg_{1}(uint8 const character[], uint8 position) `=ReentrantKeil($INSTANCE_NAME . \"_WriteString16Seg_{1}\")`", m_instanceName, index);
            writer.WriteLine("{");
            writer.WriteLine("    uint8 ch;");
            writer.WriteLine("    uint8 i = 0u;");
            writer.WriteLine("");
        ...
        void PutCharDotMatrix_n(TextWriter writer, TextWriter writer_h, int index, string varDecl)
        {
            ...
            writer.WriteLine("            {");
            writer.WriteLine("                {0}_WRITE_PIXEL({1}_disp{2}[position][j + (i * {3}_DM_CHAR_WIDTH)], \\", m_instanceName, m_instanceName, index, m_instanceName);
            writer.WriteLine("                (((uint8) ({0}_charDotMatrix[character][j] >> i)) & ((uint8) {1}_PIXEL_STATE_ON)));", m_instanceName, m_instanceName);
            writer.WriteLine("            }");
            writer.WriteLine("        }");
            writer.WriteLine("    }");
            writer.WriteLine("}");
        ...
        {
            writer_h.WriteLine("void    {0}_WriteStringDotMatrix_{1}(uint8 const character[], uint8 position) `=ReentrantKeil($INSTANCE_NAME . \"_WriteStringDotMatrix_{1}\")`;", m_instanceName, index);
            ...
            writer.WriteLine("*");
            writer.WriteLine("*******************************************************************************/");
            writer.WriteLine("void {0}_WriteStringDotMatrix_{1}(uint8 const character[], uint8 position) `=ReentrantKeil($INSTANCE_NAME . \"_WriteStringDotMatrix_{1}\")`", m_instanceName, index);
            writer.WriteLine("{");
            writer.WriteLine("    uint8 ch;");
            writer.WriteLine("    uint8 i = 0u;");
            writer.WriteLine("");
        ...
        void WriteBargraph_n(TextWriter writer, TextWriter writer_h, int index, int maxNumber, string varDecl)
        {
            ...
            writer.WriteLine("        }");
            writer.WriteLine("        ");
            writer.WriteLine("        for(i = 1u; i <= ((uint16){0}_DIGIT_NUM_{1}); i++) ", m_instanceName, index);
            writer.WriteLine("        {");
            ...
            writer.WriteLine("                {0}_WRITE_PIXEL({1}_disp{2}[locationInt][0u], {3}_PIXEL_STATE_ON);", m_instanceName, m_instanceName, index, m_instanceName);
            writer.WriteLine("                break;");
            writer.WriteLine("            case 1:");
            writer.WriteLine("                for(i = locationInt; i >= 1u; i--) ");
            writer.WriteLine("                {");
            writer.WriteLine("                    {0}_WRITE_PIXEL({1}_disp{2}[i][0u], {3}_PIXEL_STATE_ON);", m_instanceName, m_instanceName, index, m_instanceName);
            writer.WriteLine("                }");
            writer.WriteLine("                break;");            
            writer.WriteLine("            case -1:");
            ...
            writer.WriteLine("                if(((uint16) ((locationInt + ((uint16) modeInt)) - 1u)) <= maxValue) ");
            writer.WriteLine("                {");
            writer.WriteLine("                    maxValue = (locationInt + ((uint16) modeInt)) - 1u;");
            writer.WriteLine("                }");
            writer.WriteLine("                for(i = locationInt; i <= maxValue; i++) ");
            writer.WriteLine("                {");
            writer.WriteLine("                    {0}_WRITE_PIXEL({1}_disp{2}[i][0u], {3}_PIXEL_STATE_ON);", m_instanceName, m_instanceName, index, m_instanceName);
