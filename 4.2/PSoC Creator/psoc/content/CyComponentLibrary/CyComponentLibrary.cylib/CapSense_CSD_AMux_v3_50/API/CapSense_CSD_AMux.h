...
#if !defined(CY_CAPSENSE_CSD_AMUX_`$INSTANCE_NAME`_H)
#define CY_CAPSENSE_CSD_AMUX_`$INSTANCE_NAME`_H
...
void `$INSTANCE_NAME`_Select(uint8 channel) CYREENTRANT;
void `$INSTANCE_NAME`_FastSelect(uint8 channel) CYREENTRANT;
void `$INSTANCE_NAME`_DisconnectAll(void) CYREENTRANT;
...
#define `$INSTANCE_NAME`_CHANNELS  (`$Channels`u + 2u +`$IdacEnable`u+ `$VrefEnable`u)
...
#else
    void `$INSTANCE_NAME`_Connect(uint8 channel) CYREENTRANT;
    void `$INSTANCE_NAME`_Disconnect(uint8 channel) CYREENTRANT;
#endif  /* End (`$INSTANCE_NAME`_MUXTYPE == `$INSTANCE_NAME`_MUX_SINGLE) */

#endif /* CY_CAPSENSE_CSD_AMUX_`$INSTANCE_NAME`_H */
