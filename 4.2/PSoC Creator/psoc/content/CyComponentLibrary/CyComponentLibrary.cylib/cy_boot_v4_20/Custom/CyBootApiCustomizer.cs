...
namespace cy_boot_v4_20
{
    public class CyCustomizer : ICyAPICustomize_v2
    {
                                                          ...
                                                          IEnumerable<CyAPICustomizer> apis)
        {
            ...
            bool enableCmsis = "0" != (args.APIFitterQuery.GetFitterAPIEntry(USE_BUNDLED_CMSIS) ?? "1").TrimEnd('u');
                    ...
                    case STARTUP_CM3_IAR:
                        if (projType == PROJ_TYPE_LOADABLE && appCount > 1)
                        {
                            ...
                            for (int i = 1; i <= appCount; i++)
                            {
                                ...
                                newContent = newContent.Replace("`$CYLOADER_PRJ`", "0");
                        ...
                        else
                        {
                            ...
                            if (projType == PROJ_TYPE_LOADER || projType == PROJ_TYPE_LOADER_MULTIAPP)
                                newContent = newContent.Replace("`$CYLOADER_PRJ`", "1");
                            else
                                newContent = newContent.Replace("`$CYLOADER_PRJ`", "0");
