...
namespace ADC_SAR_v3_10
{
    ...
    public class CyParamRange
    {
        ...
        public const uint MAX_FREQ_KHZ = 18000;
        public const uint MAX_FREQ_NOT_BYPASSED_KHZ = 1600;
    ...
    public class CyParameters
    {
        ...
        public bool m_clockNeedsUpdate = false;
        ...
        public uint MaxClockFrequencyKHz
        {
            get
            {
                // Max clock frequency depends on the Input configuration.
                // SamplePrechange is the number of additional clocks for sampling data and can be equal 4 or 6.
                // 4 corresponds the following configuration:
                // 1) ADC Reference equals "Internal Vref"
                // 2) ADC Input range is NOT "0.0 +/- Vdda (Differential)  -Input +/- Vdda"
                return (this.SamplePrecharge > 4)
                  ? CyParamRange.MAX_FREQ_KHZ
                  : CyParamRange.MAX_FREQ_NOT_BYPASSED_KHZ;
            }
        }
        ...
        public uint GetMaxSampleRateForInternalVref()
        {
            return (this.AdcInputRange == CyEAdcInputRangeType.Vneg_Vdda_Diff)
                ? GetMaxSampleRate() / 2 // 9 MHz
                : GetMaxSampleRate();    // 1.6 MHz
        }
