...
namespace BoostConv_v5_0
{
    ...
    public class CyCustomizer : ICyParamEditHook_v1, ICyInstValidateHook_v1, ICyDRCProvider_v1
    {
        ...
        public IEnumerable<CyDRCInfo_v1> GetDRCs(ICyDRCProviderArgs_v1 args)
        {
            ...
            if(parameters.Frequency == CyFrequency.SWITCH_FREQ_32KHZ)
            {
                if (parameters.IsPSoC5LP())
                {
                    yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Error,
                        Properties.Resources.DRCSwitchingFreqPSoC5LPLimit);                    
                }
                else if (parameters.IsPSoC3())
                {
                    yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Error,
                        Properties.Resources.DRCSwitchingFreqPSoC3Limit);                                        
                }
            }

            if ((parameters.ExternalClockSrc != CyClockFr.EXTCLK_NONE) &&
                  (parameters.IsPSoC5LP()))
            {
                yield return new CyDRCInfo_v1(CyDRCInfo_v1.CyDRCType_v1.Error,
                    Properties.Resources.DRCClockSourcePSoC5LPLimit);
            }
        }
    }
}
