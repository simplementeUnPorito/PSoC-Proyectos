...
`$CY_API_CALLBACK_HEADER_INCLUDE`
    ...
    *******************************************************************************/
    CY_ISR(`$INSTANCE_NAME`_DATA_SEND_ISR)
    {
        ...
        fanNum = `$INSTANCE_NAME`_TACH_ADDR_CONTROL_REG & `$INSTANCE_NAME`_TACH_CNTL_NUM_FANS_MASK;
        
        /* Get measured actual speed period for a specific fan */
        actSpeed = CY_GET_REG16(`$INSTANCE_NAME`_TACH_STORED_ACT_PERIOD_PTR);
        ...
        if(0u == actSpeed)
        {
            `$INSTANCE_NAME`_stallStatus |= (uint16) ((((uint16) 1u) << fanNum));
        }

        /* Calculate the new fan number value */
        fanNum = ((fanNum == 0u) ? ((uint8)(`$INSTANCE_NAME`_NUMBER_OF_FANS - 1u)) : (fanNum - 1u));
        
        /* Clear the previous fan number value in the control register */
        `$INSTANCE_NAME`_TACH_ADDR_CONTROL_REG &= ((uint8) ~`$INSTANCE_NAME`_TACH_CNTL_NUM_FANS_MASK);

        /* Set new fan number value and End-Of-Measurement bit if required */
        `$INSTANCE_NAME`_TACH_ADDR_CONTROL_REG |= 
                fanNum | ((fanNum == ((uint8)(`$INSTANCE_NAME`_NUMBER_OF_FANS - 1u))) ? `$INSTANCE_NAME`_TACH_EOM : 0u);
        ...
        */
        if(((uint8)(`$INSTANCE_NAME`_NUMBER_OF_FANS - 1u)) == fanNum)
        {
            ...
            if (0u != (`$INSTANCE_NAME`_stallStatus & `$INSTANCE_NAME`_alertMask))
            {
                `$INSTANCE_NAME`_GLOBAL_CONTROL_REG |= `$INSTANCE_NAME`_SW_STALL;
            }
        }
        ...
        for(i = 0u; i < `$INSTANCE_NAME`_NUMBER_OF_FANS; i++)
        {
            ...
            **************************************/
            ...
            if (`$INSTANCE_NAME`_SPEED_ERRORS_TRESHOLD < `$INSTANCE_NAME`_spdErrCount[i])
            {
                ...
                */
                mask = `$INSTANCE_NAME`_GetAlertMask();
