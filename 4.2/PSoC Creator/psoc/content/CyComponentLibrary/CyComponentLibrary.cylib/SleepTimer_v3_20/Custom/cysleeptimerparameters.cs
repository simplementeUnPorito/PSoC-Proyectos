...
namespace SleepTimer_v3_20
{
    ...
    public class CySleepTimerParameters
    {
        ...
        public static readonly string[] ALLOWED_INTERVAL = { "4", "8", "16", "32", "64", "128", "256" };
        #endregion
        ...
        public bool IsIntervalAllowedForPSoC5A()
        {
            return CySleepTimerParameters.IsIntervalAllowedForPSoC5A(this.Interval);
        }

        public static bool IsPSoC5A(ICyDeviceQuery_v1 deviceQuery)
        {
            return (deviceQuery.ArchFamilyMember == "PSoC5A");
        }

        public static bool IsIntervalAllowedForPSoC5A(string interval)
        {
            int? index = null;
            for (int i = 0; i < ALLOWED_INTERVAL.Length; i++)
            {
                if (interval == (PREFIX + ALLOWED_INTERVAL[i] + POSTFIX))
                {
                    index = i;
                    break;
                }
