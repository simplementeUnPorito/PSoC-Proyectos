...
*  CYBLE_EVT_CPSC_NOTIFICATION event at the GATT Client's end.
* 
...
*  which results in CYBLE_EVT_CPSC_INDICATION or 
...
*  before. For update broadcasting data this function must be called when 
*  CyBle_GetBleSsState() returns CYBLE_BLESS_STATE_EVENT_CLOSE state.
*    
*  It is available only in Broadcaster role.  
* 
*  \param advInterval: Advertising interval in 625 us units. The valid range 
*                      is from CYBLE_GAP_ADV_ADVERT_INTERVAL_NONCON_MIN
*                      to CYBLE_GAP_ADV_ADVERT_INTERVAL_MAX.
*  \param attrSize:    The size of the characteristic value attribute.
*                      This size is limited by maximum advertising packet 
*                      length and advertising header size.
*  \param attrValue:   The pointer to the Cycling Power Measurement 
*                      characteristic that include the mandatory fields (e.g. 
*                      the Flags field and the Instantaneous Power field) and 
*                      depending on the Flags field,  some optional fields in 
*                      a non connectable undirected advertising event.
* 
* \return
...
*   procedure will continue until you call CyBle_GapcStopObserve().
