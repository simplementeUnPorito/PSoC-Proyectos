...
void CyBLE_Uart_Start (void)
{
        ...
        /* Setup ISR */
        `$INSTANCE_NAME`_uart_isr_StartEx(&CyBLE_Uart_Interrupt);

        `$INSTANCE_NAME`_HAL_Uart_Start();
...
void CyBLE_Uart_Stop (void)
{
        ...
        `$INSTANCE_NAME`_uart_isr_Stop();

        `$INSTANCE_NAME`_HAL_Uart_Stop();
    #endif /* (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART) */
}
...
{
    #if (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART)
        `$INSTANCE_NAME`_HAL_Uart_SpiUartPutArray(dataBuf, (uint32)length);
    #endif /* (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART) */
}
...
{
    #if (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART)
        `$INSTANCE_NAME`_uart_isr_Enable();
    #endif /* (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART) */
}
...
{
    #if (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART)
        `$INSTANCE_NAME`_uart_isr_Disable();
    #endif /* (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART) */
}
...
/* Interface to `$INSTANCE_NAME`_bless_isr_Enable function */
void CyBleHal_EnableGlobalInterrupts(void)
{
    `$INSTANCE_NAME`_bless_isr_Enable();
}

/* Interface to `$INSTANCE_NAME`_bless_isr_Disable function */
void CyBleHal_DisableGlobalInterrupts(void)
{
    `$INSTANCE_NAME`_bless_isr_Disable();
}
...
void CyBle_HalInit(void)
{
    ...
    `$INSTANCE_NAME`_bless_isr_SetVector(&CyBLE_Bless_Interrupt);

    /* Set the priority. */
    `$INSTANCE_NAME`_bless_isr_SetPriority((uint8)`$INSTANCE_NAME`_bless_isr_INTC_PRIOR_NUMBER);
}
...
void CYBLE_BlessStart(void)
{
    ...
    `$INSTANCE_NAME`_bless_isr_StartEx(&CyBLE_Bless_Interrupt);
}
