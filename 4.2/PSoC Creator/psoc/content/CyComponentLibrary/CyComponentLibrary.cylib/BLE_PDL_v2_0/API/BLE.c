...
cy_en_ble_api_result_t Cy_BLE_Start(cy_ble_callback_t callbackFunc)
{
    ...
    if(callbackFunc != NULL)
    {
        ...
        cy_ble_config.blessIsrConfig = &`$INSTANCE_NAME`_bless_isr_cfg;
    #endif /* (CY_BLE_CONFIG_STACK_CONTR_CORE) */

    #if(CY_BLE_CONFIG_STACK_MODE_HOST_UART)
        cy_ble_config.uartHostIsrConfig = &`$INSTANCE_NAME`_host_uart_isr_cfg;
        cy_ble_config.uartHostConfig = &`$INSTANCE_NAME`_Host_Uart_config;
        cy_ble_config.uartHostHw = `$INSTANCE_NAME`_Host_Uart_HW;
    #endif  /* (CY_BLE_CONFIG_STACK_MODE_HOST_UART) */
...
#else
{
    ...
    {
    #if(CY_BLE_CONFIG_STACK_CONTR_CORE)
        .blessIsrConfig = &`$INSTANCE_NAME`_bless_isr_cfg,
    #endif /* (CY_BLE_CONFIG_STACK_CONTR_CORE) */
    
    #if(CY_BLE_CONFIG_STACK_MODE_CONTR_UART)
        .uartIsrConfig = &`$INSTANCE_NAME`_uart_isr_cfg,
        .uartConfig = &`$INSTANCE_NAME`_HAL_Uart_config,
        .uartHw = `$INSTANCE_NAME`_HAL_Uart_HW
    #endif /* CY_BLE_CONFIG_STACK_MODE_CONTR_UART */
    };
