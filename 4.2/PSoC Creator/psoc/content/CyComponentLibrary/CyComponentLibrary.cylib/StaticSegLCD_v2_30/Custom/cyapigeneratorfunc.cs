...
namespace StaticSegLCD_v2_30
{
    partial class CyAPIGenerator
    {
        ...
        void Write7SegDigit_n(TextWriter writer, TextWriter writer_h, int index, string varDecl)
        {
            ...
            writer.WriteLine("    }");
            writer.WriteLine("");
            writer.WriteLine("    /* Check if position<{0}_DIGIT_NUM_{1}, if not then do 'mod' operation */", m_instanceName, index);
        ...
        void PutChar14seg_n(TextWriter writer, TextWriter writer_h, int index, string varDecl)
        {
            ...
            writer.WriteLine("               ((uint8) ({0}_14SegChars[character] >> i)) & {0}_PIXEL_STATE_ON);", m_instanceName);
            writer.WriteLine("        }");
            writer.WriteLine("    }");
            writer.WriteLine("}");
            writer.WriteLine("");
        ...
        void PutChar16seg_n(TextWriter writer, TextWriter writer_h, int index, string varDecl)
        {
            ...
            writer.WriteLine("        {");
            writer.WriteLine("            {0}_WRITE_PIXEL({0}_disp{1}[position][i],", m_instanceName, index);
            writer.WriteLine("                ((uint8) ({0}_16SegChars[character] >> i)) & {0}_PIXEL_STATE_ON);", m_instanceName);
            writer.WriteLine("        }");
            writer.WriteLine("    }");
            writer.WriteLine("}");
        }
        ...
        void WriteBargraph_n(TextWriter writer, TextWriter writer_h, int index, int maxNumber, string varDecl)
        {
            ...
            writer.WriteLine("        }");
            writer.WriteLine("        ");
            writer.WriteLine("        for(i = 1u; i <= {0}_DIGIT_NUM_{1}; i++) ", m_instanceName, index);
            writer.WriteLine("        {");
            ...
            writer.WriteLine("                if(((locationInt + ((uint16) modeInt)) - 1u) <= maxValue) ");
            writer.WriteLine("                {");
