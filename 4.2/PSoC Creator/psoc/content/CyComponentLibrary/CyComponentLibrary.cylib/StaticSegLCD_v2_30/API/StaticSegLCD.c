        ...
        for(i = 0u; i < `$INSTANCE_NAME`_LCD_TD_SIZE; i++)
        {
            ...
            if(((j * `$INSTANCE_NAME`_LCD_TD_SIZE) + i) == (comTdCount - 1u))
            {
                (void) CyDmaTdSetConfiguration(`$INSTANCE_NAME`_lcdTd[(j * `$INSTANCE_NAME`_LCD_TD_SIZE) + i],
                                        (uint16) `$INSTANCE_NAME`_StSegLcdPort_DMA_TD_PROTO_BLOCK[i].length,
            ...
            */
            (void) CyDmaTdSetAddress(`$INSTANCE_NAME`_lcdTd[(j * `$INSTANCE_NAME`_LCD_TD_SIZE) + i],
                              ...
                              (uint16) `$INSTANCE_NAME`_StSegLcdPort_DMA_TD_PROTO_BLOCK[i].offset,
...
uint8 `$INSTANCE_NAME`_WritePixel(uint16 pixelNumber, uint8 pixelState) `=ReentrantKeil($INSTANCE_NAME . "_WritePixel")`
{
    ...
    */
    ...
    else
    {
        ...
        byteAddr = (pixelNumber & `$INSTANCE_NAME`_PORT_MASK) >> `$INSTANCE_NAME`_PORT_SHIFT;
        ...
        mask = ~(uint8) (((uint8) `$INSTANCE_NAME`_PIXEL_STATE_ON) << bitAddr);
...
uint8 `$INSTANCE_NAME`_WriteInvertState(uint8 invertState) `=ReentrantKeil($INSTANCE_NAME . "_WriteInvertState")`
{
    ...
    if(invertState <= `$INSTANCE_NAME`_INVERTED_STATE)
    {
        ...
        * set it to "0".
        */
        if(`$INSTANCE_NAME`_INVERTED_STATE == invertState)
        {
            `$INSTANCE_NAME`_WRITE_PIXEL(`$INSTANCE_NAME`_commons[0u], `$INSTANCE_NAME`_PIXEL_STATE_ON);
        }
        else
        {
            `$INSTANCE_NAME`_WRITE_PIXEL(`$INSTANCE_NAME`_commons[0u], `$INSTANCE_NAME`_PIXEL_STATE_OFF);
...
*
* Return:
*  `$INSTANCE_NAME`_NORMAL_STATE (0) - for normal non-inverted display.
*  `$INSTANCE_NAME`_INVERTED_STATE (1) -  for inverted display.
*
*******************************************************************************/
uint8 `$INSTANCE_NAME`_ReadInvertState(void) `=ReentrantKeil($INSTANCE_NAME . "_ReadInvertState")`
{
    uint8 state = `$INSTANCE_NAME`_INVERTED_STATE;

    /* Get only invert bit of Driver Control Register */
    if(`$INSTANCE_NAME`_PIXEL_STATE_OFF == `$INSTANCE_NAME`_ReadPixel(`$INSTANCE_NAME`_commons[0u]))
    {
        state = `$INSTANCE_NAME`_NORMAL_STATE;
    }
