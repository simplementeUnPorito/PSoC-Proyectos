...
extern uint8 CYDATA `$INSTANCE_NAME`_lastAuxVoltChan;
    ...
    extern uint8 CYDATA `$INSTANCE_NAME`_index;
#endif /* (`$INSTANCE_NAME`_NUM_CURRENT_SOURCES > 0u) */

#if (`$INSTANCE_NAME`_NUM_AUX_INPUTS > 0u)
    extern uint8 CYDATA `$INSTANCE_NAME`_auxIndex;
    extern int16 `$INSTANCE_NAME`_auxStore;
...
`$ActiveCurrentChan`
    ...
    volatile uint8 `$INSTANCE_NAME`_auxEnableMask;
#endif /* (`$INSTANCE_NAME`_NUM_AUX_INPUTS > 0u) */

/* Local function protoptype */
static void `$INSTANCE_NAME`_StartupIsr(void)                                           \
                                                                `=ReentrantKeil($INSTANCE_NAME . "_StartupIsr")`;
static int32 `$INSTANCE_NAME`_mVToCounts(uint16 mV, uint8 chan, uint8 * convSuccess)     \
...
void `$INSTANCE_NAME`_Init(void) `=ReentrantKeil($INSTANCE_NAME . "_Init")`
{
    ...
    `$INSTANCE_NAME`_SetWarnMask(`$INSTANCE_NAME`_DEFAULT_WARN_MASK);
        ...
        `$INSTANCE_NAME`_auxEnableMask = `$INSTANCE_NAME`_DEFAULT_AUX_CHANNELS_MASK;
    #endif /* (`$INSTANCE_NAME`_NUM_AUX_INPUTS > 0) */
    `$INSTANCE_NAME`_maxChannels = `$INSTANCE_NAME`_MAX_CHANNELS;
}
...
/*******************************************************************************
* Function Name: `$INSTANCE_NAME`_GetAuxiliaryVoltage
********************************************************************************
*
* Summary:
                                                                ...
                                                                `=ReentrantKeil($INSTANCE_NAME . "_mVToCounts")`
{
    ...
    *convSuccess = `$INSTANCE_NAME`_CYFALSE;
    if ((chan > 0u) && (chan <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        ...
        {
            counts = pinV * `$INSTANCE_NAME`_VOLTAGE_FILTER_SIZE + 0.5f;   /* runing total */
        }
    }
    return (int32)(counts);
}
    ...
    *convSuccess = `$INSTANCE_NAME`_CYFALSE;
    if ((chan > 0u) && (chan <= `$INSTANCE_NAME`_NUMBER_OF_CONVERTERS))
    {
        if(`$INSTANCE_NAME`_CurrentType[chan - 1u] != `$INSTANCE_NAME`_CURRENT_TYPE_NA)
        {
            ...
            if(*convSuccess == `$INSTANCE_NAME`_CYTRUE)
            {
                counts = pinV * `$INSTANCE_NAME`_CURRENT_FILTER_SIZE + 0.5f;   /* runing total */
            }
        }
    }
    return (int32)(counts);
...
static uint8 `$INSTANCE_NAME`_GetArrayIndex(uint8 chan) `=ReentrantKeil($INSTANCE_NAME . "_GetArrayIndex")`
{
    ...
    /*******************************************************************************
    * Function Name: `$INSTANCE_NAME`_SetAuxiliarySampleMode()
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
                                            ...
                                            `=ReentrantKeil($INSTANCE_NAME . "_SetAuxiliarySampleMode")`
    {
        ...
        if ((auxNum > 0u) && (auxNum <= `$INSTANCE_NAME`_NUM_AUX_INPUTS))
        {
            auxMask = (uint8)(0x01u << (auxNum - 1u));

            /* Change auxiliary channels enable mask if it is required */
            if( ((sampleMode == `$INSTANCE_NAME`_CONTINUOUS) && ((`$INSTANCE_NAME`_auxEnableMask & auxMask) == 0u)) ||
                ((sampleMode != `$INSTANCE_NAME`_CONTINUOUS) && ((`$INSTANCE_NAME`_auxEnableMask & auxMask) != 0u)) )
            {
                ...
                {
                    `$INSTANCE_NAME`_auxEnableMask |= auxMask;
                    /* Update total channels number */
                    `$INSTANCE_NAME`_maxChannels++;
                }
                else /* `$INSTANCE_NAME`_ON_DEMAND */
                {
                    `$INSTANCE_NAME`_auxEnableMask &= (uint8)~auxMask;
                    /* Update total channels number */
                    `$INSTANCE_NAME`_maxChannels--;
                }
                /* Enable interrupt */
    ...
    /*******************************************************************************
    * Function Name: `$INSTANCE_NAME`_GetAuxiliarySampleMode()
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 auxMask;
        uint8 status = `$INSTANCE_NAME`_ON_DEMAND;

        if ((auxNum > 0u) && (auxNum <= `$INSTANCE_NAME`_NUM_AUX_INPUTS))
        {
            auxMask = (uint8)(0x01u << (auxNum - 1u));

            if((`$INSTANCE_NAME`_auxEnableMask & auxMask) != 0u)
            {
                status = `$INSTANCE_NAME`_CONTINUOUS;
            }
        }
        else
        {
    ...
    /*******************************************************************************
    * Function Name: `$INSTANCE_NAME`_RequestAuxiliarySample()
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
                                            ...
                                            `=ReentrantKeil($INSTANCE_NAME . "_RequestAuxiliarySample")`
    {
        ...
        if ((auxNum > 0u) && (auxNum <= `$INSTANCE_NAME`_NUM_AUX_INPUTS))
        {
            ...
            `$INSTANCE_NAME`_adcConvNext       = `$INSTANCE_NAME`_STATE_MEASURE_AUX;
            `$INSTANCE_NAME`_auxIndex          = auxNum - 1;
            ...
            ***************************************************************************/
            while ((`$INSTANCE_NAME`_adcConvNext == `$INSTANCE_NAME`_STATE_MEASURE_AUX) ||   /* wait start */
                   (`$INSTANCE_NAME`_adcConvNow  == `$INSTANCE_NAME`_STATE_MEASURE_AUX));    /* wait finish*/
            /* read mesured value */
            retval  = (float)`$INSTANCE_NAME`_auxStore;
