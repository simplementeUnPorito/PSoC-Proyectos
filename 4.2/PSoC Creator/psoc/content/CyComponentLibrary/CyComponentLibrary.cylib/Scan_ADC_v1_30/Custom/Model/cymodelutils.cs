    ...
    public static class CyModelUtils
    {
        ...
        public static List<uint> ConfigCtrlVrefSelMasks(CyParameters parameters)
        {
            ...
            for (int configNum = 0; configNum < parameters.NumberOfConfigs; configNum++)
            {
                ...
                mask = mask << 0x4;
                if (configModel.VrefBypass && ((mask != 0x60) || (mask != 0x70)))    
                {
        ...
        /// <summary>
        /// Return true if any configuration uses the SoC terminal or false if not.
        /// </summary>
        /// <param name="parameters">Scan_ADC symbol parameters</param>
        /// <returns>True iff any active configuration uses an external Vref</returns>
        public static bool AnyConfigHasUseSoc(CyParameters parameters)
        {
        ...
        public static bool AdcClockOutOfRange(CyParameters parameters, int configNum)
        {
            ...
            var config = parameters.GetConfig(configNum);
            if ((config.ActualAdcClockHz < config.MinSarFrequency)
                || (config.ActualAdcClockHz > config.MaxSarFrequency))
            {
                return true;
            }
