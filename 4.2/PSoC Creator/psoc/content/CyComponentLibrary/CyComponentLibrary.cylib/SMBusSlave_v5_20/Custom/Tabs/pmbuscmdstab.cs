...
namespace SMBusSlave_v5_20
{
    public partial class CyPmBusCmdsTab : CyTabWrapper
    {
        ...
        public void SetPMBusCmdsTabVisibility(CyEModeSelType selectedMode)
        {
            ...
            m_params.CustomCmdsTab.ShowHideImportExportAllFeature(selectedMode != CyEModeSelType.SMBUS_SLAVE);
        }
        #endregion
        ...
        public override void UpdateUIFromTable(int row)
        {
            ...
            {
                CyParameters.SetCellReadOnlyState(dgv, colSize.Index, row, false);
            }
            ...
            // Most of fields of PAGE and QUERY commands should be readonly
            if (CyCustomTable.IsCmdPageOrQuery(m_params.PmBusTable[row].Name))
            {
                ...
                bool isPageCmd = (m_params.PmBusTable[row].Name == CyCustomTable.PAGE);
                CyParameters.SetCellReadOnlyState(writeConfigCell, (isPageCmd == false));
                CyParameters.SetCellReadOnlyState(readConfigCell, (isPageCmd == false));
            }
            ...
            if (group != null)
            {
                switch ((CyECmdGroup)group)
                {
                    ...
                    case CyECmdGroup.GROUP6:
                        ...
                        writeConfigCell.Items.Remove(CyEnumConverter.GetEnumDesc(CyEReadWriteConfigType.None));
                        readConfigCell.Items.Remove(CyEnumConverter.GetEnumDesc(CyEReadWriteConfigType.None));
                        break;
        ...
        private void dataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (m_params.GlobalEditMode)
            {
                ...
                {
                    ValidateCodes();
                    m_params.CustomCmdsTab.ValidateCodes(); // update error providers on Custom tab
                }
            }
        }
        ...
        protected override bool RangeCheck(int rowIndex, int colIndex)
        {
            ...
            if (dgv[colReference.Index, rowIndex].Value != null)
            {
                ...
                {
                    if (dgv[colIndex, rowIndex].ReadOnly == false && pmBusTableRow.m_enable)
                    {
