...
namespace Timer_v2_70
{
    public partial class CyTimerControl : UserControl
    {
        ...
        private void SetAParameter(Control sender, string parameter, string value)
        {
            ...
            if (m_component.GetCommittedParam(parameter).ErrorCount != 0)
            {
                ...
                ep_Errors.SetError(sender,
                    String.Format(CyTimerParameters.S_PARAM_MESSAGE1, parameter, value, errors));
            }
            else
            {
                ep_Errors.SetError(sender, String.Empty);
            }
        }
        ...
        {
            UnhookUpdateEvents(); // Unhook events so that none of the changes made here causes a parameter to be set 
            CyTimerParameters prms = new CyTimerParameters(inst);

            // Uncheck all radio buttons before cheking one of them, so single radio button 
            ...
            switch (prms.m_resolution.Value)
            {
            ...
            m_numPeriod.Value = Convert.ToInt64(prms.m_period.Value) + 1;
            ...
            ep_Errors.SetError(m_cbTriggerMode, errMsg);
            ...
            errMsg = (err.IsOk) ? string.Empty : err.Message;
            ep_Errors.SetError(m_cbEnableMode, errMsg);
            ...
            errMsg = (err.IsOk) ? string.Empty : err.Message;
            ep_Errors.SetError(m_cbCaptureMode, errMsg);


            //Grab Capture Count Information to populate the Capture Type boxes
            m_chbxEnableCaptureCounter.Checked = (prms.m_captureCounterEnabled.Value == "true");
            ...
            try
            {
                m_numCaptureCount.Value = Convert.ToDecimal(prms.m_captureCount.Value);
            }
            ...
            m_chbxIntSrcCapture.Checked = prms.m_interruptOnCapture.Value == "true" ? true : false;
            m_chbxIntSrcTC.Checked = prms.m_interruptOnTC.Value == "true" ? true : false;
            m_chbxIntSrcFifoFull.Checked = prms.m_interruptOnFIFOFull.Value == "true" ? true : false;

            m_numInterruptOnCaptureCount.Value = Convert.ToDecimal(prms.m_numberOfCaptures.Value);
        ...
        public void UpdateCaptureModeCB(bool fixedFunction)
        {
            if (fixedFunction)
            {
                ...
                m_cbCaptureMode.Items.Add(m_defaultCaptureModeFF);
                if (m_captureModeFFIndex >= 0 && m_captureModeFFIndex < m_cbCaptureMode.Items.Count)
        ...
        {
            SetAParameter(m_chbxIntSrcCapture, CyTimerParameters.S_INTERRUPTONCAPTURE,
                          m_chbxIntSrcCapture.Checked ? "true" : "false");
            UpdateInterruptOnCapture();
        }
        ...
        {
            SetAParameter(m_numCaptureCount, CyTimerParameters.S_CAPTURECOUNTERENABLED,
                         (m_chbxEnableCaptureCounter.Visible && m_chbxEnableCaptureCounter.Checked) ? "true" : "false");
            UpdateCaptureCounter(m_chbxEnableCaptureCounter.Visible, m_chbxEnableCaptureCounter.Checked);
        }
        ...
        {
            SetAParameter(m_chbxIntSrcFifoFull, CyTimerParameters.S_INTERRUPTONFIFOFULL,
                          m_chbxIntSrcFifoFull.Checked ? "true" : "false");
        }

        private void CheckEnableModeRunModeConflict()
        {
            if ((m_rbUDB.Checked == true) && (m_cbEnableMode.Text.Equals(CyTimerParameters.S_HARDWARE_ONLY)) &&
        ...
        private void m_numInterruptOnCaptureCount_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (!ce.Cancel)
            {
                SetAParameter(m_numInterruptOnCaptureCount, CyTimerParameters.S_NUMBEROFCAPTURES,
                              m_numInterruptOnCaptureCount.Value.ToString());
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void m_numCaptureCount_DownEvent(object sender, CyDownButtonEventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void m_numCaptureCount_UpEvent(object sender, CyUpButtonEventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void m_numInterruptOnCaptureCount_DownEvent(object sender, CyDownButtonEventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void m_numInterruptOnCaptureCount_UpEvent(object sender, CyUpButtonEventArgs e)
        {
        ...
        private void CyTimerControl_Load(object sender, EventArgs e)
        {
    ...
    /// <summary>
    /// Custom Event Args for the UpButtonEvent Delegate needs to allow 
    /// the event to be cancelled before the value is changed
    /// </summary>
    ...
    /// <summary>
    /// Custom Event Args for the DownButtonEvent Delegate needs to allow 
    /// the event to be cancelled before the value is changed
    /// </summary>
