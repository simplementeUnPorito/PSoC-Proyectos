...
*  call `$INSTANCE_NAME`_UpdateTemperature() function if the temperature of the
...
*
*******************************************************************************/
cystatus `$INSTANCE_NAME`_WriteByte(uint8 dataByte, uint16 address) `=ReentrantKeil($INSTANCE_NAME . "_WriteByte")`
{
    cystatus status;
    uint16 rowNumber;
    uint16 byteNumber;
    ...
    if (address < CY_EEPROM_SIZE)
    {
        ...
        if(CYRET_SUCCESS == CySpcLock())
        {
            status = CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, byteNumber, &dataByte, \
                                                                    `$INSTANCE_NAME`_SPC_BYTE_WRITE_SIZE);
...
/*******************************************************************************
* Function Name: `$INSTANCE_NAME`_UpdateTemperature
********************************************************************************
*
* Summary:
...
cystatus `$INSTANCE_NAME`_EraseSector(uint8 sectorNumber) `=ReentrantKeil($INSTANCE_NAME . "_EraseSector")`
{
    ...
    if(sectorNumber < (uint8) `$INSTANCE_NAME`_SECTORS_NUMBER)
    {
...
*  `$INSTANCE_NAME`_UpdateTemperature() function if the temperature of the
...
*  `$INSTANCE_NAME`_Query(). `$INSTANCE_NAME`_Query() must be called
*  until it returns a status other than CYRET_STARTED. That indicates that the
*  write has completed. Until `$INSTANCE_NAME`_Query() detects that
...
*  you should call `$INSTANCE_NAME`_UpdateTemperature() API if the temperature
...
/*******************************************************************************
* Function Name: `$INSTANCE_NAME`_StartErase
********************************************************************************
*
* Summary:
...
*  must be used in combination with `$INSTANCE_NAME`_Query().
*  `$INSTANCE_NAME`_Query() must be called until it returns a status
*  other than CYRET_STARTED. That indicates the erase has been completed.
*  Until `$INSTANCE_NAME`_Query() detects that the erase is
...
*
* Summary:
*  Checks the status of an earlier call to `$INSTANCE_NAME`_StartWrite() or
*  `$INSTANCE_NAME`_StartErase().
...
/*******************************************************************************
* Function Name: `$INSTANCE_NAME`_ByteWritePos
********************************************************************************
*
* Summary:
    ...
    if((rowNumber < (uint8) CY_EEPROM_NUMBER_ROWS) && (byteNumber < (uint8) SIZEOF_EEPROM_ROW))
    {
        ...
        if(CySpcLock() == CYRET_SUCCESS)
        {
            ...
            if(CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, (uint16)byteNumber, &dataByte,\
                                                                `$INSTANCE_NAME`_SPC_BYTE_WRITE_SIZE) == CYRET_STARTED)
            {
                while(CY_SPC_BUSY)
                {
                    /* Wait until SPC becomes idle */
