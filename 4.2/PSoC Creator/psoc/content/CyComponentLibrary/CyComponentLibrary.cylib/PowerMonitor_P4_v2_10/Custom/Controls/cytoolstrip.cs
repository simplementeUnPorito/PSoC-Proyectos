...
namespace PowerMonitor_P4_v2_10
{
    public partial class CyToolStrip : UserControl
    {
        ...
        private const string FILE_FILTER_CSV = "CSV files (*.csv)|*.csv|All files (*.*)|*.*";
        ...
        public void ExportAll()
        {
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                string fileContent = string.Format("{0}", voltageTab);
        ...
        {
            dialog.DefaultExt = FILE_EXTENSION_CSV;
            dialog.Filter = FILE_FILTER_CSV;
        ...
        public void ImportAll()
        {
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    List<StringBuilder> listTables = CyImportExport.GetTables(
                        CyImportExport.GetFileText(openFileDialog.FileName));
