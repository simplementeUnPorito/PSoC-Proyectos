...
uint16 `$INSTANCE_NAME`_sensorBaseline[`$INSTANCE_NAME`_TOTAL_SENSOR_COUNT];
uint8 `$INSTANCE_NAME`_sensorBaselineLow[`$INSTANCE_NAME`_TOTAL_SENSOR_COUNT];
`$SizeReplacementString` `$INSTANCE_NAME`_sensorSignal[`$INSTANCE_NAME`_TOTAL_SENSOR_COUNT];
uint8 `$INSTANCE_NAME`_sensorOnMask[(((`$INSTANCE_NAME`_TOTAL_SENSOR_COUNT - 1u) / 8u) + 1u)];
                                  ...
                                  `=ReentrantKeil($INSTANCE_NAME . "_CheckIsSensorActive")`
{
    ...
    uint8 widget = `$INSTANCE_NAME`_widgetNumber[sensor];
    `$SizeReplacementString` fingerThreshold = `$INSTANCE_NAME`_fingerThreshold[widget];
    `$SizeReplacementString` hysteresis = `$INSTANCE_NAME`_hysteresis[widget];
    uint8 debounce = `$INSTANCE_NAME`_debounce[widget];
    
`$writerCSHLIsSensor`
    
    /* Was on */
    if (0u != (`$INSTANCE_NAME`_sensorOnMask[pos] & onMask))
    {
        /* Hysteresis minus */
        if (`$INSTANCE_NAME`_sensorSignal[sensor] < (fingerThreshold - hysteresis))
        {
            `$INSTANCE_NAME`_sensorOnMask[pos] &= (uint8)~onMask;
    ...
    return (((`$INSTANCE_NAME`_sensorOnMask[pos] & onMask) != 0u) ? 1u : 0u);
}
                                    ...
                                    `=ReentrantKeil($INSTANCE_NAME . "_DisableWidget")`
{
    ...
    do
    {
        ...
        `$INSTANCE_NAME`_sensorEnableMask[pos] &= (uint8)~enMask;
        rawIndex++;
    }
    while(rawIndex < numberOfSensors);
...
`$writerCSHLDisWidget`
    ...
    *******************************************************************************/
	                                          ...
	                                          `=ReentrantKeil($INSTANCE_NAME . "_GetMatrixButtonPos")`
    {
        ...
        if (0u != `$INSTANCE_NAME`_CheckIsWidgetActive(widget))
        {
