...
/* SmartSense functions */
    ...
    extern void `$INSTANCE_NAME`_AutoTune(void) `=ReentrantKeil($INSTANCE_NAME . "_AutoTune")`;
#endif /* End (`$INSTANCE_NAME`_TUNING_METHOD == `$INSTANCE_NAME`_AUTO_TUNING) */

/* Global software variables */
extern volatile uint8 `$INSTANCE_NAME`_csv;            /* CapSense CSD status, control variable */
extern volatile uint8 `$INSTANCE_NAME`_sensorIndex;    /* Index of scannig sensor */

#if (`$INSTANCE_NAME`_CURRENT_SOURCE == `$INSTANCE_NAME`_EXTERNAL_RB)
    extern uint8  `$INSTANCE_NAME`_extRbCh0Cur;
    #if (`$INSTANCE_NAME`_DESIGN_TYPE == `$INSTANCE_NAME`_TWO_CHANNELS_DESIGN)
        extern uint8  `$INSTANCE_NAME`_extRbCh1Cur;
    #endif /* (`$INSTANCE_NAME`_DESIGN_TYPE == `$INSTANCE_NAME`_TWO_CHANNELS_DESIGN)*/ 
#endif /* (`$INSTANCE_NAME`_CURRENT_SOURCE == `$INSTANCE_NAME`_EXTERNAL_RB) */ 
...
#define `$INSTANCE_NAME`_IdacCH0_RESTORE_CFG (1u)
#define `$INSTANCE_NAME`_IdacCH1_RESTORE_CFG (2u)
/* DAC_CR0 register value when IDAC is stopped*/
#define `$INSTANCE_NAME`_IDAC_STOP_CR0_VAL   (0x1Eu)
...
//#define `$INSTANCE_NAME`_AnalogSwitchDivider    `$INSTANCE_NAME`_analogSwitchDivider
