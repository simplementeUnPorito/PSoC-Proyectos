...
*/ 
cy_stc_tcpwm_counter_config_t `=$CY_CONST_CONFIG ? "const " : ""``$INSTANCE_NAME`_config =
{
        .period = `$Period`UL,
        .clockPrescaler = `$ClockPrescaler`UL,
        .runMode = `$RunMode`UL,
        .countDirection = `$CountDirection`UL,
        .compareOrCapture = `$CompareOrCapture`UL,
        ...
        .captureInputMode = `$CaptureInputMasked`UL,
...
void `$INSTANCE_NAME`_Start(void)
{
    ...
    {
        (void)Cy_TCPWM_Counter_Init(`$INSTANCE_NAME`_HW, `$INSTANCE_NAME`_CNT_NUM, &`$INSTANCE_NAME`_config); 
