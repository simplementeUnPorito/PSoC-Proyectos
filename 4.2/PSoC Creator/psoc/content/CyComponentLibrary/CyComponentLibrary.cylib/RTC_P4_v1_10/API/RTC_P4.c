...
void `$INSTANCE_NAME`_Init(void)
{
    ...
    tmpTime = `$INSTANCE_NAME`_ConstructTime(`$INSTANCE_NAME`_INITIAL_TIME_FORMAT,
        ...
        `$INSTANCE_NAME`_SetDSTStartTime(&dstTimeTmp, (`$INSTANCE_NAME`_DST_DATETYPE_ENUM)`$INSTANCE_NAME`_INITIAL_DST_DATE_TYPE);
        ...
        `$INSTANCE_NAME`_SetDSTStopTime(&dstTimeTmp, (`$INSTANCE_NAME`_DST_DATETYPE_ENUM)`$INSTANCE_NAME`_INITIAL_DST_DATE_TYPE);
...
uint32 `$INSTANCE_NAME`_GetDate(void)
{
    ...
    *
    * \param
    *  alarmTime The pointer to the `$INSTANCE_NAME`_date_time global structure where
    ...
    *******************************************************************************/
    void `$INSTANCE_NAME`_SetAlarmDateAndTime(const `$INSTANCE_NAME`_DATE_TIME* alarmTime)
    {
        ...
        `$INSTANCE_NAME`_unixTimeAlarm = `$INSTANCE_NAME`_ConstructUnixAlarmTime(alarmTime, (uint8)`$INSTANCE_NAME`_alarmCfgMask);
    }
    ...
    *
    * \param
    *  alarmTimeDate The pointer to the `$INSTANCE_NAME`_date_time structure to which
    ...
    * \param
    *  `$INSTANCE_NAME`_ALARM_SEC_MASK The second alarm mask allows
    *  matching the alarm second register with the current second register.
    * \param
    *  `$INSTANCE_NAME`_ALARM_MIN_MASK The minute alarm mask allows matching the
    *  alarm minute register with the current minute register.
    * \param
    *  `$INSTANCE_NAME`_ALARM_HOUR_MASK The hour alarm mask allows
    * matching the alarm hour register with the current hour register.
    * \param
    *  `$INSTANCE_NAME`_ALARM_DAYOFWEEK_MASK The day of the week alarm mask 
    * allows matching the alarm day of the week register with the current day of
    * the week register.
    * \param
    * `$INSTANCE_NAME`_ALARM_DAYOFMONTH_MASK The day of the month alarm mask
    * allows matching the alarm day of the month register with the current day 
    * of the month register.
    * \param
    * `$INSTANCE_NAME`_ALARM_MONTH_MASK The month alarm mask allows matching 
    * the alarm month register with the current month register.
    * \param
    * `$INSTANCE_NAME`_ALARM_YEAR_MASK The year alarm mask allows matching the
    ...
    *******************************************************************************/
    void `$INSTANCE_NAME`_SetAlarmMask(uint32 mask)
    {
        ...
        `$INSTANCE_NAME`_unixTimeAlarm = `$INSTANCE_NAME`_ConstructUnixAlarmTime(&`$INSTANCE_NAME`_alarmCfgTimeDate, (uint8)mask);
    }
    ...
    *
    * \return
    *  `$INSTANCE_NAME`_ALARM_SEC_MASK - The second alarm mask allows matching the
    ...
    *
    * \return
    *  `$INSTANCE_NAME`_ALARM_HOUR_MASK - The hour alarm mask allows matching the 
    * alarm hour register with the current hour register.
    *
    * \return
    *  `$INSTANCE_NAME`_ALARM_DAYOFWEEK_MASK - The day of the week alarm mask allows
    ...
    *******************************************************************************/
    ...
    {
        return((0uL != (`$INSTANCE_NAME`_currentTimeDate.status & (1uL << `$INSTANCE_NAME`_STATUS_ALARM_OFFSET))) ? 1uL : 0uL);
    }
    ...
    *******************************************************************************/
    uint64 `$INSTANCE_NAME`_ConstructUnixAlarmTime(const `$INSTANCE_NAME`_DATE_TIME* alarmTime, uint8 alarmCfgMask)
    {
        ...
        tmpAlarmDate = 0u;
        tmpAlarmTime = 0u;
        `$INSTANCE_NAME`_UnixToDateTime(&curDateTime, `$INSTANCE_NAME`_unixTime, (uint32)`$INSTANCE_NAME`_24_HOURS_FORMAT);
        ...
        if(0u != ((~((uint32)alarmCfgMask)) & `$INSTANCE_NAME`_ALARM_DAYOFMONTH_MASK))
        {
            ...
            {
                tmpNextVal = (0u == carryFlag) ? tmpCurVal : `$INSTANCE_NAME`_GetNextDay(`$INSTANCE_NAME`_GetYear(curDateTime.date),
                                                              `$INSTANCE_NAME`_GetMonth(curDateTime.date),
                                                               tmpCurVal,
...
*
* \return
* `$INSTANCE_NAME`_STATUS_DST - Status of Daylight Saving Time. This bit
...
*
* \return
* `$INSTANCE_NAME`_STATUS_LY - Status of Leap Year. This bit goes high when the 
* current year is a leap year.
*
* \return
* `$INSTANCE_NAME`_STATUS_AM_PM - Status of Current Time. This bit is low from 
...
uint32 `$INSTANCE_NAME`_ReadStatus(void)
{
    ...
    *******************************************************************************/
    uint64 `$INSTANCE_NAME`_GetDstUnixTime(const `$INSTANCE_NAME`_DST_TIME* dstTime)
    {
        ...
        {
            tmpDate = `$INSTANCE_NAME`_RelativeToFixed(dstTime->dayOfWeek, dstTime->weekOfMonth, dstTime->month, tmpYear);
        }

        tmpTimeDate.time = `$INSTANCE_NAME`_ConstructTime((uint32)`$INSTANCE_NAME`_24_HOURS_FORMAT, 0u, dstTime->hour, 0u, 0u);
...
{
    static volatile uint32 `$INSTANCE_NAME`_currentTickNumber;
    ...
    if(`$INSTANCE_NAME`_currentTickNumber >= `$INSTANCE_NAME`_tickRefOneSec)
    {
        ...
        `$INSTANCE_NAME`_currentTickNumber = 0u;
        ...
        if((`$INSTANCE_NAME`_unixTime + `$INSTANCE_NAME`_dstTimeOffset) == `$INSTANCE_NAME`_unixTimeAlarm)
        {
            ...
            if (`$INSTANCE_NAME`_alarmCallbackPtr != (void *) 0)
            {
                `$INSTANCE_NAME`_alarmCallbackPtr();
            }
            `$INSTANCE_NAME`_unixTimeAlarm = `$INSTANCE_NAME`_GetNexAlarmTime(`$INSTANCE_NAME`_unixTime + `$INSTANCE_NAME`_dstTimeOffset,
                                                                                                  (uint8)`$INSTANCE_NAME`_alarmCfgMask);
        }
    #endif/* (0u != `$INSTANCE_NAME`_INITIAL_ALARM_STATUS) */
    }
...
uint64 `$INSTANCE_NAME`_DateTimeToUnix(uint32 inputDate, uint32 inputTime)
{
    ...
    tmpMonth = `$INSTANCE_NAME`_GetMonth(inputDate);
    tmpVal = `$INSTANCE_NAME`_daysBeforeMonthTbl[tmpMonth - 1u];

    if((tmpMonth > 2u) && (0u != `$INSTANCE_NAME`_LeapYear(tmpYear)))
    {
        tmpVal++;
    }

    /* Calculates how many seconds are in current month days prior to today. */
    unixTime += tmpVal * `$INSTANCE_NAME`_SECONDS_PER_DAY;
    unixTime += (`$INSTANCE_NAME`_GetDay(inputDate) - 1u) * `$INSTANCE_NAME`_SECONDS_PER_DAY;
    ...
    unixTime += tmpVal * `$INSTANCE_NAME`_SECONDS_PER_HOUR;

    /* Calculates how many seconds have elapsed today up to the current minute. */
    unixTime += `$INSTANCE_NAME`_GetMinutes(inputTime) * `$INSTANCE_NAME`_SECONDS_PER_MINUTE;
...
*                 `$INSTANCE_NAME`_FIRST <br>
*                 `$INSTANCE_NAME`_SECOND <br>
*                 `$INSTANCE_NAME`_THIRD <br>
*                 `$INSTANCE_NAME`_FOURTH <br>
*                 `$INSTANCE_NAME`_LAST <br>
*
...
void `$INSTANCE_NAME`_UnixToDateTime(`$INSTANCE_NAME`_DATE_TIME* dateTime, uint64 unixTime, uint32 timeFormat)
{
    ...
    */
    ...
    tmpDay = (unixTime / `$INSTANCE_NAME`_SECONDS_PER_DAY);
    ...
    tmpHour = unixTime / `$INSTANCE_NAME`_SECONDS_PER_HOUR;
    ...
    tmpMinute = unixTime / `$INSTANCE_NAME`_SECONDS_PER_MINUTE;
    ...
    dateTime->time = `$INSTANCE_NAME`_ConstructTime(timeFormat, tmpAmPmState, tmpHour, tmpMinute, unixTime);
...
*  `$INSTANCE_NAME`_JANUARY
*  `$INSTANCE_NAME`_FEBRUARY
*  `$INSTANCE_NAME`_MARCH
*  `$INSTANCE_NAME`_APRIL
*  `$INSTANCE_NAME`_MAY
*  `$INSTANCE_NAME`_JUNE
*  `$INSTANCE_NAME`_JULY
*  `$INSTANCE_NAME`_AUGUST
*  `$INSTANCE_NAME`_SEPTEMBER
*  `$INSTANCE_NAME`_OCTOBER
*  `$INSTANCE_NAME`_NOVEMBER
*  `$INSTANCE_NAME`_DECEMBER
*
* \param
* year A year value.
*
...
uint32 `$INSTANCE_NAME`_DaysBeforeMonth(uint32 month, uint32 year)
{
    ...
    retVal = `$INSTANCE_NAME`_daysBeforeMonthTbl[month];
