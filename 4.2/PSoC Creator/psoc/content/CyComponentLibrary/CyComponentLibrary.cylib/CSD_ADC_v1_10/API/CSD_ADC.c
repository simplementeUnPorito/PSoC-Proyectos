...
*
* Return:
*  `$INSTANCE_NAME`_START_CONV_OK : the conversion started successfully
*  -or-
...
{   
        ...
        (`$INSTANCE_NAME`_IDAC2_GENERAL << `$INSTANCE_NAME`_IDAC2_MODE_SHIFT))
    {
        ...
        status = `$INSTANCE_NAME`_START_CONV_ERR;
    } else {
        ...
        status = `$INSTANCE_NAME`_START_CONV_OK;
    }
    return status;
}
...
int32 `$INSTANCE_NAME`_Read_mVolts(void)
{
    ...
    {
        voltage = `$INSTANCE_NAME`_START_CONV_ERR;
    } else {
    
        (void) `$INSTANCE_NAME`_IsEndConversion(`$INSTANCE_NAME`_WAIT_FOR_RESULT);
    
        voltage = `$INSTANCE_NAME`_GetResult_mVolts();
    }
    return voltage;
}
...
*
* Return:
*  `$INSTANCE_NAME`_START_CONV_OK : the conversion started successfully and the
