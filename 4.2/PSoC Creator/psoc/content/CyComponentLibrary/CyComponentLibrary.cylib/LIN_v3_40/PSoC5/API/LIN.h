...
l_bool  l_sys_init`$POSTFIX`(void)  `=ReentrantKeil("l_sys_init")`;
...
l_u16 l_ifc_ioctl_`$INSTANCE_NAME`(l_ioctl_op op, void* pv) `=ReentrantKeil("l_ifc_ioctl_" . $INSTANCE_NAME)`;
...
#define `$INSTANCE_NAME`_CONTROL_REG            (* (reg8 *) `$INSTANCE_NAME`_bLIN_CtrlReg__CONTROL_REG)
#define `$INSTANCE_NAME`_CONTROL_PTR            (  (reg8 *) `$INSTANCE_NAME`_bLIN_CtrlReg__CONTROL_REG)
...
#define `$INSTANCE_NAME`_UART_RX_FIFO_REG     (* (reg8 *) `$INSTANCE_NAME`_UART_BUART_sRX_RxShifter_u0__DP_AUX_CTL_REG)
#define `$INSTANCE_NAME`_UART_RX_FIFO_PTR     (  (reg8 *) `$INSTANCE_NAME`_UART_BUART_sRX_RxShifter_u0__DP_AUX_CTL_REG)
