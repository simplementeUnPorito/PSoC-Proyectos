...
void `$INSTANCE_NAME`_Init(void)
{
    ...
    /* Turn off the DU */
    `$INSTANCE_NAME`_DU_SEL = `$INSTANCE_NAME`_REG_ZEROS;
    `$INSTANCE_NAME`_DU_CTL = `$INSTANCE_NAME`_REG_ZEROS;
    `$INSTANCE_NAME`_DATA = `$INSTANCE_NAME`_REG_ZEROS;
}
...
cystatus `$INSTANCE_NAME`_LUT_SelectInputs(uint32 lutNum, uint32 inputNum, uint32 inputSrc)
{
    ...
    if(inputSrc == `$INSTANCE_NAME`_SRC_LUT0)
    {
