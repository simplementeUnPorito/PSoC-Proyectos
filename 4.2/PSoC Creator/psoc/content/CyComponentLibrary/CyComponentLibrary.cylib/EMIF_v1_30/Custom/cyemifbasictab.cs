...
namespace EMIF_v1_30
{
    public partial class CyEMIFBasicTab : UserControl, ICyParamEditingControl
    {
        ...
        private void chbLowPowerVoltage_CheckedChanged(object sender, EventArgs e)
        {
            UpdateCalculator();
        }
        ...
        private void UpdateCalculator()
        {
            ...
            if (busClkFreq > 0)
                ...
                m_lblBusClockFrequencyCalc.Text = CyEMIFRresource.UnknownText;
            ...
            if (busClkFreq > 0)
            {
                CyCustomizer.RWStates(m_params.MemorySpeed, busClkFreq, m_params.m_isPSoC5LP, out writeCPUCycles,
            ...
            m_lblWriteCycleLengthCalc.Text = Math.Round(writeCPUCyclesNs, 1).ToString() + " ns ("
                + writeCPUCycles.ToString() + " " + CyEMIFRresource.CyclesText + ")";
            m_lblReadCycleLengthCalc.Text = Math.Round(readCPUCyclesNs, 1).ToString() + " ns ("
                + readCPUCycles.ToString() + " " + CyEMIFRresource.CyclesText + ")";
