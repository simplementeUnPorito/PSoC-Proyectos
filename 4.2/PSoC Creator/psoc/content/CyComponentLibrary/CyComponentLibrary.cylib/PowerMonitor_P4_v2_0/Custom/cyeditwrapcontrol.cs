...
namespace PowerMonitor_P4_v2_0
{
    public class CyEditingWrapperControl : UserControl, ICyParamEditingControl
    {
        ...
        public IEnumerable<CyCustErr> GetErrors()
        {
            ...
            if (m_errorProvider != null)
            {
                ...
                foreach (Control control in this.Controls)
                {
                    ...
                    {
                        errs.AddRange(GetChildErrs(internalControl));
                    }
                }
            }
        ...
        private IEnumerable<CyCustErr> GetChildErrs(Control ctrl)
        {
            List<CyCustErr> errs = new List<CyCustErr>();
            string errorMessage = string.Empty;
            ...
            if (m_errorProvider != null)
            {
                errorMessage = m_errorProvider.GetError(ctrl);
                if (string.IsNullOrEmpty(errorMessage) == false)
                    errs.Add(new CyCustErr(errorMessage));

                // Check controls inside groupbox
                foreach (Control internalControl in ctrl.Controls)
                {
                    errs.AddRange(GetChildErrs(internalControl));
                }
