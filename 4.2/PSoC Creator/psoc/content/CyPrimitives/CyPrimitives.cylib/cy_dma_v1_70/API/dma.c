...
*
*******************************************************************/
uint8 `$INSTANCE_NAME`_DmaInitialize(uint8 BurstCount, uint8 ReqestPerBurst, uint16 UpperSrcAddress, uint16 UpperDestAddress) `=ReentrantKeil($INSTANCE_NAME ."_DmaInitialize")`
{

    /* Allocate a DMA channel. */
    `$INSTANCE_NAME`_DmaHandle = (uint8)`$INSTANCE_NAME`__DRQ_NUMBER;

    /* Configure the channel. */
    (void)CyDmaChSetConfiguration(`$INSTANCE_NAME`_DmaHandle,
                                  BurstCount,
                                  ReqestPerBurst,
                                  (uint8)`$INSTANCE_NAME`__TERMOUT0_SEL,
                                  (uint8)`$INSTANCE_NAME`__TERMOUT1_SEL,
                                  (uint8)`$INSTANCE_NAME`__TERMIN_SEL);

    /* Set the extended address for the transfers */
    (void)CyDmaChSetExtendedAddress(`$INSTANCE_NAME`_DmaHandle, UpperSrcAddress, UpperDestAddress);

    /* Set the priority for this channel */
    (void)CyDmaChPriority(`$INSTANCE_NAME`_DmaHandle, (uint8)`$INSTANCE_NAME`__PRIORITY);
...
{
    /* Disable the channel */
    (void)CyDmaChDisable(`$INSTANCE_NAME`_DmaHandle);
}
