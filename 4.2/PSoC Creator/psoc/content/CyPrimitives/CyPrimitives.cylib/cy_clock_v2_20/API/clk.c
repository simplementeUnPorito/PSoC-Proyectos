...
void `$INSTANCE_NAME`_Stop(void) `=ReentrantKeil($INSTANCE_NAME ."_Stop")`
{
    ...
    `$INSTANCE_NAME`_CLKEN &= (uint8)(~`$INSTANCE_NAME`_CLKEN_MASK);
	`$INSTANCE_NAME`_CLKSTBY &= (uint8)(~`$INSTANCE_NAME`_CLKSTBY_MASK);
}
...
{
    if ((`$INSTANCE_NAME`_CLKEN & `$INSTANCE_NAME`_CLKEN_MASK) != 0u)
    {
...
*
*******************************************************************************/
void `$INSTANCE_NAME`_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                `=ReentrantKeil($INSTANCE_NAME ."_SetDividerRegister")`
{
    uint8 enabled;
    ...
    if (clkDivider != oldDivider)
    {
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                `$INSTANCE_NAME`_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((`$INSTANCE_NAME`_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    `$INSTANCE_NAME`_MOD_SRC |= modeBitMask & (uint8)`$INSTANCE_NAME`_MODE_MASK;
}
...
{
    `$INSTANCE_NAME`_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(`$INSTANCE_NAME`_MODE_MASK));
}
...
{
    return `$INSTANCE_NAME`_MOD_SRC & (uint8)(`$INSTANCE_NAME`_MODE_MASK);
}
...
void `$INSTANCE_NAME`_SetSourceRegister(uint8 clkSource) `=ReentrantKeil($INSTANCE_NAME ."_SetSourceRegister")`
{
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        `$INSTANCE_NAME`_MOD_SRC =
            (`$INSTANCE_NAME`_MOD_SRC & (uint8)(~`$INSTANCE_NAME`_SRC_SEL_MSK)) | clkSource;
    }
