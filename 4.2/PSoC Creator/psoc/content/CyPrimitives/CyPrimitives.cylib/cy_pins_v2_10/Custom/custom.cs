...
namespace Cypress.Comps.PinsAndPorts.cy_pins_v2_10
{
        ...
        ICySystemBuilderComp_v1
    {
            ...
            ICyTerminalEdit_v1 termEdit)
        {
            ...
            // Don't draw the interrupt if it is buried by the system builder interconnect tab
            bool drawInterrupts = pins.UsesInterrupts && !pins.SysBuilderInterruptInternal; 
        ...
        public CyCustErr CustomizeVerilog(ICyInstQuery_v1 query, ICyTerminalQuery_v1 termQuery, out string codeSnippet)
        {
            ...
            // Add an interrupt if TermMode_Interrupt == 1 and it is using interrupts
            
            if (pins.UsesInterrupts && pins.SysBuilderInterruptInternal)
            {
                CyVerilogWriter vwInt = new CyVerilogWriter(ISR_COMP_NAME, query.InstanceName + ISR_INST_SUFFIX);
                // set the interrupt type to DERIVED
                vwInt.AddGeneric("int_type","2'b10");
                
                string intWireName = string.Format("tmp{0}_0__{1}_net", "INTERRUPT", query.InstanceName);
                vwInt.AddPort("int_signal", intWireName);
                
                codeSnippet = codeSnippet + vwInt.ToString();
            }
            ...
            if (pins.PinCount > index)
            {
                switch (pins[index].OutputSlewRate)
                {
                    ...
                    case CyOutputSlewRate.I2CHS:
                        slewRate = CyPinDataSlewRate_v1.I2CHS;
                        break;
                    default:
