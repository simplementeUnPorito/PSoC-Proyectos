...
namespace Cypress.Components.Primitives.cy_net_constraint_v1_50
{
		...
		ICyParamEditingControl
    {
        ...
        public CyGlobalAnalogLocationControl(
            ...
            bool exclusiveUse)
        {
            Initialize(edit, gaLocs, selectedGALocID);
           
            m_exclusiveUseChkBx.Checked = !IsDefault && exclusiveUse;
            m_exclusiveUseChkBx.Enabled = !IsDefault;
            m_exclusiveUseChkBx.CheckedChanged += new EventHandler(m_exclusiveUseChkBx_CheckedChanged);
        }
            ...
            string selectedGALocID)
        {
            ...
            m_exclusiveUseChkBx.Visible = false;
        }

        private void Initialize(ICyInstEdit_v1 edit, IEnumerable<CyGlobalAnalogLoc> gaLocs, string selectedGALocID)
        {
            InitializeComponent();
        ...
        private void PerformDispose()
        {
            ...
            if (m_exclusiveUseChkBx.Visible)
                m_exclusiveUseChkBx.CheckedChanged -= m_exclusiveUseChkBx_CheckedChanged;
        }
        ...
        {  
            get 
            {
                string locId = SelectedGALocId;
                if (string.IsNullOrEmpty(locId)) 
                    return true;
                    
                if (m_edit.DesignQuery != null) 
                    return locId == m_edit.DesignQuery.GblAnaLocDefaultID;
        ...
        bool ExclusiveUse
        {
            get { return m_exclusiveUseChkBx.Checked; }
            set { m_exclusiveUseChkBx.Checked = value; }
        }

        void m_exclusiveUseChkBx_CheckedChanged(object sender, EventArgs e)
        {
            CyGlobalAnalogLocInfo.SetExclusiveUse(m_edit, ExclusiveUse);
