/*******************************************************************************
* Copyright 20016, Cypress Semiconductor Corporation.  All rights reserved.
...
namespace Cypress.Components.System.SysClk_PDL_v1_0
{
    public class CyCustomizer :
        ...
        ICyClockControlData_v3<ICyInstEdit_v1>,
        ICyInstValidateHook_v1
    {
        ...
        public DialogResult EditParams(ICyInstEdit_v1 edit, ICyTerminalQuery_v1 termQuery, ICyExpressMgr_v1 mgr)
        {
            ...
            ICyClockControl_v3 control = design.CreateClockCustomizerControl(this, edit);
        ...
        void UpdateConfigureClkControl(ICyInstQuery_v1 query, ICyClockControl_v1 ctrl, CyCompDevParam param)
        {
        ...
        {   
            const string PERI_FEATURE_M0S8 = "m0s8peri";
            const string PERI_FEATURE_MX = "mxperi";
            
            int numBitsInClockDivider;
            if (instVal.DeviceQuery.GetFeatureParameter(PERI_FEATURE_M0S8, "PCLK_DIV_24_5_NR") > 0 ||
                instVal.DeviceQuery.GetFeatureParameter(PERI_FEATURE_MX, "DIV_24_5_NR") > 0)
            {
                numBitsInClockDivider = 24;
            }
            else if (instVal.DeviceQuery.GetFeatureParameter(PERI_FEATURE_M0S8, "PCLK_DIV_16_5_NR") > 0 ||
                instVal.DeviceQuery.GetFeatureParameter(PERI_FEATURE_MX, "DIV_16_5_NR") > 0 ||
                instVal.DeviceQuery.GetFeatureParameter(PERI_FEATURE_M0S8, "PCLK_DIV_16_NR") > 0 ||
                instVal.DeviceQuery.GetFeatureParameter(PERI_FEATURE_MX, "DIV_16_NR") > 0)
            {
                numBitsInClockDivider = 16;    
            }
            else if (instVal.DeviceQuery.GetFeatureParameter(PERI_FEATURE_M0S8, "PCLK_DIV_8_NR") > 0 ||
                instVal.DeviceQuery.GetFeatureParameter(PERI_FEATURE_MX, "DIV_8_NR") > 0)
            {
                numBitsInClockDivider = 8;    
            }
            else 
            {
                //Used by PSoC3/5
            ...
            double maxValue = Math.Pow(2, numBitsInClockDivider);
            CyClockParamInfo info = new CyClockParamInfo(instVal);
            if (info.Divider < 1 || maxValue < info.Divider)
            {
                instVal.AddError(CyClockParamInfo.DivisorParamName, new CyCustErr(
                    "Divider out of range. The divider must fit in {0}-bits (1 to {1}).", 
                    numBitsInClockDivider, 
                    maxValue));
            }
        }
