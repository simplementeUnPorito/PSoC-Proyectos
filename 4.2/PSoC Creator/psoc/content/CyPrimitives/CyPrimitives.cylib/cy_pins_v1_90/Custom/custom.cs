...
using Cypress.Comps.PinsAndPorts.cy_pins_v1_90;
...
namespace Cypress.Comps.PinsAndPorts.cy_pins_v1_90
{
        ...
        ICyParamEditHook_v1
    {
        ...
        public DialogResult EditParams(ICyInstEdit_v1 edit, ICyTerminalQuery_v1 termQuery, ICyExpressMgr_v1 mgr)
        {
            ...
            CyClockingControl clockingControl = new CyClockingControl(edit);
            clockingControl.Edit = edit;
                ...
                delegate
                    {
                        ...
                        clockingControl.UpdateFromExprs();
                    };
            ...
            bool isPorSettable = !edit.DeviceQuery.IsPSoC4 &&
                     !(edit.DeviceQuery.IsPSoC5 && edit.DeviceQuery.SiliconRevision == 1 &&
                       edit.DeviceQuery.ArchFamilyMember == PSoC5A);
            if(isPorSettable)
                editor.AddCustomPage(ResetTabName, porControl, UpdateGUIFromExprViewChanging, ResetTabName);

            if (edit.DeviceQuery.IsPSoC4)
            {
            ...
            ICyTerminalEdit_v1 termEdit)
        {
            ...
            byte numPins;
            CyCustErr err = CyParamInfo.GetNumPinsValue(query, out numPins);
            ...
            CyStringArrayParamData showAnnotData;
            err = CyStringArrayParamData.CreateBitVectorData(query, CyBitVectorParam.Formal_ShowAnnotationTerm,
                out showAnnotData);
            if (err.IsNotOk) return err;

            bool showInClk, showInClkEn, showInRst, showOutClk, showOutClkEn, showOutRst;
            err = CyParamInfo.GetShowInClkValue(query, out showInClk);
            if (err.IsNotOk) return err;
            showInClk &= query.DeviceQuery.IsPSoC4;
            err = CyParamInfo.GetShowInClkEnValue(query, out showInClkEn);
            if (err.IsNotOk) return err;
            showInClkEn &= query.DeviceQuery.IsPSoC4;
            err = CyParamInfo.GetShowInRstValue(query, out showInRst);
            if (err.IsNotOk) return err;
            showInRst &= query.DeviceQuery.IsPSoC4;
            err = CyParamInfo.GetShowOutClkValue(query, out showOutClk);
            if (err.IsNotOk) return err;
            showOutClk &= query.DeviceQuery.IsPSoC4;
            err = CyParamInfo.GetShowOutClkEnValue(query, out showOutClkEn);
            if (err.IsNotOk) return err;
            showOutClkEn &= query.DeviceQuery.IsPSoC4;
            err = CyParamInfo.GetShowOutRstValue(query, out showOutRst);
            if (err.IsNotOk) return err;
            showOutRst &= query.DeviceQuery.IsPSoC4;

            if (displayAsBus && isHomogeneous)
            {
                err = drawer.DrawAsBus(true, numPins, sioGroupCnt, usesInterrupts,
                                       pinTypes, sioInfoData, sioRefSelData, displayInputsData, displayOutputsData,
                                       showAnnotData, showInClk, showInClkEn, showInRst, showOutClk,
                                       showOutClkEn, showOutRst);
            }
            else
            {
        ...
        class CyPinsDrawer
        {
            ...
            public CyCustErr DrawAsIndividualTerminals(bool isHomogeneous, byte width, bool usesInterrupts,
                ...
                CyStringArrayParamData showAnnotData, bool showInClk, bool showInClkEn, bool showInRst,
                bool showOutClk, bool showOutClkEn, bool showOutRst)
            {
                CyCustErr err = Clear();
                if (err.IsNotOk) return err;

                bool drawAsSinglePin;
                err = GetDrawAsSinglePin(isHomogeneous, false, width, usesInterrupts, pinTypes, sioInfos, sioRefSels,
                                         showAnnotData, showInClk, showInClkEn, showInRst, showOutClk, showOutClkEn,
                                         showOutRst, out drawAsSinglePin);
                if (err.IsNotOk) return err;
                ...
                for (int index = 0; index < width; index++)
                {
                    ...
                    string showAnnotation = showAnnotData.GetValue(index);
                    ...
                    err = CyPinTypeData.CreatePinTypeData(pinType, sioInfo, sioRefSel, showAnnotation, out pinTypeData);
                    if (err.IsNotOk) return err;
                ...
                {
                    DrawOuterComponentAndIRQ(usesInterrupts, showInClk, showInClkEn, showInRst, showOutClk, showOutClkEn,
                                             showOutRst, y + YOffset);
                }
            ...
            static int GetSIOGroupNumber(CyStringArrayParamData sioInfos, int index)
            {
                int sioCnt = 0;
            ...
            public CyCustErr DrawAsBus(bool isHomogeneous, byte width, byte sioGroupCnt, bool usesInterrupts,
                CyStringArrayParamData pinTypes, CyStringArrayParamData sioInfos, CyStringArrayParamData sioRefSels,
                CyStringArrayParamData displayInputsData, CyStringArrayParamData displayOutputsData, 
                CyStringArrayParamData showAnnotData, bool showInClk, bool showInClkEn, bool showInRst, bool showOutClk,
                bool showOutClkEn, bool showOutRst)
            {
                CyCustErr err = Clear();
                if (err.IsNotOk) return err;

                bool drawAsSinglePin;
                err = GetDrawAsSinglePin(isHomogeneous, true, width, usesInterrupts, pinTypes,
                                         sioInfos, sioRefSels, showAnnotData, showInClk, showInClkEn, showInRst,
                                         showOutClk, showOutClkEn, showOutRst, out drawAsSinglePin);
                if (err.IsNotOk) return err;
                ...
                string showAnnotation = showAnnotData.GetValue(0);
                ...
                {
                    DrawOuterComponentAndIRQ(usesInterrupts, showInClk, showInClkEn, showInRst, showOutClk, showOutClkEn,
                                             showOutRst, pinTypeData.NumTerminalsTall*SingleReservedSize);
                }
                ...
                bool showOutClk, bool showOutClkEn, bool showOutRst, float height)
            {
                ...
                if (advancedClockingSpaceNeeded > 0)
                {
                    ...
                    string[] compClkBoxTags = new string[] { compClkBoxTag, BodyLineTag };
                    m_shapeEdit.CreateRectangle(compClkBoxTags, new PointF(loc.X, loc.Y - clkBoxHeight), compBodyWidth,
                                                clkBoxHeight);
                    m_shapeEdit.SetFillColor(compClkBoxTag, Color.FromArgb(220, 220, 220)); //From YFS159
                    m_shapeEdit.SetOutlineColor(compClkBoxTag, Color.Black);
                ...
                m_shapeEdit.CreateRectangle(compTitleTags,
                    new PointF(loc.X, loc.Y - 2 * BaseUnit - clkBoxHeight),
                    compBodyWidth, 2 * BaseUnit);
                ...
                PointF center =
                    CyRectangleF.GetCenterCenter(new RectangleF(loc.X, loc.Y - 2*BaseUnit - clkBoxHeight,
                                                                compBodyWidth, 2*BaseUnit));
                ...
                m_shapeEdit.CreateAnnotation(TextTags, "Pins", center,
                                             new Font(FontFamily.GenericSansSerif, OtherTextSize, FontStyle.Regular),
                                             titleFmt);
                ...
                if (usesInterrupts)
                {
                    ...
                    m_termEdit.TerminalAdd(CyTermInfo.RootTermName_Interrupt + "_0",
                                           CyCompDevTermDir.OUTPUT, CyCompDevTermType.DIGITAL, termLoc, string.Empty, 90,
                                           false);
                    ...
                    m_shapeEdit.CreateAnnotation(TextTags, "irq",
                                                 ...
                                                 new Font(FontFamily.GenericSansSerif, ParameterTextSize,
                                                          FontStyle.Regular), irqFmt);
                }
            ...
            void SetShapeStyles()
            {
                ...
                m_shapeEdit.SetOutlineColor(AnnotationTag, AnnotationOutlineColor);
                m_shapeEdit.SetOutlineWidth(ThinLineTag, 0.25f);
                m_shapeEdit.SetOutlineWidth(BodyLineTag, 1);

                m_shapeEdit.SetOutlineWidth(AnnotationTag, m_query.Preferences.WireSize);
            ...
            /// <param name="showOutRst"> </param>
            /// <param name="drawAsSinglePin"></param>
            /// <param name="sioInfos"> </param>
            /// <param name="sioRefSels"> </param>
            /// <param name="showAnnotData"> </param>
            /// <param name="showInClk"> </param>
            /// <param name="showInClkEn"> </param>
            /// <param name="showInRst"> </param>
            /// <param name="showOutClk"> </param>
            /// <param name="showOutClkEn"> </param>
            /// <returns></returns>
            static CyCustErr GetDrawAsSinglePin(bool isHomogeneous, bool drawAsBus,
                byte width, bool usesInterrupts, CyStringArrayParamData pinTypes,
                CyStringArrayParamData sioInfos, CyStringArrayParamData sioRefSels,
                CyStringArrayParamData showAnnotData, bool showInClk, bool showInClkEn, bool showInRst,
                bool showOutClk, bool showOutClkEn, bool showOutRst, out bool drawAsSinglePin)
            {
                    ...
                    !showInRst && !showOutClk && !showOutClkEn && !showOutRst)
                {
                    ...
                    string showAnnotation = showAnnotData.GetValue(0);
                    CyPinTypeData pinTypeData;
                    CyCustErr err = CyPinTypeData.CreatePinTypeData(pinType, sioInfo, sioRefSel, showAnnotation, out pinTypeData);
                    if (err.IsNotOk) return err;
                ...
                string displayInputs, string displayOutputs)
            {
                ...
                bool showAssignment = showIndex;
                CyPinAssignmentInfo_v1 info = null;
                if (showAssignment)
                {
                    info = GetAssignment(m_query, indexOfLeftMostBit);
                    showAssignment = (info != null && info.IsLocked);
                }
                ...
                if (showAssignment)
                {
                    DrawAssignmentInPinBody(bodyBounds, info);
                }
                else if (showIndex)
                {
            ...
            private CyCustErr CalculateAndDrawAnnotation(PointF reservedLoc, CyPinTypeData pinTypeData, int indexOfLeftMostBit, int width, out bool annotationClobbersName)
            {
                ...
                CyCustErr err = DrawAnnotation(reservedLoc, indexOfLeftMostBit, width, termRow, termIsOnLeft, pinTypeData.SIOVRefUsed);
                return err;
            }
            ...
            private CyCustErr DrawSIOVRef(PointF reservedLoc, int index, int width)
            {
                ...
                CyCustErr err = m_shapeEdit.CreatePolyline(AnalogTags, start, middle, end);
                if (err.IsNotOk) return err;
            ...
            private CyCustErr DrawDigitalOutput(PointF reservedLoc, int index, int width, string displayOutputs)
            {
                ...
                CyCustErr err = m_shapeEdit.CreateLine(DigitalTags,
                                                       displayOutputs == CyPortConstants.Display_FALSE
            ...
            private CyCustErr DrawOutputEnable(PointF reservedLoc, int index, int width, string displayOutputs)
            {
                ...
                CyCustErr err = m_shapeEdit.CreatePolyline(DigitalTags, start, middle, end);
                if (err.IsNotOk) return err;
            ...
            private CyCustErr DrawDigitalInput(PointF reservedLoc, int index, int width, string displayInputs)
            {
                ...
                err = m_shapeEdit.CreateLine(DigitalTags, start,
                                             new PointF(
                                                 displayInputs == CyPortConstants.Display_FALSE
                                                     ? end.X
                                                     : end.X - m_shapeEdit.UserBaseUnit, end.Y));
                if (err.IsNotOk) return err;
            ...
            private CyCustErr DrawAnnotation(PointF reservedLoc, int index, int width, int termRow, bool termIsOnLeft, bool SIOVrefUsed)
            {
                ...
                else                // terminal comes from bottom of pin, then goes to a side
                {
                    ...
                    err = m_shapeEdit.CreatePolyline(AnnotationTags, start, middle, end);
                    if (err.IsNotOk) return err;
                }
            ...
            void _drawTextInPin(RectangleF reservedBounds, string text, string tag)
            {
                ...
                m_shapeEdit.CreateAnnotation(new string[] { IndexTag, uniqueTag, tag }, text, center,
                    new Font(FontFamily.GenericSansSerif, IndexTextSize, FontStyle.Regular), fmt);
                m_shapeEdit.ClearOutline(uniqueTag);
                m_shapeEdit.SetFillColor(uniqueTag, IndexFillColor);
            ...
            class CyPinTypeData
            {
                ...
                public readonly bool ShowAnnotation = false;
                ...
                public static CyCustErr CreatePinTypeData(string pinType, string sioInfo, string sioRefSel,
                    string showAnnot, out CyPinTypeData data)
                {
                    CyCustErr err;
                    data = new CyPinTypeData(pinType, sioInfo, sioRefSel, showAnnot, out err);
                    return err;
                }

                CyPinTypeData(string pinType, string sioInfo, string sioRefSel, string showAnnot, out CyCustErr err)
                {
                    err = CyCustErr.Ok;

                    if (showAnnot == CyPortConstants.ShowAnnotValue_TRUE)
                    {
                        ShowAnnotation = true;
                    }
...
namespace Cypress.Comps.PinsAndPorts.Common_v1_90
{
    #region Constants
    ...
    public class CyPortConstants
    {
        ...
        public const string InputsSyncModeValue_DOUBLE = "0";
        public const string InputsSyncModeValue_SINGLE = "1";
        public const string DefaultInputsSyncModeValue = InputsSyncModeValue_DOUBLE;

        public const string ClkEnableModeValue_EDGE = "0";
        public const string ClkEnableModeValue_LEVEL = "1";
        public const string DefaultClkEnableModeValue = ClkEnableModeValue_EDGE;
        ...
        public const string OESyncsValue_NOSYNC = "0";
        public const string OESyncsValue_SYNC = "1";
        public const string DefaultOESyncsValue = OESyncsValue_NOSYNC;

        public const string OutputModesValue_DATA = "0";
        public const string OutputModesValue_CLOCK = "1";
        public const string DefaultOutputModesValue = OutputModesValue_DATA;

        public const string OutputClockModesValue_NORMAL = "0";
        public const string OutputClockModesValue_INVERTED = "1";
        public const string DefaultOutputClockModesValue = OutputClockModesValue_NORMAL;

        public const string OutputComboMode_DATA_TRANSPARENT = "00";
        public const string OutputComboMode_DATA_SINGLESYNC = "01";
        public const string OutputComboMode_CLOCK_NORMAL = "10";
        public const string OutputComboMode_CLOCK_INVERTED = "11";
        public const string DefaultOutputModeValue = OutputComboMode_DATA_TRANSPARENT;

        public const string OEResetsValue_DISABLED = "0";
        public const string OEResetsValue_ENABLED = "1";
        public const string DefaultOEResetsValue = OEResetsValue_DISABLED;

        public const string OutputResetsValue_DISABLED = "0";
        public const string OutputResetsValue_ENABLED = "1";
        public const string DefaultOutputResetsValue = OutputResetsValue_DISABLED;

        public const string InvertValue_NORMAL = "0";
        public const string InvertValue_INVERTED = "0";
        public const string DefaultInvertValue = InvertValue_NORMAL;
        ...
        public const string InputSyncModeValue_TRANSPARENT = "00";
        public const string InputSyncModeValue_SINGLESYNC = "01";
        public const string InputSyncModeValue_DOUBLESYNC = "10";
        ...
        public const string ShowAnnotValue_FALSE = "0";
        public const string ShowAnnotValue_TRUE = "1";
        public const string DefaultShowAnnotValue = ShowAnnotValue_FALSE;
        ...
        ICyDesignClient_v2
    {
        ...
        public static CyPinAssignmentInfo_v1 GetAssignment(ICyInstQuery_v1 query, int index)
        {
            if (query.DesignQuery.DesignInfoAvailable)
            {
                List<CyPinAssignmentInfo_v1> assignmentInfo = query.DesignQuery.GetPinAssignmentInfo_v1(index);
        ...
        public static CyCustErr GetParamValue<T>(ICyInstQuery_v1 query, string paramName, out T value)
        {
            ...
            if (param == null)
            {
                return new CyCustErr(string.Format(Resource1.UnknownParam, paramName));
            }

            CyCustErr err = param.TryGetValueAs(out value);
            if (err.IsNotOk)
            {
                value = default(T);
                return err;
            ...
            int index, out bool isSIO)
        {
            ...
            switch (thresholdLevelValue)
            {
                ...
                default:
                    return new CyCustErr(string.Format(Resource1.UnhandledThreshold, thresholdLevelValue));
            }
            ...
            string driveCurrentString, string pinTypesString, int index, out bool isSIO)
        {
            ...
            CyCustErr err = IsSIOBecauseOfHotSwap(hotSwapString, pinTypesString, index, out isSIO);
            if (err.IsNotOk) return err;
            ...
            //Return either way. isSio will have the right value now
            return err.IsNotOk ? err : CyCustErr.Ok;
        }
        ...
        public static string GetPinPoundDefineName(ICyInstQuery_v1 query, int index)
        {
            ...
            CyStringArrayParamData.CreatePinAliasData(query, true, out data);
            if (data == null)
                data = new CyStringArrayParamData.CyAliasParamData(string.Empty);

            string instName;
            CyPortCustomizer.GetParamValue<string>(query, CyParamInfo.Builtin_ParamName_InstName, out instName);
        ...
        public virtual CyExprEvalFunc GetFunction(string exprFuncName)
        {
            switch (exprFuncName)
            {
                ...
                case "CreateCSVFromOutputOnlyBitVector":
                    return CreateCSVFromOutputOnlyBitVector;

                //`=CreateOutputConnVisibleBitVector($PinTypes, $DisplayOutputHWConnections, $NumPins)`
                case "CreateOutputConnVisibleBitVector":
                    return CreateOutputConnVisibleBitVector;

                //`=CreateOutputEnableConnVisibleBitVector($PinTypes, $NumPins)`
                case "CreateOutputEnableConnVisibleBitVector":
                    return CreateOutputEnableConnVisibleBitVector;
        ...
        {
            return new CyCustErr(string.Format(Resource1.CustFunctError, errorParam, functName, errorMsg));
        }
        ...
        object CreateSIOInfoCSV(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            ...
            if (args.Length != 7)
            {
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateSIOInfoCVS, exprFuncName));
            }
            ...
            CyCustErr err = ValidateBitVectorParam(CyParamInfo.Formal_ParamName_SIOGroups, sioGroupsString);
            if (err.IsNotOk)
            ...
            for (int i = 0; i < width; i++)
            {
                ...
                if (isSIO)
                {
                    ...
                    switch (sioGroupValue)
                    {
                        ...
                        default:
                            return new CyCustErr(string.Format(Resource1.UnhandledSIOGroup, sioGroupValue));
                    }
                }
                else
                {
        ...
        object CreateHysteresisCSV(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            ...
            if (args.Length != 4)
            {
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateHysteresisCSV, exprFuncName));
            }
            ...
            //Validate params before they are used. (There is no width validation to do.)
            CyCustErr err = ValidateBitVectorParam(CyParamInfo.Local_ParamName_SIOInfo, sioInfosString);
            if (err.IsNotOk)
            ...
            CyStringArrayParamData.CreateBitVectorData(CyBitVectorParam.Local_SIOInfo, sioInfosString,
                                                       out sioInfos);
            ...
            for (int i = 0; i < width; i++)
            {
                ...
                switch (sioInfoType)
                {
                    ...
                    default:
                        return new CyCustErr(string.Format(Resource1.UnhandledSIOInfo, sioInfoType));
                }
                ...
                if (threshold != null)
                {
                    switch (threshold)
                    {
                        ...
                        default:
                            return new CyCustErr(string.Format(Resource1.UnhandledThreshold, threshold));
                    }
                }
                else
                {
        ...
        object CreateVTripCSV(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            ...
            if (args.Length != 3)
            {
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateVTripCSV, exprFuncName));
            }
            ...
            //Validate params before they are used. (There is no width validation to do.)
            CyCustErr err = ValidateBitVectorParam(CyParamInfo.Formal_ParamName_PinTypes, pinTypesString);
            if (err.IsNotOk)
                return WrapCustFunctionErr(exprFuncName, CyParamInfo.Formal_ParamName_PinTypes, err.Message);
            err = ValidateInputOnlyBitVectorParam(CyParamInfo.Formal_ParamName_ThresholdLevels, thresholdLevelString,
                                                  pinTypesString);
        ...
        object CreateSIOVTripBitVector(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            ...
            if (args.Length != 4)
            {
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateSIOVTripBitVector, exprFuncName));
            }
            List<string> vtripValues = new List<string>();
            ...
            CyStringArrayParamData thresholdData;
            CyStringArrayParamData.CreateInputOnlyBitVectorData(
                CyInputOnlyBitVectorParam.Formal_ThresholdLevels, thresholdLevelString, pinTypesString, out thresholdData);
            ...
            return numSIOGroups == 0
                       ? "\"\""
                       : string.Format("{0}'b{1}", numSIOGroups, string.Join("_", vtripValues.ToArray()));
        }
        ...
        object CreateSIORefSelCSV(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            ...
            if (args.Length != 5)
            {
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateSIORefSelCSV, exprFuncName));
            }
            ...
            //Validate params before they are used. (There is no width validation to do.)
            CyCustErr err = ValidateBitVectorParam(CyParamInfo.Local_ParamName_SIOInfo, sioInfoString);
            if (err.IsNotOk)
                return WrapCustFunctionErr(exprFuncName, CyParamInfo.Local_ParamName_SIOInfo, err.Message);
            err = ValidateBitVectorParam(CyParamInfo.Formal_ParamName_PinTypes, pinTypesString);
            if (err.IsNotOk)
                return WrapCustFunctionErr(exprFuncName, CyParamInfo.Formal_ParamName_PinTypes, err.Message);
            err = ValidateOutputOnlyBitVectorParam(CyParamInfo.Formal_ParamName_OutputDriveLevels,
                                                   outputDriveLevelsString,
                                                   pinTypesString);
            if (err.IsNotOk)
                return WrapCustFunctionErr(exprFuncName, CyParamInfo.Formal_ParamName_OutputDriveLevels, err.Message);
            ...
            err =
                CyStringArrayParamData.CreateOutputOnlyBitVectorData(
                    CyOutputOnlyBitVectorParam.Formal_OutputDriveLevels,
            ...
            err = CyStringArrayParamData.CreateInputOnlyBitVectorData(CyInputOnlyBitVectorParam.Formal_ThresholdLevels,
                                                                      thresholdLevelsString, pinTypesString,
            ...
            for (int i = 0; i < width; i++)
            {
                ...
                switch (sioInfoType)
                {
                    ...
                    case CyPortConstants.SIOInfoValue_SINGLE_SIO:
                        ...
                        refSelValues.Add(vrefUsed
                                             ? CyPortConstants.SIORefSelValue_VOHREF
                                             : CyPortConstants.SIORefSelValue_VCC_IO);
                        break;
        ...
        static CyCustErr _vrefUsed(string outputDriveLevel, string thresholdLevel, out bool vrefUsed)
        {
            vrefUsed = false;
            switch (outputDriveLevel)
            {
                ...
                default:
                    return new CyCustErr(string.Format(Resource1.UnhandledOutputDriveLevel, outputDriveLevel));
            }
            ...
            switch (thresholdLevel)
            {
                ...
                default:
                    return new CyCustErr(string.Format(Resource1.UnhandledThreshold, thresholdLevel));
            }
        ...
        object CreateOutputBufferBitVector(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            ...
            if (args.Length != 4)
                return new CyCustErr(string.Format(
                    Resource1.ParamCntErr_CreateOutputBufferBitVector, exprFuncName));
            ...
            return SIOBitVectorString(outputBufferValues);
        }
        ...
        object CreateInputBufferBitVector(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguements have been passed in.
            if (args.Length != 4)
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateInputBufferBitVector, exprFuncName));
            ...
            CyStringArrayParamData thresholdData;
            err = CyStringArrayParamData.CreateInputOnlyBitVectorData(CyInputOnlyBitVectorParam.Formal_ThresholdLevels,
                                                                      thresholdLevelString, pinTypesString,
                                                                      out thresholdData);
            if (err.IsNotOk) return err;
            ...
            return SIOBitVectorString(inputBufferValues);
        }

        private static string SIOBitVectorString(List<string> inputBufferValues)
        {
            int numSIOGroups = inputBufferValues.Count;
            string inputBufferBitVector = numSIOGroups == 0
                                              ? "\"\""
                                              : string.Format("{0}'b{1}", numSIOGroups,
                                                              string.Join("_", inputBufferValues.ToArray()));
            return inputBufferBitVector;
        }

        object CalculateSIOGroupCount(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguments have been passed in.
            if (args.Length != 2)
            {
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CalculateSIOGroupCount, exprFuncName));
            }
            ...
            for (int i = 0; i < width; i++)
            {
                ...
                switch (sioInfoValue)
                {
                    ...
                    default:
                        return new CyCustErr(string.Format(Resource1.UnhandledSIOInfo, sioInfoValue));
                }
            }
            return sioGroupCnt;
        }
        ...
        object CreatePinModeString(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            ...
            if (args.Length != 2)
            {
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreatePinModeString, exprFuncName));
            }
            ...
            for (int i = 0; i < width; i++)
            {
                ...
                switch (pinType)
                {
                    ...
                    case CyPortConstants.PinTypesValue_DIGOUT:
                        ...
                        break;
                    default:
                        return new CyCustErr(string.Format(Resource1.UnhandledPinType, pinType));
                }
            }
            return "\"" + pinModes + "\"";
        }
        ...
        object CreateDriveModesCSV(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguements have been passed in.
            if (args.Length != 3)
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateDriveModesCSV, exprFuncName));
        ...
        object CreateInputBufferesEnabledCSV(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguements have been passed in.
            if (args.Length != 3)
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateInputBufferesEnabledCSV, exprFuncName));
        ...
        object CreateInitialDriveStatesCSV(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguments have been passed in.
            if (args.Length != 7)
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateInitialDriveStatesCSV, exprFuncName));

            string initialDriveStatesString = typeConverter.GetAsString(args[0]);
            string driveModesString = typeConverter.GetAsString(args[1]);
            string pinTypesString = typeConverter.GetAsString(args[2]);
            byte width = typeConverter.GetAsByte(args[3]);
            string hwConnString = typeConverter.GetAsString(args[4]);
            string outputModesString = typeConverter.GetAsString(args[5]);
            bool isPSoC4 = typeConverter.GetAsBool(args[6]);
            ...
            err = ValidateInitialDriveStatesParam(initialDriveStatesString, driveModesString, pinTypesString,
                                                  hwConnString, outputModesString, isPSoC4);
            if (err.IsNotOk)
                return WrapCustFunctionErr(exprFuncName, CyParamInfo.Formal_ParamName_InitialDriveStates, err.Message);

            CyStringArrayParamData data;
            err = CyStringArrayParamData.CreateInitialDriveStateData(initialDriveStatesString, driveModesString,
                                                                     pinTypesString, hwConnString, outputModesString,
                                                                     isPSoC4, out data);
            if (err.IsNotOk) return err;
        ...
        object CreateCSVFromBitVector(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguements have been passed in.
            if (args.Length != 3)
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateCSVFromBitVector, exprFuncName));
            ...
            //Validate params before they are used. (There is no width validation to do.)
            CyCustErr err = ValidateBitVectorParam(bitVectorParamName, bitVectorString);
            if (err.IsNotOk)
        ...
        object CreateCSVFromInputOnlyBitVector(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguments have been passed in.
            if (args.Length != 4)
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateCSVFromInputOnlyBitVector, exprFuncName));
            ...
            //Validate params before they are used. (There is no width validation to do.)
            CyCustErr err = ValidateInputOnlyBitVectorParam(bitVectorParamName, bitVectorString, pinTypesString);
            if (err.IsNotOk)
                return WrapCustFunctionErr(exprFuncName, bitVectorParamName, err.Message);
        ...
        object CreateCSVFromOutputOnlyBitVector(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguements have been passed in.
            if (args.Length != 4)
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateCSVFromOutputOnlyBitVector, exprFuncName));
            ...
            //Validate params before they are used. (There is no width validation to do.)
            CyCustErr err = ValidateOutputOnlyBitVectorParam(bitVectorParamName, bitVectorString, pinTypesString);
            if (err.IsNotOk)
                return WrapCustFunctionErr(exprFuncName, bitVectorParamName, err.Message);
        ...
        object CreateIOVoltagesCSV(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguements have been passed in.
            if (args.Length != 2)
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateIOVoltagesCSV, exprFuncName));

            string ioVoltagesString = typeConverter.GetAsString(args[0]);
            byte width = typeConverter.GetAsByte(args[1]);

            //Validate params before they are used. (There is no width validation to do.)
            CyCustErr err = ValidateIOVoltagesParam(ioVoltagesString);
            if (err.IsNotOk)
        ...
        object CreatePinAliasesCSV(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguements have been passed in.
            if (args.Length != 2)
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreatePinAliasesCSV, exprFuncName));

            string pinAliasesString = typeConverter.GetAsString(args[0]);
            byte width = typeConverter.GetAsByte(args[1]);

            //Validate params before they are used. (There is no width validation to do.)
            CyCustErr err = ValidatePinAliasParam(pinAliasesString);
            if (err.IsNotOk)
        ...
        object CreateOutputConnVisibleBitVector(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguements have been passed in.
            if (args.Length != 3)
                return new CyCustErr(string.Format(Resource1.ParamCntErr_CreateOutputConnVisibleBitVector, exprFuncName));
            ...
            err = CyStringArrayParamData.CreateBitVectorData(CyBitVectorParam.Formal_DisplayOutputConnections,
                                                             displayOutputHWConnectionsString,
                                                             out displayOutputHWConnectionsData);
            if (err.IsNotOk) return err;
        ...
        object CreateOutputEnableConnVisibleBitVector(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            //Generate an error if the wrong number of arguements have been passed in.
            if (args.Length != 2)
                return
                    new CyCustErr(string.Format(Resource1.ParamCntErr_CreateOutputEnableConnVisibleBitVector,
                                                exprFuncName));

            List<string> outputEnableConnVisible = new List<string>();
            ...
            for (int i = 0; i < width; i++)
            {
                ...
                switch (pinType)
                {
                    ...
                    case CyPortConstants.PinTypesValue_DIGOUT_DIGIN:
                    case CyPortConstants.PinTypesValue_DIGOUT_DIGIN_ANALOG:
                        outputEnableConnVisible.Add(CyPortConstants.Display_FALSE);
                        break;
                    ...
                    case CyPortConstants.PinTypesValue_DIGOUT_OE_ANALOG:
                    case CyPortConstants.PinTypesValue_DIGOUT_OE_DIGIN_ANALOG:
                        outputEnableConnVisible.Add(CyPortConstants.Display_TRUE);
                        break;
            ...
            return string.Format("{0}'b{1}", width, string.Join("_", outputEnableConnVisible.ToArray()));
        }
        ...
        protected CyCustErr ValidateBitVectorParam(string paramName, string value)
        {
            CyStringArrayParamData data;
            CyBitVectorParam paramType;
            CyCustErr err = CyStringArrayParamData.GetBitVectorParamType(paramName, out paramType);
            if (err.IsNotOk) return err;
        ...
        protected CyCustErr ValidateInputOnlyBitVectorParam(string paramName, string value, string pinTypesValue)
        {
            CyStringArrayParamData data;
            CyInputOnlyBitVectorParam paramType;
            CyCustErr err = CyStringArrayParamData.GetInputOnlyBitVectorParamType(paramName, out paramType);
            if (err.IsNotOk) return err;
        ...
        protected CyCustErr ValidateOutputOnlyBitVectorParam(string paramName, string value, string pinTypesValue)
        {
            CyStringArrayParamData data;
            CyOutputOnlyBitVectorParam paramType;
            CyCustErr err = CyStringArrayParamData.GetOutputOnlyBitVectorParamType(paramName, out paramType);
            if (err.IsNotOk) return err;
        ...
        protected CyCustErr ValidateDriveModesParam(string value, string pinTypesValue)
        {
            ...
            CyCustErr err = CyStringArrayParamData.CreateDriveModeData(value, pinTypesValue, out data);
            if (err.IsNotOk) return err;
        ...
        protected CyCustErr ValidateInitialDriveStatesParam(string value, string driveModesValue,
            string pinTypesValue, string hwConnValue, string outputModesValue, bool isPSoC4)
        {
            CyStringArrayParamData data;

            CyCustErr err = CyStringArrayParamData.CreateInitialDriveStateData(value, driveModesValue, pinTypesValue,
                                                                               hwConnValue, outputModesValue, isPSoC4,
                                                                               out data);
            if (err.IsNotOk) return err;
        ...
        protected CyCustErr ValidateInputBuffersEnabledParam(string value, string pinTypesValue)
        {
            ...
            CyCustErr err = CyStringArrayParamData.CreateInputBufferEnabledData(value, pinTypesValue, out data);
            if (err.IsNotOk) return err;
        ...
        protected CyCustErr ValidateIOVoltagesParam(string value)
        {
            ...
            CyCustErr err = CyStringArrayParamData.CreateIOVoltageData(value, out data);
            if (err.IsNotOk) return err;
        ...
        protected CyCustErr ValidatePinAliasParam(string value)
        {
            ...
            CyCustErr err = CyStringArrayParamData.CreatePinAliasData(value, out data);
            if (err.IsNotOk) return err;
        ...
        public CyCustErr CustomizeVerilog(ICyInstQuery_v1 query, ICyTerminalQuery_v1 termQuery, out string codeSnippet)
        {
            ...
            foreach (string paramName in query.GetParamNames())
            {
                ...
                // handle use_annotation specially
                // It isn't a hardware parameter per-se, but is very close. Right now, the actual name of the
                // parameter is not "use_annotation" so we will treat it specially for now.
                else if(param.Name == CyParamInfo.Formal_ParamName_ShowAnnotationTerm)
                {
                    CyStringArrayParamData data;
                    err = CyStringArrayParamData.CreateStringArrayData(query,
                                                                       CyParamInfo.Formal_ParamName_ShowAnnotationTerm,
                                                                       out data);
                    if (err.IsNotOk) return err;
                    string value = data.GetVerilog(numPins);
                    vw.AddGeneric("use_annotation", value);
                }
            }
        ...
        public void ValidateParams(ICyInstValidate_v1 instVal)
        {
            ...
            foreach (string paramName in instVal.GetParamNames())
            {
                ...
                instVal.GetCommittedParam(paramName);
            ...
            if (err.IsOk)
            {
                CheckCannotUseAnalogIfSIO(instVal, numPins);
                ...
                CheckInputThresholdSameIfContiguous(instVal, numPins);
                CheckCannotBeResistivePullAndVrefOut(instVal, numPins);

                if (instVal.IsPSoC4)
                {
                    CheckPORM0S8(instVal);
                    CheckSlewRateSameIfContiguousM0S8(instVal, numPins);
                    if (instVal.IsTSG4)
                    {
                        CheckNotAnalogGen4(instVal, numPins);
                    }
                }
            }
        }

        private static void CheckCannotBeResistivePullAndVrefOut(ICyInstValidate_v1 instVal, byte numPins)
        {
            CyStringArrayParamData outputDriveLevels;
            CyCustErr err = CyStringArrayParamData.CreateStringArrayData(instVal, CyParamInfo.Formal_ParamName_OutputDriveLevels,
                out outputDriveLevels);

            if (err.IsOk)
            {
                CyStringArrayParamData driveModes;
                err = CyStringArrayParamData.CreateStringArrayData(instVal, CyParamInfo.Formal_ParamName_DriveMode,
                out driveModes);
                if (err.IsOk)
                {
                    for (int i = 0; i < numPins; i++)
                    {
                        string dLevel = outputDriveLevels.GetValue(i);
                        string dMode = driveModes.GetValue(i);
                        if (dLevel == CyPortConstants.OutputDriveLevelValue_VREF)
                        {
                            if (dMode == CyPortConstants.DriveModeValue_RES_PULL_DOWN ||
                                dMode == CyPortConstants.DriveModeValue_RES_PULL_UP ||
                                dMode == CyPortConstants.DriveModeValue_RES_PULL_UP_DOWN)
                            {
                                instVal.AddError(CyParamInfo.Formal_ParamName_DriveMode,
                           new CyCustErr(Resource1.Err_P35_ResistivePullAndVrefOut));
        ...
        private static void CheckNotAnalogGen4(ICyInstValidate_v1 instVal, byte numPins)
        {
            CyStringArrayParamData pinTypesData;
            CyCustErr err = CyStringArrayParamData.CreateStringArrayData(instVal, CyParamInfo.Formal_ParamName_PinTypes,
                out pinTypesData);
                ...
                for (int i = 0; i < numPins; i++)
                {
                        ...
                        pinType == CyPortConstants.PinTypesValue_DIGOUT_OE_DIGIN_ANALOG)
                    {
                        instVal.AddError(CyParamInfo.Formal_ParamName_PinTypes,
                           new CyCustErr(String.Format(Resource1.Err_Gen4_Analog, i)));
        ...
        private static void CheckPORM0S8(ICyInstValidate_v1 instVal)
        {
            int porValue;
            GetParamValue(instVal, CyParamInfo.Formal_ParamName_PowerOnReset, out porValue);
            if (porValue != CyParamInfo.PORValue_HiZAnalog && porValue != CyParamInfo.PORValue_Unspecified)
            {
                instVal.AddError(CyParamInfo.Formal_ParamName_PowerOnReset,
                                 new CyCustErr(Resource1.Err_M0S8_Invalid_POR));
            }
        }

        private static void CheckSlewRateSameIfContiguousM0S8(ICyInstValidate_v1 instVal, byte numPins)
        {
            if (numPins > 1)
            {
                bool isContiguous;
                ...
                if (err.IsOk)
                {
                    CyStringArrayParamData pinSlewRates;
                    err = CyStringArrayParamData.CreateStringArrayData(instVal, CyParamInfo.Formal_ParamName_SlewRate,
                            out pinSlewRates);
                    ...
                    out pinTypesData);
                        if (err.IsOk)
                        {
                            // go until we get a pin that is an output
                            int pinNum = 0;
                            while (!CyPortConstants.IsOutput(pinTypesData.GetValue(pinNum)) && !CyPortConstants.IsBidir(pinTypesData.GetValue(pinNum)))
                            {
                                pinNum++;
                                // return if no output pins
                                if (pinNum > numPins) return;
                            }

                            string firstSlewRate = pinSlewRates.GetValue(pinNum);
                            pinNum++;
                            for (; pinNum < numPins; pinNum++)
                            {
                                // first check if it's even an output pin
                                string pinType = pinTypesData.GetValue(pinNum);
                                if (!CyPortConstants.IsOutput(pinType) && !CyPortConstants.IsBidir(pinType))
                                {
                                    continue;
                                }

                                string currSlewRate = pinSlewRates.GetValue(pinNum);
                                if (firstSlewRate != currSlewRate)
                                {
                                    instVal.AddError(CyParamInfo.Formal_ParamName_SlewRate,
                                        new CyCustErr(Resource1.Err_M0S8_SlewRateContiguousMismatch));
        ...
        private static CyCustErr CheckInputThresholdSameIfContiguous(ICyInstValidate_v1 instVal, byte numPins)
        {
            if (numPins > 1)
            {
                bool isContiguous;
                ...
                CyStringArrayParamData pinInputThresholds;
                err = CyStringArrayParamData.CreateStringArrayData(instVal,
                                                                   CyParamInfo.Formal_ParamName_ThresholdLevels,
                                                                   out pinInputThresholds);
                if (err.IsNotOk) return err;

                if (!isContiguous) return CyCustErr.Ok;
                ...
                // go until we get a pin that is an input
                int pinNum = 0;
                while (!CyPortConstants.IsInput(pinTypesData.GetValue(pinNum)) &&
                       !CyPortConstants.IsBidir(pinTypesData.GetValue(pinNum)))
                {
                    pinNum++;
                    // return if no input pins
                    if (pinNum > numPins) return CyCustErr.Ok;
                }

                string firstThreshold = pinInputThresholds.GetValue(pinNum);
                pinNum++;
                for (; pinNum < numPins; pinNum++)
                {
                    string currThreshold = pinInputThresholds.GetValue(pinNum);

                    // check that this pin is an input
                    string pinType = pinTypesData.GetValue(pinNum);
                    if (!CyPortConstants.IsInput(pinType) && !CyPortConstants.IsBidir(pinType) ||
                        firstThreshold == currThreshold)
                    {
                        continue;
                    }

                    if ((firstThreshold == CyPortConstants.ThresholdLevelValue_CMOS ||
                              firstThreshold == CyPortConstants.ThresholdLevelValue_CMOS_LVTTL ||
                              firstThreshold == CyPortConstants.ThresholdLevelValue_LVTTL) && 
                              (currThreshold == CyPortConstants.ThresholdLevelValue_PT4_VDDIO ||
                               currThreshold == CyPortConstants.ThresholdLevelValue_PT4_VDDIO_HYST ||
                               currThreshold == CyPortConstants.ThresholdLevelValue_PT5_VDDIO ||
                               currThreshold == CyPortConstants.ThresholdLevelValue_PT5_VDDIO_HYST ||
                               currThreshold == CyPortConstants.ThresholdLevelValue_PT5_VREF ||
                               currThreshold == CyPortConstants.ThresholdLevelValue_PT5_VREF_HYST ||
                               currThreshold == CyPortConstants.ThresholdLevelValue_VREF ||
                               currThreshold == CyPortConstants.ThresholdLevelValue_VREF_HYST))
                    {
                        instVal.AddError(CyParamInfo.Formal_ParamName_ThresholdLevels,
                                         new CyCustErr(Resource1.Err_ThresholdContiguousMismatch));
                        instVal.AddError(CyParamInfo.Formal_ParamName_LayoutMode,
                                         new CyCustErr(Resource1.Err_ThresholdContiguousMismatch));
                    }
                    else if ((currThreshold == CyPortConstants.ThresholdLevelValue_CMOS ||
                              currThreshold == CyPortConstants.ThresholdLevelValue_CMOS_LVTTL ||
                              currThreshold == CyPortConstants.ThresholdLevelValue_LVTTL) && (
                             firstThreshold == CyPortConstants.ThresholdLevelValue_PT4_VDDIO ||
                             firstThreshold == CyPortConstants.ThresholdLevelValue_PT4_VDDIO_HYST ||
                             firstThreshold == CyPortConstants.ThresholdLevelValue_PT5_VDDIO ||
                             firstThreshold == CyPortConstants.ThresholdLevelValue_PT5_VDDIO_HYST ||
                             firstThreshold == CyPortConstants.ThresholdLevelValue_PT5_VREF ||
                             firstThreshold == CyPortConstants.ThresholdLevelValue_PT5_VREF_HYST ||
                             firstThreshold == CyPortConstants.ThresholdLevelValue_VREF ||
                             firstThreshold == CyPortConstants.ThresholdLevelValue_VREF_HYST))
                    {
                        instVal.AddError(CyParamInfo.Formal_ParamName_ThresholdLevels,
                                         new CyCustErr(Resource1.Err_ThresholdContiguousMismatch));
                        instVal.AddError(CyParamInfo.Formal_ParamName_LayoutMode,
                    ...
                    else if (firstThreshold == CyPortConstants.ThresholdLevelValue_CMOS &&
                             currThreshold == CyPortConstants.ThresholdLevelValue_LVTTL)
                    {
                        instVal.AddError(CyParamInfo.Formal_ParamName_ThresholdLevels,
                                         new CyCustErr(Resource1.Err_ThresholdContiguousMismatch));
        ...
        private static CyCustErr CheckSIOPairSettingsMatch(ICyInstValidate_v1 instVal, byte numPins)
        {
            CyStringArrayParamData sioInfoData;
                ...
                switch (sioInfo)
                {
                    ...
                    case CyPortConstants.SIOInfoValue_FIRST_IN_SIO_PAIR:
                            ...
                            switch (threshold1)
                            {
                                ...
                                case CyPortConstants.ThresholdLevelValue_LVTTL:
                                    if (threshold1 != threshold2)
                                    {
                                        instVal.AddError(CyParamInfo.Formal_ParamName_ThresholdLevels,
                                                         new CyCustErr(
                                                             string.Format(Resource1.Err_SIOPairInputsDontMatch, i)));
                                        instVal.AddError(CyParamInfo.Formal_ParamName_ThresholdLevels,
                                                         new CyCustErr(
                                                             string.Format(Resource1.Err_SIOPairInputsDontMatch, i + 1)));
                                    }
                                    break;
                                        ...
                                        threshold2 != CyPortConstants.ThresholdLevelValue_PT4_VDDIO_HYST)
                                    {
                                        instVal.AddError(CyParamInfo.Formal_ParamName_ThresholdLevels,
                                                         new CyCustErr(string.Format(
                                                             Resource1.Err_SIOPairInputsDontMatch, i)));
                                        instVal.AddError(CyParamInfo.Formal_ParamName_ThresholdLevels,
                                                         new CyCustErr(string.Format(
                                                             Resource1.Err_SIOPairInputsDontMatch, i + 1)));
                                    }
                                    break;
                        ...
                        if (CyPortConstants.IsOutput(pinType1) && CyPortConstants.IsOutput(pinType2))
                        {
                            ...
                            err = CyStringArrayParamData.CreateStringArrayData(instVal,
                                                                               CyParamInfo.
                                                                                   Formal_ParamName_OutputDriveLevels,
                                                                               out outputDriveLevelData);
                            if (err.IsNotOk)
                                return err;
                            ...
                            {
                                instVal.AddError(CyParamInfo.Formal_ParamName_OutputDriveLevels,
                                                 new CyCustErr(string.Format(Resource1.Err_SIOPairOutputsDontMatch, i)));
                                instVal.AddError(CyParamInfo.Formal_ParamName_OutputDriveLevels,
                                                 new CyCustErr(string.Format(Resource1.Err_SIOPairOutputsDontMatch,i + 1)));
                            }
                        }
                        break;

                    default:
                        Debug.Fail(string.Format(Resource1.UnhandledSIOInfo, sioInfo));
                        break;
                }
            }
            return CyCustErr.Ok;
        ...
        private static CyCustErr CheckCannotBeContiguousAndAnalog(ICyInstValidate_v1 instVal, byte numPins)
        {
            if (numPins > 1)
            {
                bool isContiguous;
                ...
                if (err.IsNotOk) return CyCustErr.Ok;
                ...
                for (int i = 0; i < numPins; i++)
                {
                    ...
                    {
                        instVal.AddError(CyParamInfo.Formal_ParamName_PinTypes,
                                         new CyCustErr(string.Format(Resource1.Err_CannotBeContiguousAndAnalog, i)));
        ...
        private static CyCustErr CheckSIOLayoutRestrictions(ICyInstValidate_v1 instVal, byte numPins)
        {
            ...
            for (int i = 0; i < numPins; i++)
            {
                ...
                if (runOfSingleSIOs == 2 && foundPair)
                {
                    instVal.AddError(CyParamInfo.Formal_ParamName_SIOGroups,
                                     new CyCustErr(Resource1.Err_TooManySingleSIONextToSIOPair));
                    instVal.AddError(CyParamInfo.Formal_ParamName_LayoutMode,
                                     new CyCustErr(Resource1.Err_TooManySingleSIONextToSIOPair));
                }
                ...
                {
                    instVal.AddError(CyParamInfo.Formal_ParamName_SIOGroups,
                                     new CyCustErr(Resource1.Err_TooManySingleSIOInARow));
                    instVal.AddError(CyParamInfo.Formal_ParamName_LayoutMode,
                                     new CyCustErr(Resource1.Err_TooManySingleSIOInARow));
                    break;
                }
        ...
        private static CyCustErr CheckCannotEnableInputBufferIfAnalogHiZ(ICyInstValidate_v1 instVal, byte numPins)
        {
            ...
            for (int i = 0; i < numPins; i++)
            {
                if (driveModesData.GetValue(i) == CyPortConstants.DriveModeValue_ANALOG_HI_Z &&
                    ...
                    (CyPortConstants.IsInput(pinTypesData.GetValue(i)) || CyPortConstants.IsBidir(pinTypesData.GetValue(i))))
                {
                    instVal.AddError(CyParamInfo.Formal_ParamName_InputBuffersEnabled,
                        new CyCustErr(string.Format(Resource1.Err_CannotEnableIBufAndAnalogHiZ, i)));
        ...
        private CyCustErr CheckCannotUseAnalogIfSIO(ICyInstValidate_v1 instVal, byte numPins)
        {
            //Cannot use analog if SIO.
            ...
            {
                bool isSIO;
                err = IsSIO(hotSwapString, thresholdLevelString, outputDriveLevelString, driveCurrentsString,
                            pinTypesString, i, out isSIO);
                ...
                if (isSIO && (pinType == CyPortConstants.PinTypesValue_ANALOG ||
                              ...
                              pinType == CyPortConstants.PinTypesValue_DIGOUT_OE_DIGIN_ANALOG))
                {
                    string errMsg = string.Format(Resource1.Err_CannotUseAnalogAndSIO, i);
                    ...
                    err = IsSIOBecauseOfOutputDriveLevels(outputDriveLevelString, pinTypesString, i, out tmpSIO);
                    if (err.IsOk && tmpSIO)
                    {
                        instVal.AddError(CyParamInfo.Formal_ParamName_OutputDriveLevels, new CyCustErr(errMsg));
                    }

                    err = IsSIOBecauseOfThresholdLevels(thresholdLevelString, pinTypesString, i, out tmpSIO);
                    if (err.IsOk && tmpSIO)
                    {
        ...
        static CyCustErr _getSIOValidationDatas(ICyInstValidate_v1 instVal, out string hotSwapString,
        ...
        {
            thresholdLevelString = outputDriveLevelString = driveCurrentsString = pinTypesString = null;
            pinTypesData = thresholdLevelData = null;

            CyCustErr err = GetParamValue(instVal, CyParamInfo.Formal_ParamName_HotSwaps, out hotSwapString);
            if (err.IsNotOk) return err;

            err = GetParamValue(instVal, CyParamInfo.Formal_ParamName_ThresholdLevels, out thresholdLevelString);
            if (err.IsNotOk) return err;
            ...
            err = GetParamValue(instVal, CyParamInfo.Formal_ParamName_OutputDriveLevels, out outputDriveLevelString);
            if (err.IsNotOk) return err;

            err = GetParamValue(instVal, CyParamInfo.Formal_ParamName_DriveCurrents, out driveCurrentsString);
            if (err.IsNotOk) return err;

            err = GetParamValue(instVal, CyParamInfo.Formal_ParamName_PinTypes, out pinTypesString);
            if (err.IsNotOk) return err;
            ...
            out IEnumerable<KeyValuePair<int, int>> contiguousSegments)
        {
            ...
            bool isContiguous;
            CyCustErr err = CyParamInfo.GetIsContiguousValue(instQuery, out isContiguous);
            if (err.IsNotOk) return err;
        ...
        public CyCustErr GetPinType(ICyInstQuery_v1 instQuery, int index, out CyPinDataPinType_v1 pinType)
        {
            ...
            switch (pinTypeString)
            {
                ...
                default:
                    return new CyCustErr(string.Format(Resource1.UnhandledPinType, pinTypeString));
            }
        }
        ...
        public CyCustErr GetSIOInfo(ICyInstQuery_v1 instQuery, int index, out CyPinDataSIOType_v1 sioInfo)
        {
            ...
            switch (sioInfoString)
            {
                ...
                default:
                    return new CyCustErr(string.Format(Resource1.UnhandledSIOInfo, sioInfoString));
            }
        }
        ...
        public CyCustErr GetInputThreshold(ICyInstQuery_v1 instQuery, int index, out CyPinDataInputThreshold_v1 threshold)
        {
            ...
            CyStringArrayParamData data;
            CyCustErr err = CyStringArrayParamData.CreateInputOnlyBitVectorData(instQuery, CyInputOnlyBitVectorParam.Formal_ThresholdLevels, out data);
            if (err.IsNotOk) return err;

            string thresholdString = data.GetValue(index);
            switch (thresholdString)
            {
                ...
                default:
                    return new CyCustErr(string.Format(Resource1.UnhandledInputThreshold, thresholdString));
            }
        }
        ...
        public CyCustErr GetDriveMode(ICyInstQuery_v1 instQuery, int index, out CyPinDataDriveMode_v1 driveMode)
        {
            ...
            switch (driveModeString)
            {
                ...
                default:
                    return new CyCustErr(string.Format(Resource1.UnhandledDriveMode, driveModeString));
            }
        }
        ...
        public CyCustErr GetSlewRate(ICyInstQuery_v1 instQuery, int index, out CyPinDataSlewRate_v1 slewRate)
        {
            ...
            switch (slewRateString)
            {
                ...
                default:
                    return new CyCustErr(string.Format(Resource1.UnhandledSlewRate, slewRateString));
            }
        }
        ...
        public string CustomizeToolTip(ICyInstQuery_v1 query, ICyTerminalQuery_v1 termQuery)
        {
            ...
            byte width;
            CyCustErr err = CyParamInfo.GetNumPinsValue(query, out width);
            if (err.IsOk)
            {
            ...
            for (int i = 0; i < width; i++)
            {
                ...
                CyPinAssignmentInfo_v1 assigment = GetAssignment(query, i);
        ...
        List<CyPinAssignmentInfo_v1> GetAssignmentInfo(ICyInstQuery_v1 query)
        {
            byte width;
            CyCustErr err = CyParamInfo.GetNumPinsValue(query, out width);
            if (err.IsNotOk) return new List<CyPinAssignmentInfo_v1>();

            List<CyPinAssignmentInfo_v1> info = new List<CyPinAssignmentInfo_v1>(width);
            for (int i = 0; i < width; i++)
            {
                CyPinAssignmentInfo_v1 assigment = GetAssignment(query, i);
                info.Insert(i, assigment);
            }
            return info;
        }
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            ...
            string instanceName;
            CyCustErr err = GetParamValue(query, CyParamInfo.Builtin_ParamName_InstName, out instanceName);
            ...
            const string setDriveModeImplParamName = "SetDriveModeImpl_API_GEN";
            ...
            const string setDriveModeImplP4ParamName = "SetDriveModeImplP4_API_GEN";
            ...
            const string poundDefineMappingsParamName = "PoundDefineMappings_API_GEN";
            ...
            foreach (CyAPICustomizer file in apiCustomizers)
            {
                ...
                paramDict[setDriveModeImplP4ParamName] = driveModeImplP4;
            ...
            bool isSpanning;
            CyCustErr spanErr = CyParamInfo.GetIsSpanningValue(query, out isSpanning);

            if (contErr.IsOk && spanErr.IsOk && ((isContiguous && !isSpanning) || width == 1))
            {
                //Return all the APIs.
        ...
        static void CheckSIO(ICyInstEdit_v1 instEdit)
        {
            ...
            bool result = instEdit.SetParamExpr(CyParamInfo.Formal_ParamName_HotSwaps, hotSwapsValue);
        ...
        static void SetupDefaultValueTesting(ICyInstEdit_v1 instEdit)
        {
            ...
            bool result = instEdit.SetParamExpr(CyParamInfo.Formal_ParamName_LayoutMode,
                                                CyPortConstants.LayoutModeValue_CONT_SPANNING);
            Trace.Assert(result, "param was not set");
            instEdit.CommitParamExprs();
            ...
            byte numPins;
            CyCustErr err = CyParamInfo.GetNumPinsValue(instEdit, out numPins);
        ...
        static void CheckInitDriveStates(ICyInstEdit_v1 instEdit)
        {
            ...
            bool result = instEdit.SetParamExpr(CyParamInfo.Formal_ParamName_DriveMode, driveModes);
            Trace.Assert(result, "param was not set");
            instEdit.CommitParamExprs();
            ...
            result = instEdit.SetParamExpr(CyParamInfo.Formal_ParamName_DriveMode, defaultDriveMode);
            Trace.Assert(result, "param was not set");
            instEdit.CommitParamExprs();
        }
        ...
        static void CheckInputRestrictSettings(ICyInstEdit_v1 instEdit)
        {
            ...
            string hotswapSetupValue =
                ...
                CyPortConstants.HotSwapValue_FALSE,
                CyPortConstants.HotSwapValue_FALSE);
            bool result = instEdit.SetParamExpr(CyParamInfo.Formal_ParamName_HotSwaps, hotswapSetupValue);
            Trace.Assert(result, "param was not set");
            instEdit.CommitParamExprs();
        ...
        static void CheckOutputRestrictSettings(ICyInstEdit_v1 instEdit)
        {
            ...
            string driveCurrentsSetupValue =
                ...
                CyPortConstants.DriveCurrentValue_4SOURCE_8SINK,
                CyPortConstants.DriveCurrentValue_4SOURCE_8SINK);
            bool result = instEdit.SetParamExpr(CyParamInfo.Formal_ParamName_DriveCurrents, driveCurrentsSetupValue);
            Trace.Assert(result, "param was not set");
            instEdit.CommitParamExprs();
        ...
        static void Check(ICyInstQuery_v1 instQuery, string paramName, IList<string> expectedVals)
        {
            CyStringArrayParamData values;
            CyCustErr err = CyStringArrayParamData.CreateStringArrayData(instQuery, paramName, out values);
        ...
        static void CheckWarpValuesNonBitVector(ICyInstQuery_v1 instQuery, string warpName, IList<string> expectedValues)
        {
            string rawData;
            CyCustErr err = GetParamValue(instQuery, warpName, out rawData);
        ...
        /// <param name="warpName"></param>
        /// <param name="expectedValues"></param>
        static void CheckWarpValuesBitVector(ICyInstQuery_v1 instQuery, string warpName, IList<string> expectedValues)
        {
            string rawData;
            CyCustErr err = GetParamValue(instQuery, warpName, out rawData);
            Trace.Assert(err.IsOk, "Fail to get value back from warp");
    ...
    class CyTermInfo
    {
        ...
        public static CyCustErr GetExpectedTerminalWidth(ICyInstQuery_v1 query, string rootTermName, out byte numPins)
        {
            ...
            return new CyCustErr(string.Format(Resource1.UnknownTerm, rootTermName));
        }
    ...
    class CyParamInfo
    {
        ...
        public const string Formal_ParamName_InputsSyncMode = "InputSyncModes";
        ...
        public const string Formal_ParamName_ShowAnnotationTerm = "ShowAnnotationTerm";
        public const string Formal_ParamName_ShowInClkTerm = "ShowInClkTerm";
        public const string Formal_ParamName_ShowInClkEnTerm = "ShowInClkEnTerm";
        public const string Formal_ParamName_ShowInRstTerm = "ShowInRstTerm";
        public const string Formal_ParamName_ShowOutClkTerm = "ShowOutClkTerm";
        public const string Formal_ParamName_ShowOutClkEnTerm = "ShowOutClkEnTerm";
        public const string Formal_ParamName_ShowOutRstTerm = "ShowOutRstTerm";
        public const string Formal_ParamName_OutputModes = "OutputModes";
        public const string Formal_ParamName_OutputClockModes = "OutputClockModes";
        public const string Formal_ParamName_OESyncs = "OESyncs";
        public const string Formal_ParamName_OutClkEnableMode = "OutClkEnableMode";
        public const string Formal_ParamName_InClkEnableMode = "InClkEnableMode";
        public const string Formal_ParamName_InvertInClock = "InvertInClock";
        public const string Formal_ParamName_InvertInClockEn = "InvertInClockEn";
        public const string Formal_ParamName_InvertInReset = "InvertInReset";
        public const string Formal_ParamName_InvertOutClock = "InvertOutClock";
        public const string Formal_ParamName_InvertOutClockEn = "InvertOutClockEn";
        public const string Formal_ParamName_InvertOutReset = "InvertOutReset";   
        public const string Formal_ParamName_OEResetEnabled = "OEResetEnabled";
        public const string Formal_ParamName_OutputResetEnabled = "OutputResetEnabled";
        public const string Formal_ParamName_PaPin = "PaPin";
        public const string Formal_ParamName_UsePaInClk = "UsePaInClk";
        public const string Formal_ParamName_UsePaInClkEn = "UsePaInClkEn";
        public const string Formal_ParamName_UsePaInRst = "UsePaInRst";
        public const string Formal_ParamName_UsePaOutClk = "UsePaOutClk";
        public const string Formal_ParamName_UsePaOutClkEn = "UsePaOutClkEn";
        public const string Formal_ParamName_UsePaOutRst = "UsePaOutRst";
        ...
        public const string Local_ParamName_InputsSyncMode = "input_sync_mode";
        public const string Local_ParamName_IOVoltages = "io_voltage";
        public const string Local_ParamName_InvertInClock = "invert_in_clock";
        public const string Local_ParamName_InvertInClockEn = "invert_in_clock_en";
        public const string Local_ParamName_InvertInReset = "invert_in_reset";
        public const string Local_ParamName_InvertOutClock = "invert_out_clock";
        public const string Local_ParamName_InvertOutClockEn = "invert_out_clock_en";
        public const string Local_ParamName_InvertOutReset = "invert_out_reset";
        ...
        public const string Local_ParamName_OutputModes = "output_mode";
        public const string Local_ParamName_OutputClockModes = "output_clock_mode";
        public const string Local_ParamName_OESyncs = "oe_sync";
        public const string Local_ParamName_OEResets = "oe_reset";
        public const string Local_ParamName_OutputResets = "output_reset";

        public static CyCustErr GetPORValue(ICyInstQuery_v1 query, out CyPOR por)
        {
            por = CyPOR.Unspecified;
            int porValue;
            CyCustErr err = CyPortCustomizer.GetParamValue(query, Formal_ParamName_PowerOnReset, out porValue);
            ...
            switch (porValue)
            {
                ...
                case PORValue_Unspecified:
                    ...
                    break;
                default:
                    return new CyCustErr(string.Format("Unhandled Power-On Reset mode '{0}'.", porValue));
            }
            return err;
        }
        ...
        {
            return CyPortCustomizer.GetParamValue(query, Formal_ParamName_DisplayAsBus, out displayAsBus);
        }

        public static CyCustErr GetNumPinsValue(ICyInstQuery_v1 query, out byte numPins)
        {
            return CyPortCustomizer.GetParamValue(query, Formal_ParamName_NumPins, out numPins);
        }
        ...
        {
            return CyPortCustomizer.GetParamValue(query, Local_ParamName_SIOGroupCnt, out count);
        }

        public static CyCustErr GetUsesInterruptsValue(ICyInstQuery_v1 query, out bool usesInterrupts)
        {
            return CyPortCustomizer.GetParamValue(query, Local_ParamName_UsesInterrupts, out usesInterrupts);
        }

        public static CyCustErr GetShowInClkValue(ICyInstQuery_v1 query, out bool showInClk)
        {
            return CyPortCustomizer.GetParamValue(query, Formal_ParamName_ShowInClkTerm, out showInClk);
        }

        public static CyCustErr GetShowInClkEnValue(ICyInstQuery_v1 query, out bool showInClkEn)
        {
            return CyPortCustomizer.GetParamValue(query, Formal_ParamName_ShowInClkEnTerm, out showInClkEn);
        }

        public static CyCustErr GetShowInRstValue(ICyInstQuery_v1 query, out bool showInRst)
        {
            return CyPortCustomizer.GetParamValue(query, Formal_ParamName_ShowInRstTerm, out showInRst);
        }

        public static CyCustErr GetShowOutClkValue(ICyInstQuery_v1 query, out bool showOutClk)
        {
            return CyPortCustomizer.GetParamValue(query, Formal_ParamName_ShowOutClkTerm, out showOutClk);
        }

        public static CyCustErr GetShowOutClkEnValue(ICyInstQuery_v1 query, out bool showOutClkEn)
        {
            return CyPortCustomizer.GetParamValue(query, Formal_ParamName_ShowOutClkEnTerm, out showOutClkEn);
        }

        public static CyCustErr GetShowOutRstValue(ICyInstQuery_v1 query, out bool showOutRst)
        {
            return CyPortCustomizer.GetParamValue(query, Formal_ParamName_ShowOutRstTerm, out showOutRst);
        }
        ...
        public static CyCustErr GetIsContiguousValue(ICyInstQuery_v1 query, out bool isContiguous)
        {
            ...
            switch (mode.ToString())
            {
                ...
                default:
                    return new CyCustErr(string.Format(Resource1.UnhandledLayoutMode, mode));
            }
            return err;
        }
        ...
        public static CyCustErr GetIsSpanningValue(ICyInstValidate_v1 instVal, out bool isSpanning)
        {
            ...
            int mode;
            CyCustErr err = CyPortCustomizer.GetParamValue(instVal, Formal_ParamName_LayoutMode, out mode);
    ...
    enum CyBitVectorParam
    {
        ...
        Local_SIORefSels,
        Formal_SIOGroups,
        Formal_ShowAnnotationTerm,
        Formal_ShowInClkTerm,
        Formal_ShowInClkEnTerm,
        Formal_ShowInRstTerm,
        Formal_ShowOutClkTerm,
        Formal_ShowOutClkEnTerm,
        Formal_ShowOutRstTerm,
    ...
    internal abstract class CyStringArrayParamData
    {
        ...
        readonly bool m_useLastValueAsDefault;
        ...
        /// <summary>
        /// Used to determine how long each entry should be.
        /// </summary>
        ...
        static string GetBitVectorParamDefaultValue(CyBitVectorParam paramType)
        {
            switch (paramType)
            {
                ...
                case CyBitVectorParam.Formal_ShowAnnotationTerm:
                    return CyPortConstants.DefaultShowAnnotValue;

                default:
                    Debug.Fail(string.Format(Resource1.UnhandledBitVectorParamType, paramType));
                    return null;
            }
        }
        ...
        static string GetInputOnlyBitVectorParamDefaultValue(CyInputOnlyBitVectorParam paramType)
        {
            switch (paramType)
            {
                ...
                case CyInputOnlyBitVectorParam.Local_InputsSyncMode:
                case CyInputOnlyBitVectorParam.Formal_InputsSyncMode:
                    return CyPortConstants.DefaultInputsSyncModeValue;
                ...
                default:
                    Debug.Fail(string.Format(Resource1.UnhandledInputOnlyBitVectorParamType, paramType));
                    return null;
            }
        }
        ...
        static string GetOutputOnlyBitVectorParamDefaultValue(CyOutputOnlyBitVectorParam paramType)
        {
            switch (paramType)
            {
                ...
                case CyOutputOnlyBitVectorParam.Local_OESyncs:
                case CyOutputOnlyBitVectorParam.Formal_OESyncs:
                    return CyPortConstants.DefaultOESyncsValue;

                case CyOutputOnlyBitVectorParam.Local_OutputModes:
                case CyOutputOnlyBitVectorParam.Formal_OutputModes:
                    return CyPortConstants.DefaultOutputModesValue;

                case CyOutputOnlyBitVectorParam.Local_OutputClockModes:
                case CyOutputOnlyBitVectorParam.Formal_OutputClockModes:
                    return CyPortConstants.DefaultOutputClockModesValue;

                case CyOutputOnlyBitVectorParam.Local_OEResets:
                case CyOutputOnlyBitVectorParam.Formal_OEResets:
                    return CyPortConstants.DefaultOEResetsValue;

                case CyOutputOnlyBitVectorParam.Local_OutputResets:
                case CyOutputOnlyBitVectorParam.Formal_OutputResets:
                    return CyPortConstants.DefaultOutputResetsValue;

                default:
                    Debug.Fail(string.Format(Resource1.UnhandledOutputBitVectorParamType, paramType));
                    return null;
            }
        }
        ...
        static string GetParamName(CyBitVectorParam paramType)
        {
            switch (paramType)
            {
                ...
                case CyBitVectorParam.Formal_ShowAnnotationTerm:
                    return CyParamInfo.Formal_ParamName_ShowAnnotationTerm;
        ...
        static string GetParamName(CyInputOnlyBitVectorParam paramType)
        {
            switch (paramType)
            {
                ...
                case CyInputOnlyBitVectorParam.Formal_InputsSyncMode:
                    return CyParamInfo.Formal_ParamName_InputsSyncMode;
                ...
                case CyInputOnlyBitVectorParam.Local_InputsSyncMode:
                    return CyParamInfo.Local_ParamName_InputsSyncMode;
        ...
        static string GetParamName(CyOutputOnlyBitVectorParam paramType)
        {
            switch (paramType)
            {
                ...
                case CyOutputOnlyBitVectorParam.Formal_OESyncs:
                    return CyParamInfo.Formal_ParamName_OESyncs;

                case CyOutputOnlyBitVectorParam.Formal_OutputModes:
                    return CyParamInfo.Formal_ParamName_OutputModes;

                case CyOutputOnlyBitVectorParam.Formal_OutputClockModes:
                    return CyParamInfo.Formal_ParamName_OutputClockModes;
                ...
                case CyOutputOnlyBitVectorParam.Local_OESyncs:
                    return CyParamInfo.Local_ParamName_OESyncs;

                case CyOutputOnlyBitVectorParam.Local_OutputModes:
                    return CyParamInfo.Local_ParamName_OutputModes;

                case CyOutputOnlyBitVectorParam.Local_OutputClockModes:
                    return CyParamInfo.Local_ParamName_OutputClockModes;
        ...
        public static CyCustErr GetBitVectorParamType(string paramName, out CyBitVectorParam paramType)
        {
            switch (paramName)
            {
                ...
                case CyParamInfo.Formal_ParamName_ShowAnnotationTerm:
                    paramType = CyBitVectorParam.Formal_ShowAnnotationTerm;
                    return CyCustErr.Ok;

                default:
                    paramType = default(CyBitVectorParam);
                    return new CyCustErr(string.Format(Resource1.UnhandledParamName, paramName));
            }
        }
            ...
            out CyInputOnlyBitVectorParam paramType)
        {
            switch (paramName)
            {
                ...
                case CyParamInfo.Formal_ParamName_InputsSyncMode:
                    paramType = CyInputOnlyBitVectorParam.Formal_InputsSyncMode;
                    return CyCustErr.Ok;
                ...
                case CyParamInfo.Local_ParamName_InputsSyncMode:
                    paramType = CyInputOnlyBitVectorParam.Local_InputsSyncMode;
                    return CyCustErr.Ok;
            ...
            out CyOutputOnlyBitVectorParam paramType)
        {
            switch (paramName)
            {
                ...
                case CyParamInfo.Formal_ParamName_OESyncs:
                    paramType = CyOutputOnlyBitVectorParam.Formal_OESyncs;
                    return CyCustErr.Ok;

                case CyParamInfo.Formal_ParamName_OutputModes:
                    paramType = CyOutputOnlyBitVectorParam.Formal_OutputModes;
                    return CyCustErr.Ok;

                case CyParamInfo.Formal_ParamName_OutputClockModes:
                    paramType = CyOutputOnlyBitVectorParam.Formal_OutputClockModes;
                    return CyCustErr.Ok;
                ...
                case CyParamInfo.Local_ParamName_OESyncs:
                    paramType = CyOutputOnlyBitVectorParam.Local_OESyncs;
                    return CyCustErr.Ok;

                case CyParamInfo.Local_ParamName_OutputModes:
                    paramType = CyOutputOnlyBitVectorParam.Local_OutputModes;
                    return CyCustErr.Ok;

                case CyParamInfo.Local_ParamName_OutputClockModes:
                    paramType = CyOutputOnlyBitVectorParam.Local_OutputClockModes;
                    return CyCustErr.Ok;
        ...
        {
            return IsFormalStringArrayParam(paramName) || IsLocalStringArrayParam(paramName);
        }

        public static bool IsLocalStringArrayParam(string paramName)
        {
            return IsLocalBitVectorParam(paramName) ||
                   IsLocalInputOnlyBitVectorParam(paramName) ||
                   IsLocalOutputOnlyBitVectorParam(paramName) ||
                   IsLocalPinAliasParam(paramName) ||
                   IsLocalIOVoltageParam(paramName) ||
                   IsLocalInitialDriveStateParam(paramName) ||
                   IsLocalDriveModesParam(paramName) ||
                   IsLocalInputBuffersEnabledParam(paramName);
        }

        public static bool IsFormalStringArrayParam(string paramName)
        {
            return IsFormalBitVectorParam(paramName) ||
                   IsFormalInputOnlyBitVectorParam(paramName) ||
                   IsFormalOutputOnlyBitVectorParam(paramName) ||
                   IsFormalPinAliasParam(paramName) ||
                   IsFormalIOVoltageParam(paramName) ||
                   IsFormalInitialDriveStateParam(paramName) ||
                   IsFormalDriveModesParam(paramName) ||
                   IsFormalInputBuffersEnabledParam(paramName);
        }
            ...
            out CyStringArrayParamData data)
        {
            ...
            return errs.Count == 0 ? CyCustErr.Ok : CombineErrs(errs);
        }
            ...
            out CyStringArrayParamData data)
        {
            List<CyCustErr> errs = new List<CyCustErr>();
            string value;
            CyCustErr err = CyPortCustomizer.GetParamValue(query, GetParamName(paramType), out value);
            if (err.IsNotOk)
                errs.Add(err);
            ...
            CyInputOnlyBitVectorParam paramType, out CyStringArrayParamData data)
        {
            ...
            CyCustErr err = CyPortCustomizer.GetParamValue(query, paramName, out value);
            if (err.IsNotOk)
                errs.Add(err);
            
            string pinTypesValue;
            err = CyPortCustomizer.GetParamValue(query, CyParamInfo.Formal_ParamName_PinTypes, out pinTypesValue);
            if (err.IsNotOk)
            ...
            string pinTypesValue, out CyStringArrayParamData data)
        {
            ...
            CyStringArrayParamData pinTypesData;
            CyCustErr err = CreateBitVectorData(CyBitVectorParam.Formal_PinTypes, pinTypesValue,
                                                                       out pinTypesData);
            if (err.IsNotOk)
                errs.Add(new CyCustErr(string.Format(
                    "Cannot determine {0} because {1} is in an error state.", GetParamName(paramType),
            ...
            out CyStringArrayParamData data)
        {
            ...
            CyCustErr err = CyPortCustomizer.GetParamValue(query, name, out value);
            if (err.IsNotOk)
                errs.Add(err);
            ...
            out CyStringArrayParamData data)
        {
            ...
            CyCustErr err = CyPortCustomizer.GetParamValue(query, name, out initDrStateValue);
            if (err.IsNotOk)
                errs.Add(err);
            
            string driveModeValue;
            err = CyPortCustomizer.GetParamValue(query, CyParamInfo.Formal_ParamName_DriveMode, out driveModeValue);
            if (err.IsNotOk)
                errs.Add(new CyCustErr(string.Format(
            ...
            string hwConnValue;
            err = CyPortCustomizer.GetParamValue(query, CyParamInfo.Formal_ParamName_DisplayOutputHWConnections, out hwConnValue);
            if (err.IsNotOk)
                errs.Add(new CyCustErr(string.Format(
                    "Cannot determine {0} because {1} is in an error state.",
                    CyParamInfo.Formal_ParamName_InitialDriveStates,
                    CyParamInfo.Formal_ParamName_DisplayOutputHWConnections)));

            string outputModesValue;
            err = CyPortCustomizer.GetParamValue(query, CyParamInfo.Formal_ParamName_OutputModes, out outputModesValue);
            if (err.IsNotOk)
                errs.Add(new CyCustErr(string.Format(
                    "Cannot determine {0} because {1} is in an error state.",
                    CyParamInfo.Formal_ParamName_InitialDriveStates,
                    CyParamInfo.Formal_ParamName_OutputModes)));

            err = CreateInitialDriveStateData(initDrStateValue, driveModeValue, pinTypesValue, hwConnValue,
                                              outputModesValue, (query.IsPSoC4 && !query.IsTSG4), out data);
            if (err.IsNotOk)
                errs.Add(err);
            ...
            out CyStringArrayParamData data)
        {
            ...
            string hwConnValue;
            err = CyPortCustomizer.GetParamValue(query, CyParamInfo.Formal_ParamName_DisplayOutputHWConnections,
                                                 out hwConnValue);
            if (err.IsNotOk)
                errs.Add(new CyCustErr(string.Format(
                    "Cannot determine {0} because {1} is in an error state.",
            ...
            string outputModesValue;
            err = CyPortCustomizer.GetParamValue(query, CyParamInfo.Formal_ParamName_OutputModes,
                                                 out outputModesValue);
            if (err.IsNotOk)
                errs.Add(new CyCustErr(string.Format(
                    "Cannot determine {0} because {1} is in an error state.",
                    CyParamInfo.Formal_ParamName_InitialDriveStates,
            ...
            err = CreateInitialDriveStateData(initDrStateValue, driveModeValue, pinTypesValue, hwConnValue,
                                              outputModesValue, (query.DeviceQuery.IsPSoC4 && !query.DeviceQuery.IsTSG4),
                                              out data);
            if (err.IsNotOk)
                errs.Add(err);
        ...
        public static CyCustErr CreateInitialDriveStateData(string initialDriveStateValue, string driveModeValue,
            string pinTypesValue, string hwConnValue, string outputModesValue, bool isPSoC4, out CyStringArrayParamData data)
        {
            CyStringArrayParamData driveModeData;
            CreateDriveModeData(driveModeValue, pinTypesValue, out driveModeData);
            CyStringArrayParamData pinTypesData;
            CreateBitVectorData(CyBitVectorParam.Formal_PinTypes, pinTypesValue, out pinTypesData);
            CyStringArrayParamData hwConnData;
            CreateBitVectorData(CyBitVectorParam.Formal_DisplayOutputConnections, hwConnValue,
                                                out hwConnData);
            CyStringArrayParamData outputModesData;
            CyCustErr err = CreateOutputOnlyBitVectorData(CyOutputOnlyBitVectorParam.Formal_OutputModes,
                                                          outputModesValue, pinTypesValue, out outputModesData);
            data = new CyInitialDriveStateParamData(initialDriveStateValue, driveModeData, pinTypesData, hwConnData,
                                                    outputModesData, isPSoC4);
            return err;
        }
            ...
            out CyStringArrayParamData data)
        {
            ...
            CyCustErr err = CyPortCustomizer.GetParamValue(query, name, out driveModeValue);
            if (err.IsNotOk)
                errs.Add(err);
        ...
        public static CyCustErr CreateDriveModeData(ICyInstQuery_v1 query, bool formal, out CyStringArrayParamData data)
        {
            ...
            string pinTypesValue;
            err = CyPortCustomizer.GetParamValue(query, CyParamInfo.Formal_ParamName_PinTypes,
                out pinTypesValue);
            if (err.IsNotOk)
                errs.Add(new CyCustErr(string.Format(
                    "Cannot determine {0} because {1} is in an error state.",
            ...
            out CyStringArrayParamData data)
        {
            ...
            CyCustErr err = CyPortCustomizer.GetParamValue(query, name, out inputBufferEnabledValue);
            if (err.IsNotOk)
                errs.Add(err);
            ...
            public override CyCustErr ValidateAt(int index)
            {
                ...
                if (value.Length != ExampleValue.Length)
                {
                    return new CyCustErr(
                        string.Format(Resource1.Err_BitVectorValueIncorrectLength,
                        index,
                        ExampleValue.Length,
                        value,
                        value.Length));
                ...
                for (int i = 0; i < value.Length; i++)
                {
                    ...
                    {
                        return new CyCustErr(string.Format(
                        Resource1.Err_BitVectorValueOnlyOnesAndZeros,
                        index,
                        value[i]));
                    }
                }
                ...
                {
                    return new CyCustErr(string.Format(Resource1.Err_BitVectorValueInvalidOption,
                        index, value));
                }
        ...
        {
            private bool m_isPSoC4;
            private CyStringArrayParamData m_driveModeData;
            private CyStringArrayParamData m_pinTypesData;
            private CyStringArrayParamData m_hwConnData;
            private CyStringArrayParamData m_outputModeData;

            public CyInitialDriveStateParamData(string data, CyStringArrayParamData driveModeData,
                                                CyStringArrayParamData pinTypesData, CyStringArrayParamData hwConnData,
                                                CyStringArrayParamData outputModeData, bool isPSoC4)
                : base(data, CyPortConstants.InitialDriveStateValue_LOW)
            {
                m_isPSoC4 = isPSoC4;
                m_driveModeData = driveModeData;
                m_pinTypesData = pinTypesData;
                m_hwConnData = hwConnData;
                m_outputModeData = outputModeData;
            }

            protected override string GetDefaultValue(int index)
            {
                // On PSoC4, digital output pin types without a hardware OE use the DR register to drive the OE
                string pinType = m_pinTypesData.GetValue(index);
                bool isDigOutOrBidir = CyPortConstants.IsOutput(pinType) || CyPortConstants.IsBidir(pinType);
                bool usesOE = CyPortConstants.IsOE(pinType);
                bool isAnalog = CyPortConstants.IsAnalog(pinType);
                bool showHWConnection = m_hwConnData.GetValue(index) == CyPortConstants.Display_TRUE;
                bool isOutputClockMode = m_outputModeData.GetValue(index) == CyPortConstants.OutputModesValue_CLOCK;

                if (m_isPSoC4 && ((isDigOutOrBidir && !usesOE && showHWConnection) || isOutputClockMode || isAnalog))
                {
                ...
                switch (driveMode)
                {
                    ...
                    default:
                        Debug.Fail(string.Format(Resource1.UnhandledDriveMode, driveMode));
                        return string.Format(Resource1.UnhandledDriveMode, driveMode);
                }
            }
        }
        ...
        class CyDriveModeParamData : CyBitVectorParamData
        {
            ...
            protected override string GetDefaultValue(int index)
            {
                ...
                switch (pinType)
                {
                    ...
                    default:
                        return string.Format(Resource1.UnhandledPinType, pinType);
                }
            }
        }
        ...
        class CyInputBuffersEnabledParamData : CyBitVectorParamData
        {
            ...
            public static string GetDefaultValue(CyStringArrayParamData pinTypes, int index)
            {
                ...
                switch (pinType)
                {
                    ...
                    default:
                        Debug.Fail(string.Format(Resource1.UnhandledPinType, pinType));
                        return string.Format(Resource1.UnhandledPinType, pinType);
                }
            }
        ...
        internal class CyAliasParamData : CyStringArrayParamData
        {
            ...
            public override CyCustErr ValidateAt(int index)
            {
                ...
                if (string.IsNullOrEmpty(value) == false)
                {
                    ...
                    {
                        return new CyCustErr(string.Format(Resource1.Err_NotValidCIdentifier,
                            index, value, msg));
                    }
                }
                return CyCustErr.Ok;
            ...
            public override CyCustErr Validate()
            {
                ...
                for (int i = 0; i < Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(val) == false)
                    {
                        ...
                        if (num > 1)
                        {
                            return new CyCustErr(string.Format(Resource1.Err_AliasRepeated, val, num));
                        }
                    }
                }
        ...
        public class CyIOVoltagesParamData : CyStringArrayParamData
        {
            ...
            public override CyCustErr ValidateAt(int index)
            {
                    ...
                    CyPortConstants.DefaultIOVolatageValue.Equals(value, StringComparison.OrdinalIgnoreCase) == false)
                {
                    ...
                    if (result == false)
                    {
                        return new CyCustErr(string.Format(Resource1.Err_InvalidNumFormat,
                            index, value));
                    }
                    ...
                    if (value.Contains("."))
                    {
                        ...
                        {
                            return new CyCustErr(string.Format(Resource1.Err_InvalidIOVoltagePercision,
                                index,
                                value));
                        }
                    }
