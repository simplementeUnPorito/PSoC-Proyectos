...
namespace Cypress.Comps.PinsAndPorts.cy_pins_v1_90
{
    ...
    public partial class CyPinsControl : UserControl, ICyParamEditingControl
    {
        ...
        public CyPinsControl(ICyInstEdit_v1 instEdit)
        {
            ...
            m_perPinDataControl = new CyPerPinDataControl(this, m_edit);
        ...
        static bool IsHandledPerPinParam(CyCompDevParam param)
        {
        ...
        static int _getPinIndex(CyTreeNode node)
        {
            if (node != null)
            {
        ...
        private void UpdatePerPinDataDisplayed()
        {
            ...
            string value;
            CyCustErr err = GetPerPinParamValuesForSelectedNodes(CyParamInfo.Formal_ParamName_DriveMode, out value);
            ...
            err = GetPerPinParamValuesForSelectedNodes(CyParamInfo.Formal_ParamName_InputsSyncMode, out value);
            m_perPinDataControl.InputSyncMode = value;
            m_perPinDataControl.InputSyncModeErrorText = (err.IsOk) ?
                String.Empty : CyStringArrayParamData.ParseOutRelatedMsgs(err, selectedPinIndexes);
            ...
            // OutputMode must come first. Sync/Clock-mode depend on Mode being set
            err = GetPerPinParamValuesForSelectedNodes(CyParamInfo.Formal_ParamName_OutputModes, out value);
            m_perPinDataControl.OutputMode = value;
            m_perPinDataControl.OutputComboModeErrorText = (err.IsOk) ?
                String.Empty : CyStringArrayParamData.ParseOutRelatedMsgs(err, selectedPinIndexes);
            ...
            err = GetPerPinParamValuesForSelectedNodes(CyParamInfo.Formal_ParamName_OutputClockModes, out value);
            m_perPinDataControl.OutputClockMode = value;
            m_perPinDataControl.OutputComboModeErrorText = (err.IsOk) ?
                String.Empty : CyStringArrayParamData.ParseOutRelatedMsgs(err, selectedPinIndexes);

            err = GetPerPinParamValuesForSelectedNodes(CyParamInfo.Formal_ParamName_OESyncs, out value);
            m_perPinDataControl.OESynchronized = value;
            m_perPinDataControl.OESynchronizedErrorText = (err.IsOk) ?
                String.Empty : CyStringArrayParamData.ParseOutRelatedMsgs(err, selectedPinIndexes);
            ...
            err = GetPerPinParamValuesForSelectedNodes(CyParamInfo.Formal_ParamName_ShowAnnotationTerm, out value);
            m_perPinDataControl.ShowAnnotationTerm = value;
            m_perPinDataControl.ShowAnnotationTermErrorText = (err.IsOk) ?
                String.Empty : CyStringArrayParamData.ParseOutRelatedMsgs(err, selectedPinIndexes);
        ...
        private bool _outputUsed(IEnumerable<int> selectedPinIndexes)
        {
            CyStringArrayParamData data;
        ...
        private bool _inputUsed(IEnumerable<int> selectedPinIndexes)
        {
            CyStringArrayParamData data;
            CyStringArrayParamData.CreateStringArrayData(m_edit, CyParamInfo.Formal_ParamName_PinTypes, out data);
        ...
        public CyCustErr GetPerPinParamValuesForSelectedNodes(string paramName, out string value)
        {
            ...
            {
                string dataString;
                CyCustErr err = CyPortCustomizer.GetParamValue(m_edit, paramName, out dataString);
                ...
                if (dataString != null) //as long as it is not a syntax error
                {
                    ...
                    foreach (int selectedPinIndex in selectedPinIndexes)
                    {
                        ...
                        {
                            string pinValue = data.GetValue(selectedPinIndex);
            ...
            out CheckState outputState, out CheckState oeState, out CheckState bidirState)
        {
            ...
            string dataString;
            CyCustErr err = CyPortCustomizer.GetParamValue(m_edit, CyParamInfo.Formal_ParamName_PinTypes,
                out dataString);
        ...
        static CheckState _updateState(bool value, bool isInitialized, CheckState currentState)
        {
            CheckState newState = (value) ? CheckState.Checked : CheckState.Unchecked;
            if (isInitialized == false) return newState;
            return newState != currentState ? CheckState.Indeterminate : currentState;
        }

        private static string CreateNewPinType(string oldPinType, CheckState analogState, CheckState inputState,
                ...
                foreach (int selectedPinIndex in selectedPinIndexes)
                {
                    ...
                    foreach (string paramName in m_edit.GetParamNames())
                    {
                        ...
                        if (IsHandledPerPinParam(param))
                        {
                            ...
                            string dataString;
                            err = CyPortCustomizer.GetParamValue(m_edit, paramName, out dataString);
                        ...
                        // update the port-adapter pin if necessary
                        if (paramName == CyParamInfo.Formal_ParamName_PaPin)
                        {
                            int paPin;
                            CyPortCustomizer.GetParamValue(m_edit, CyParamInfo.Formal_ParamName_PaPin, out paPin);
                            if (selectedPinIndex < paPin)
                            {
                                paPin--;
                            }
                            else if (selectedPinIndex == paPin)
                            {
                                paPin = -1;
                            }
        ...
        static int GetMin(IEnumerable<int> nums)
        {
            int min = -1;
    ...
    public class CyPerPinDataControl : UserControl
    {
        ...
        public CyPerPinDataControl(CyPinsControl pinControl, ICyInstEdit_v1 instEdit)
        {
            this.Dock = DockStyle.Fill;
            ...
            m_typeTab.Text = "Type";
            m_typeControl = new CyTypeControl(instEdit.DeviceQuery.IsTSG4);
            ...
            m_generalControl = new CyGeneralControl(instEdit.DeviceQuery.IsTSG4);
            ...
            m_inputControl = new CyInputControl(instEdit);
            ...
            m_outputControl = new CyOutputControl(instEdit);
        ...
        public string InputSynchronized
        {
            get
            {
                return m_inputControl.InputSyncModeCombo == CyPortConstants.InputSyncModeValue_TRANSPARENT
                           ? CyPortConstants.InputsSynchronizedValue_DISABLED
                           : CyPortConstants.InputsSynchronizedValue_ENABLED;
            }
            set
            {
                if (value == CyPortConstants.InputsSynchronizedValue_ENABLED)
                {
                    m_inputControl.InputSyncModeCombo = InputSyncMode == CyPortConstants.InputsSyncModeValue_SINGLE
                                                            ? CyPortConstants.InputSyncModeValue_SINGLESYNC
                                                            : CyPortConstants.InputSyncModeValue_DOUBLESYNC;
                }
                else
                {
                    m_inputControl.InputSyncModeCombo = CyPortConstants.InputSyncModeValue_TRANSPARENT;
                }
            }
        }

        public string InputSyncMode
        {
            get
            {
                return m_inputControl.InputSyncModeCombo == CyPortConstants.InputSyncModeValue_SINGLESYNC
                           ? CyPortConstants.InputsSyncModeValue_SINGLE
                           : CyPortConstants.InputsSyncModeValue_DOUBLE;
            }
            set
            {
                if (value == CyPortConstants.InputsSyncModeValue_SINGLE)
                {
                    m_inputControl.InputSyncModeCombo = InputSynchronized ==
                                                        CyPortConstants.InputsSynchronizedValue_ENABLED
                                                            ? CyPortConstants.InputSyncModeValue_SINGLESYNC
                                                            : CyPortConstants.InputSyncModeValue_TRANSPARENT;
                }
                else
                {
                    m_inputControl.InputSyncModeCombo = InputSynchronized ==
        ...
        {
            get { return m_inputControl.InputSyncModeErrorText; }
            set
            {
                m_inputControl.InputSyncModeErrorText = value;
                UpdateInputTabErrorIcon();
            }
        }
        ...
        public string OutputSynchronized
        {
            get
            {
                return m_outputControl.OutputComboMode == CyPortConstants.OutputComboMode_DATA_TRANSPARENT
                           ? CyPortConstants.OutputsSynchronizedValue_DISABLED
                           : CyPortConstants.OutputsSynchronizedValue_ENABLED;
            }
            set
            {
                if (OutputMode == CyPortConstants.OutputModesValue_DATA)
                {
                    m_outputControl.OutputComboMode = value == CyPortConstants.OutputsSynchronizedValue_ENABLED
                                                          ? CyPortConstants.OutputComboMode_DATA_SINGLESYNC
                                                          : CyPortConstants.OutputComboMode_DATA_TRANSPARENT;
                }
                else
                {
                    m_outputControl.OutputComboMode = OutputClockMode == CyPortConstants.OutputClockModesValue_NORMAL
                                                         ? CyPortConstants.OutputComboMode_CLOCK_NORMAL
                                                         : CyPortConstants.OutputComboMode_CLOCK_INVERTED;
                }
            }
        }
        ...
        public string OutputMode
        {
            ...
            {
                return m_outputControl.OutputComboMode == CyPortConstants.OutputComboMode_DATA_TRANSPARENT ||
                       m_outputControl.OutputComboMode == CyPortConstants.OutputComboMode_DATA_SINGLESYNC
                           ? CyPortConstants.OutputModesValue_DATA
                           : CyPortConstants.OutputModesValue_CLOCK;
            }
            set
            {
                if (value == CyPortConstants.OutputModesValue_DATA)
                {
                    m_outputControl.OutputComboMode = OutputSynchronized == CyPortConstants.OutputsSynchronizedValue_DISABLED
                                                     ? CyPortConstants.OutputComboMode_DATA_TRANSPARENT
                                                     : CyPortConstants.OutputComboMode_DATA_SINGLESYNC;
                }
                else
        ...
        public string OutputClockMode
        {
            get
            {
                return m_outputControl.OutputComboMode == CyPortConstants.OutputComboMode_CLOCK_INVERTED
            ...
            set
            {
                if (OutputMode == CyPortConstants.OutputModesValue_DATA)
                {
                    m_outputControl.OutputComboMode = OutputSynchronized == CyPortConstants.OutputsSynchronizedValue_ENABLED
                                                          ? CyPortConstants.OutputComboMode_DATA_SINGLESYNC
                                                          : CyPortConstants.OutputComboMode_DATA_TRANSPARENT;
                }
                else
                {
                    m_outputControl.OutputComboMode = value == CyPortConstants.OutputClockModesValue_NORMAL
                                                         ? CyPortConstants.OutputComboMode_CLOCK_NORMAL
                                                         : CyPortConstants.OutputComboMode_CLOCK_INVERTED;
                }
            }
        ...
        {
            get { return m_outputControl.OESynchronized; }
            set { m_outputControl.OESynchronized = value; }
        }
        ...
        {
            get { return m_outputControl.OutputComboModeErrorText; }
            set
            {
                m_outputControl.OutputComboModeErrorText = value;
                UpdateOutputTabErrorIcon();
            }
        }
        ...
        {
            get { return m_outputControl.OESynchronizedErrorText; }
            set
            {
                m_outputControl.OESynchronizedErrorText = value;
                UpdateOutputTabErrorIcon();
            }
        }
        ...
        {
            get { return m_typeControl.ShowAnnotationTerm; }
            set { m_typeControl.ShowAnnotationTerm = value; }
        }
        ...
        {
            get { return m_typeControl.ShowAnnotationTermErrorText; }
            set
            {
                m_typeControl.ShowAnnotationTermErrorText = value;
                UpdateTypeTabErrorIcon();
            }
        }
        ...
        {
            if (string.IsNullOrEmpty(InterruptModeErrorText) &&
                string.IsNullOrEmpty(HotSwapErrorText) &&
                string.IsNullOrEmpty(InputThresholdLevelErrorText) &&
                string.IsNullOrEmpty(InputBufferEnabledErrorText) &&
                string.IsNullOrEmpty(InputSynchronizedErrorText) &&
                string.IsNullOrEmpty(InputSyncModeErrorText))
            {
        ...
        void UpdateOutputTabErrorIcon()
        {
            if (string.IsNullOrEmpty(SlewRateErrorText) && string.IsNullOrEmpty(OutputDriveLevelErrorText) &&
                string.IsNullOrEmpty(DriveCurrentErrorText) && string.IsNullOrEmpty(OutputComboModeErrorText) &&
                string.IsNullOrEmpty(OESynchronizedErrorText))
            {
