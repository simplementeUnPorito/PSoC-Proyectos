...
namespace CyCustomizer.CyStatusReg_v1_80
{
    public partial class CyBitfieldControl : UserControl
    {
        ...
        private void UpdateFormFromParams(ICyInstEdit_v1 inst)
        {
            ...
            int.TryParse(m_Component.GetCommittedParam(CyStatusReg_v1_80.StatusRegParameters.MASKVALUE).Value,
                            out maskval);
            ...
            bool.TryParse(m_Component.GetCommittedParam(CyStatusReg_v1_80.StatusRegParameters.BUS).Value, out busValue);
            chBus.Checked = busValue;

            // Set Generate Interrupt check box
            bool.TryParse(m_Component.GetCommittedParam(CyStatusReg_v1_80.StatusRegParameters.INTRR).Value,
                            out intValue);
        ...
        private int GetNumInputs()
        {
            ...
            if (!Int32.TryParse(m_Component.GetCommittedParam(CyStatusReg_v1_80.StatusRegParameters.NUMINPUTS).Value,
                out inputs))
            {
        ...
        private void CheckAndSaveAllParams()
        {
            ...
            else    // No error is reported, save all the parameters to symbol
            {
                ...
                m_Component.SetParamExpr(CyStatusReg_v1_80.StatusRegParameters.NUMINPUTS, nInputs.Value.ToString());
                m_Component.CommitParamExprs();
                
                // Save Bus parameter
                if (chBus.Checked)
                {
                    m_Component.SetParamExpr(CyStatusReg_v1_80.StatusRegParameters.BUS, Resource.TRUE);
                }
                else
                {
                    m_Component.SetParamExpr(CyStatusReg_v1_80.StatusRegParameters.BUS, Resource.FALSE);
                }
                ...
                if (chInterrupt.Checked)
                {
                    m_Component.SetParamExpr(CyStatusReg_v1_80.StatusRegParameters.INTRR, Resource.TRUE);
                }
                else
                {
                    m_Component.SetParamExpr(CyStatusReg_v1_80.StatusRegParameters.INTRR, Resource.FALSE);
                }
                SetMaskValue();
        ...
        private void SetMaskValue()
        {
            ...
            m_Component.SetParamExpr(CyStatusReg_v1_80.StatusRegParameters.MASKVALUE, maskval.ToString());
            m_Component.CommitParamExprs();
        }
