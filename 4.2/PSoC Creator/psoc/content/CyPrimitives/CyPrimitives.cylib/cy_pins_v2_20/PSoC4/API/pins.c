...
#include "`$INSTANCE_NAME`.h"
    ...
    #define `$INSTANCE_NAME`_SetP4PinDriveMode(shift, mode)  \
    do { \
        `$INSTANCE_NAME`_PC =   (`$INSTANCE_NAME`_PC & \
                                (uint32)(~(uint32)(`$INSTANCE_NAME`_DRIVE_MODE_IND_MASK << \
                                (`$INSTANCE_NAME`_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (`$INSTANCE_NAME`_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define `$INSTANCE_NAME`_SetP4PinDriveMode(shift, mode)  \
        do { \
            `$INSTANCE_NAME`_USBIO_CTRL_REG = (`$INSTANCE_NAME`_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(`$INSTANCE_NAME`_DRIVE_MODE_IND_MASK << \
                                    (`$INSTANCE_NAME`_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (`$INSTANCE_NAME`_DRIVE_MODE_BITS * (shift))); \
...
void `$INSTANCE_NAME`_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  `$INSTANCE_NAME`_INTCFG & (uint32)(~(uint32)position);
    `$INSTANCE_NAME`_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
