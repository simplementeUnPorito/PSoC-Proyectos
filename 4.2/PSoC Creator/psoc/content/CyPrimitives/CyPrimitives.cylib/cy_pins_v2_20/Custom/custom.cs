...
namespace Cypress.Comps.PinsAndPorts.cy_pins_v2_20
{
        ...
        ICySystemBuilderComp_v1
    {
        ...
        public const string RESET_TAB_NAME = "Reset";
        public const string MAPPING_TAB_NAME = "Mapping";
        public const string CLOCKING_TAB_NAME = "Clocking";
        public const string PINS_TAB_NAME = "Pins";
        public const string INTERCONNECT_TAB_NAME = "Interconnect";

        private const string INTERRUPT_TERM_NAME = "interrupt";

        const string PIN_COMP_NAME = "cy_psoc3_pins_v1_10";
        const string ISR_COMP_NAME = "cy_isr_v1_0";
        const string ISR_INST_SUFFIX = "_isr";
        ...
        public DialogResult EditParams(ICyInstEdit_v1 edit, ICyTerminalQuery_v1 termQuery, ICyExpressMgr_v1 mgr)
        {
            ...
            CyPinsControl pinsControl = new CyPinsControl(deviceSupport, errMgr, pinData, edit.IsInSystemBuilder);
            CyPORControl porControl = new CyPORControl(deviceSupport, errMgr, pinData);
            CyMappingControl mappingControl = new CyMappingControl(errMgr, pinData, edit.IsInSystemBuilder);
            CyClockingControl clockingControl = new CyClockingControl(deviceSupport, errMgr, pinData);
            CyTerminalControl_v1 interconnectTab = new CyTerminalControl_v1(edit);
            if(edit.IsInSystemBuilder)
                UpdateInterconnectDisplay(pinData, interconnectTab);
            ...
            editor.AddCustomPage(MAPPING_TAB_NAME, mappingControl, UpdateGUIFromExprViewChanging, MAPPING_TAB_NAME);

            bool showClockingTab = !deviceSupport.HasData || deviceSupport.IsPSoC4 || deviceSupport.IsTSG4 || UsesPaClocking(pinData);
            showClockingTab &= !edit.IsInSystemBuilder;
            if (showClockingTab)
                editor.AddCustomPage(CLOCKING_TAB_NAME, clockingControl, UpdateGUIFromExprViewChanging, CLOCKING_TAB_NAME);
            if (!deviceSupport.HasData || deviceSupport.IsPSoC3 || deviceSupport.IsPSoC5 || UsesPor(pinData))
                editor.AddCustomPage(RESET_TAB_NAME, porControl, UpdateGUIFromExprViewChanging, RESET_TAB_NAME);

            if(edit.IsInSystemBuilder)
                editor.AddCustomPage(Resource1.InterconnectTabDisplayName, interconnectTab, UpdateGUIFromExprViewChanging, INTERCONNECT_TAB_NAME);
        ...
        public CyCustErr CustomizeShapes(ICyShapeCustomizeArgs_v1 args)
        {
            ...
            bool drawInterrupts = pins.UsesDedicatedInterrupts;
            bool displayAsBus = pins.MappingDisplayAsBus;
            bool isHomogeneous = pins.IsHomogeneous;
            ...
            {
                err = drawer.DrawAsBus(true, numPins, drawInterrupts, pins, 
                    showInClk, showInClkEn, showInRst, showOutClk, showOutClkEn, showOutRst);
            }
            else
            {
                err = drawer.DrawAsIndividualTerminals(isHomogeneous, drawInterrupts, pins,
                    showInClk, showInClkEn, showInRst, showOutClk, showOutClkEn, showOutRst);
            }
        ...
        class CyPinsDrawer
        {
                ...
                bool showInClk, bool showInClkEn, bool showInRst, bool showOutClk, bool showOutClkEn, bool showOutRst)
            {
                ...
                for (int index = 0; index < pins.PinCount; index++)
                {
                    ...
                    int sioGroupIndex = GetSIOGroupIndex(pins.Pins, index);
            ...
            static int GetSIOGroupIndex(IList<CyPinData.PerPin> pins, int index)
            {
                int sioCnt = 0;
                for (int i = 0; i <= index; i++)
                {
                    if (pins[i].RequiresSIO())
                    {
                ...
                bool showInClk, bool showInClkEn, bool showInRst, bool showOutClk, bool showOutClkEn, bool showOutRst)
            {
                ...
                CyPinTypeData pinTypeData = new CyPinTypeData((pins.PinCount > 1) ? pins[1] : pins[0]);

                PointF reservedLoc = new PointF(0, -YOffset);
                int sioGroupCnt = GetSIOGroupIndex(pins.Pins, pins.PinCount - 1) + 1;
                int sioGroupIndex = GetSIOGroupIndex(pins.Pins, 0);
                ...
                bool showOutClk, bool showOutClkEn, bool showOutRst, float height)
            {
                ...
                if (advancedClockingSpaceNeeded > 0)
                {
                    ...
                    m_shapeEdit.CreateRectangle(
                        ...
                        LineJoin.Miter, Color.FromArgb(220, 220, 220), Color.Black, BODY_LINE_WIDTH);
                ...
                bool inputHw, bool outputHw, int sioGroupIndex, CyPinAssignmentInfo_v1 assignment)
            {
                ...
                {
                    err = DrawAnalog(reservedLoc, pinTypeData.DigitalInputUsed, inputHw, indexOfLeftMostBit, width);
                    if (err.IsNotOk) return err;
                }
                if (pinTypeData.DigitalInputUsed && inputHw)
                {
                ...
                if (pinTypeData.DigitalOutputUsed)
                {
                    ...
                    if (outputHw || pinTypeData.OutputEnableUsed)
                    {
            ...
            private CyCustErr DrawSIOVRef(PointF reservedLoc, int index, int width)
            {
                ...
                //Add Terminal
                string termCanName = GetTerminalName(CyTermInfo.RootTermName_SIOVRef, index, width);
                err = m_termEdit.TerminalAdd(
                    ...
                    180, false);
                if (err.IsNotOk) return err;
                // Invalid to have an external vref and leave it unconnected. But since this is a new DRC and we have
                // a ton of existing designs out there for PSoC 3/5, only enforce this for PSoC 4 devices
                if (m_query.DeviceQuery.IsPSoC4)
                {
                ...
                if (width > 1)
                {
                    AddTermWidthLabel(new PointF(end.X + m_shapeEdit.UserBaseUnit/2, end.Y), width);
                }
            ...
            private CyCustErr DrawDigitalOutput(PointF reservedLoc, int index, int width, bool outputHw)
            {
                ...
                float arrowRightX = arrowLeftX + m_shapeEdit.UserBaseUnit / 2;
                ...
                PointF labelPos = new PointF(start.X + m_shapeEdit.UserBaseUnit / 2, start.Y);
                if (outputHw)
                {
            ...
            private CyCustErr DrawDigitalInput(PointF reservedLoc, int index, int width, bool inputHw)
            {
                ...
                err = m_shapeEdit.CreateLine(
                    DigitalTags, start, new PointF(inputHw ? end.X - m_shapeEdit.UserBaseUnit : arrowLeftX, end.Y),
                    DigitalOutlineColor, m_query.Preferences.WireSize);
                if (err.IsNotOk) return err;
                ...
                PointF labelPos = new PointF(end.X - m_shapeEdit.UserBaseUnit/2, end.Y);
                if (inputHw)
                {
            ...
            private CyCustErr DrawAnalog(PointF reservedLoc, bool digitalInputUsed, bool digitalInputHw, int index, int width)
            {
                PointF end;
                CyCustErr err;
                RectangleF pinBodyBounds = GetPinBodyBounds(reservedLoc);
            ...
            private CyCustErr DrawAnnotation(PointF reservedLoc, int index, int width, int termRow, bool termIsOnLeft, bool SIOVrefUsed)
            {
                ...
                if (width > 1)      // pin is a bus
                {
                    ...
                    if (drawBusWidthNumber)
                        AddTermWidthLabel(new PointF(end.X - m_shapeEdit.UserBaseUnit * (termIsOnLeft ? 1 : -1)/2, end.Y), width);
                    else
                        AddTermWidthTick(new PointF(end.X - m_shapeEdit.UserBaseUnit * (termIsOnLeft ? 1 : -1)/2, end.Y));
                }
            ...
            private CyCustErr DrawBidirectional(PointF reservedLoc, int index, int width)
            {
                ...
                if (width > 1)
                {
                    AddTermWidthLabel(new PointF(start.X + m_shapeEdit.UserBaseUnit/2, start.Y), width);
                }
            ...
            private void AddSwLabel(PointF textLoc, int width)
            {
                ...
                if (width < 2)
                {
                    ...
                    textLoc = new PointF(textLoc.X, textLoc.Y + m_shapeEdit.UserBaseUnit * .44f); 
                }
        ...
        object UsesInterrupts(string exprFuncName, object[] args, ICyExprTypeConverter typeConverter)
        {
            ...
            //Arg 0 & 1 used, 2 is ignored now
            string interruptModesString = typeConverter.GetAsString(args[0]);
            string[] interruptValues = interruptModesString.Split(',');
            string pinTypesString = typeConverter.GetAsString(args[1]);
            string[] pinTypeValues = pinTypesString.Split(',');
            Debug.Assert(pinTypeValues.Length == interruptValues.Length);
            for (int i = 0; i < interruptValues.Length; i++)
            {
                // If the pin type was never changed from the default, we can end up without an entry in 
                // pinTypeValues for this pin. Assume the default in that case
                string pinTypeStr = (i < pinTypeValues.Length) ? pinTypeValues[i] : CyPinData.DefaultPinTypesValue;
                CyPinType pinType =  CyPinData.PerPin.ConvertPinType(pinTypeStr);

                // Only look at interrupt setting if we're input or bidirectional; don't want to trigger
                // on a setting which is left over on a pin which used to be input and is now something else
                if (((pinType & CyPinType.DIGITAL_IN) != 0) || ((pinType & CyPinType.BIDIRECTIONAL) != 0))
                {
                    if (!string.IsNullOrEmpty(interruptValues[i]) && 
                        interruptValues[i] != CyPinData.InterruptModeValue_NONE)
                        return true;
                }
            }
            return false;
        ...
        public CyCustErr GetSIOInfo(ICyInstQuery_v1 instQuery, int index, out CyPinDataSIOType_v1 sioInfo)
        {
            ...
            if (pins.PinCount > index)
            {
                ...
                else if (pins[index].RequiresSIO())
        ...
        public CyCustErr GetInputThreshold(ICyInstQuery_v1 instQuery, int index, out CyPinDataInputThreshold_v1 threshold)
        {
            ...
            if (pins.PinCount > index)
            {
                if ((pins[index].Type & (CyPinType.DIGITAL_IN | CyPinType.BIDIRECTIONAL)) == 0)
                    threshold = CyPinDataInputThreshold_v1.Unknown; // Threshold is meaningless for non-input
                else
                {
                    switch (pins[index].InputThreshold)
                    {
                        ...
                        case CyInputThreshold.VDDIO:
                            if (pins[index].InputMultiplier == CyPinData.VddioMultiplierValues[0]) // 0.4
                        ...
                        case CyInputThreshold.VREF_INTERNAL:
                            threshold = CyPinDataInputThreshold_v1.VrefInternal;
                            break;
                        case CyInputThreshold.PT5_VREF_INTERNAL:
                            threshold = CyPinDataInputThreshold_v1.VrefInternal05;
                            break;
                        case CyInputThreshold.CMOS_1PT8V:
                            threshold = CyPinDataInputThreshold_v1.CMOS_1PT8;
                            break;
                        default:
        ...
        public CyCustErr GetInputHysteresis(ICyInstQuery_v1 instQuery, int index, out CyPinDataInputHysteresis_v1 hysteresis)
        {
            CyPinData pins = new CyPinData(instQuery);
            if (pins.PinCount > index)
            {
                if (pins[index].InputHysteresis)
                    hysteresis = CyPinDataInputHysteresis_v1.ON;
                else
                    hysteresis = CyPinDataInputHysteresis_v1.OFF;
            }
            else
            {
                hysteresis = default(CyPinDataInputHysteresis_v1);
                return new CyCustErr("Index out of range");
            }
        ...
        public CyCustErr GetSlewRate(ICyInstQuery_v1 instQuery, int index, out CyPinDataSlewRate_v1 slewRate)
        {
            ...
            if (pins.PinCount > index)
            {
                switch (pins[index].OutputSlewRate)
                {
                    ...
                    case CyOutputSlewRate.I2CFM:
                        slewRate = CyPinDataSlewRate_v1.I2CFM;
                        break;
                    case CyOutputSlewRate.I2CHS_SLOW:
                        slewRate = CyPinDataSlewRate_v1.I2CHS_SLOW;
                        break;
                    case CyOutputSlewRate.I2CHS_FAST:
                        slewRate = CyPinDataSlewRate_v1.I2CHS_FAST;
                        break;
                    default:
                        slewRate = default(CyPinDataSlewRate_v1);
                        return new CyCustErr("Unknown slew rate " + pins[index].OutputSlewRate);
                }
            }
            else
            {
        ...
        static string _getIOVoltageString(string voltage)
        {
            if (string.IsNullOrEmpty(voltage) ||
                CyPinData.DefaultIOVoltageValue.Equals(voltage, StringComparison.OrdinalIgnoreCase))
            {
                return string.Empty;
            }
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            ...
            for (int count = 0; count < pins.PinCount; count++)
            {
                ...
                driveModeImplP4 += string.Format("\t\t{0}_SetP4PinDriveMode({0}__{1}__SHIFT, mode);{2}",
                    instanceName, count, Environment.NewLine);
            }
            driveModeImpl = driveModeImpl.TrimEnd();
            driveModeImplP4 = driveModeImplP4.TrimEnd();

            // Interrupt mode settings for PSoC 3/5LP
            string intrModeImpl = string.Empty;
            for (int count = 0; count < pins.PinCount; count++)
            {
                intrModeImpl += string.Format("\t\tif((position & {0}_{1}_INTR) != 0u) {2}\t\t{{ {2}\t\t\t {0}_{1}_INTTYPE_REG = (uint8)mode; {2}\t\t}} {2}",
                    instanceName, count, Environment.NewLine);
            }
            intrModeImpl = intrModeImpl.TrimEnd();
            
            // Interrupt register #defines
            string intrPoundDefine = string.Empty;
            for (int count = 0; count < pins.PinCount; count++)
            {
                intrPoundDefine += string.Format("\t#define {0}_{1}_INTTYPE_REG \t\t(* (reg8 *) {0}__{1}__INTTYPE){2}",
                    instanceName, count, Environment.NewLine);
            }
            intrPoundDefine = intrPoundDefine.TrimEnd();
            ...
            {
                //Line generated for all devices for BWC reasons.
                poundDefineMappingsSb.AppendFormat("#define {0}_{1}\t\t\t({0}__{1}__PC){2}",
                    instanceName, count, Environment.NewLine);
                
                if (query.DeviceQuery.IsPSoC4 || query.DeviceQuery.IsTSG4)
                {
                    //Applicable for P4 (EHK#308)
                    poundDefineMappingsSb.AppendFormat("#define {0}_{1}_PS\t\t({0}__{1}__PS){2}",
                        instanceName, count, Environment.NewLine);
                    poundDefineMappingsSb.AppendFormat("#define {0}_{1}_PC\t\t({0}__{1}__PC){2}",
                        instanceName, count, Environment.NewLine);
                    poundDefineMappingsSb.AppendFormat("#define {0}_{1}_DR\t\t({0}__{1}__DR){2}",
                        instanceName, count, Environment.NewLine);
                    poundDefineMappingsSb.AppendFormat("#define {0}_{1}_SHIFT\t({0}__{1}__SHIFT){2}",
                        instanceName, count, Environment.NewLine);
                    //Interrupt mask definition
                    poundDefineMappingsSb.AppendFormat("#define {0}_{1}_INTR\t((uint16)((uint16)0x0003u << ({0}__{1}__SHIFT*2u))){2}{2}",
                        instanceName, count, Environment.NewLine);
                }
                else
                {
                    //Interrupt mask definition
                    poundDefineMappingsSb.AppendFormat("#define {0}_{1}_INTR\t((uint16)((uint16)0x0001u << {0}__{1}__SHIFT)){2}{2}",
                        instanceName, count, Environment.NewLine);
                }
                ...
                if (!string.IsNullOrEmpty(alias))
                {
                    ...
                    {
                        //Applicable for P4 (EHK#308)
                        poundDefineMappingsAliasesSb.AppendFormat("#define {0}_{1}_PS\t\t({0}__{1}__PS){2}",
                            instanceName, alias, Environment.NewLine);
                        poundDefineMappingsAliasesSb.AppendFormat("#define {0}_{1}_PC\t\t({0}__{1}__PC){2}",
                            instanceName, alias, Environment.NewLine);
                        poundDefineMappingsAliasesSb.AppendFormat("#define {0}_{1}_DR\t\t({0}__{1}__DR){2}",
                            instanceName, alias, Environment.NewLine);
                        poundDefineMappingsAliasesSb.AppendFormat("#define {0}_{1}_SHIFT\t({0}__{1}__SHIFT){2}",
                            instanceName, alias, Environment.NewLine);
                        //Interrupt mask definition
                        poundDefineMappingsAliasesSb.AppendFormat("#define {0}_{1}_INTR\t((uint16)((uint16)0x0003u << ({0}__{2}__SHIFT*2u))){3}{3}",
                            instanceName, alias, count, Environment.NewLine);
                    }
                    else
                    {
                        //Interrupt mask definition
                        poundDefineMappingsAliasesSb.AppendFormat("#define {0}_{1}_INTR\t((uint16)((uint16)0x0001u << {0}__{2}__SHIFT)){3}{3}",
                            instanceName, alias, count, Environment.NewLine);
                    }
                }
            }
            //Interrupt mask for all pins of Pin instance
            poundDefineMappingsSb.AppendFormat("#define {0}_INTR_ALL\t ((uint16)(", instanceName);
            poundDefineMappingsSb.AppendFormat("{0}_0_INTR", instanceName);
            for (byte count = 1; count < pins.PinCount; count++)
            {
                //Gather all pins interrupt masks
                poundDefineMappingsSb.AppendFormat("| {0}_{1}_INTR",
                    instanceName, count);
            }
            poundDefineMappingsSb.AppendFormat("))",
                Environment.NewLine);
            ...
            foreach (CyAPICustomizer file in apiCustomizers)
            {
                ...
                paramDict["SetIntrModeImpl_API_GEN"] = intrModeImpl;
                paramDict["IntrPoundDefine_API_GEN"] = intrPoundDefine;
        ...
        static void CheckInputRestrictSettings(ICyInstEdit_v1 instEdit)
        {
            ...
            CheckPinSetting validator = delegate(CyPinData.PerPin pin)
            {
                return (pin.GeneralHotSwap == hotSwapsExpectedValues[pin.Index]);
            };
            Check(instEdit, validator);
        ...
        #region Implementation of ICySystemBuilderComp_v1

        public CyCustErr GetTitleText(ICySystemBuilderCompArgs_v1 args, out string title)
        {
            CyPinData pinData = new CyPinData(args.InstQuery);
            title = (pinData.PinCount < 2) ? Resource1.SBComponentTitle : Resource1.SBComponentTitlePlural;
            return CyCustErr.OK;
        }

        private string GetPinTypeText(CyPinType type)
        {
            List<string> typesFound = new List<string>();
            foreach(CyPinType toCheck in Enum.GetValues(typeof(CyPinType)))
            {
                if ((type & toCheck) != 0)
                {
                    switch (toCheck)
                    {
                        case CyPinType.DIGITAL_IN:
                            typesFound.Add("Digital Input");
                            break;
                        case CyPinType.OE:
                            break; // Can never have OE without also being a digital output
                        case CyPinType.DIGITAL_OUT:
                            typesFound.Add("Digital Output");
                            break;
                        case CyPinType.BIDIRECTIONAL:
                            typesFound.Add("Bidirectional");
                            break;
                        case CyPinType.ANALOG:
                            typesFound.Add("Analog");
                            break;
                        default:
                            Debug.Fail("Unknown pin type");
                            typesFound.Add(type.ToString());
                            break;
                    }
                }
            }
            return string.Join(", ", typesFound.ToArray());
        }

        public CyCustErr GetBodyText(ICySystemBuilderCompArgs_v1 args, out string bodyText)
        {
            CyPinData pinData = new CyPinData(args.InstQuery);
            CyPinType pin0Type = pinData.Pins[0].Type;
            string portTypeString = GetPinTypeText(pin0Type);
            foreach(CyPinData.PerPin pin in pinData.Pins)
            {
                if(pin.Type != pin0Type)
                {
                    portTypeString = "Mixed Types";
                }
            }

            string contiguousOrSpanning = (pinData.MappingContigious) ? "Contiguous" : "Spanning";
            int numPins = pinData.PinCount;
            bodyText = String.Format("{0} Pin{1}, {2},\n{3}", numPins, numPins > 1 ? "s" : "", portTypeString, contiguousOrSpanning);
            return CyCustErr.OK;
        }

        #endregion

        void UpdateInterconnectDisplay(CyPinData pinData, CyTerminalControl_v1 termControl)
        {
            // Displays a readonly row set to either internal or unused, depending on the interrupt configuration
            CyTermMode_v1 mode = pinData.UsesInterrupts ? CyTermMode_v1.Internal : CyTermMode_v1.Unused;

            CyTermMode_v1[] validModes = new CyTermMode_v1[] { mode };
            CyTermControlRow_v1[] interrupts = new CyTermControlRow_v1[1];
            interrupts[0] = new CyTermControlRow_v1(INTERRUPT_TERM_NAME, INTERRUPT_TERM_NAME, 
                                                validModes, Resource1.InterruptTermDescription);
            termControl.Interrupts = interrupts;
            termControl.SetTerminalMode(INTERRUPT_TERM_NAME, mode);
        }
    }
