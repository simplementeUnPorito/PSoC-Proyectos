...
namespace Cypress.Comps.PinsAndPorts.cy_pins_v2_20
{
    ...
    public static partial class CyParameters
    {
        ...
        internal const string Local_ParamName_SIOVohSels = "sio_vohsel";
        ...
        internal const string Local_ParamName_SioHiFreq = "sio_hifreq";
        ...
        private class HwPerPinSettings
        {
            public string[] DriveMode, InputBufferEn, DrState, InputSync, InputSyncMode, IntrMode, IoVoltage,
                ...
                SioHist, SioIBuf, SioInfo, SioOBuf, SioRefSel, SioVtrip, SioHiFreq, SioVohSel, SioSlewRate, UseAnnotation, Vtrip,
        ...
        public static IDictionary<string, string> GenerateVerilogGenerics(CyPinData pins, ICyDeviceQuery_v1 deviceQuery)
        {
            ...
            verilogItems.Add(Local_ParamName_SioHiFreq, HwGetVerilogBitVector(perPinSettings.SioHiFreq));
            verilogItems.Add(Local_ParamName_SIOVohSels, HwGetVerilogBitVector(perPinSettings.SioVohSel));
        ...
        private static HwPerPinSettings CreatePerPinSetting(CyPinData pins, ICyDeviceQuery_v1 deviceQuery)
        {
            ...
            for (int i = 0; i < pins.PinCount; i++)
            {
                ...
                perPinSettings.SioOBuf[i] = 
                    pin.IsFirstSIO() ? HwConvertSioOutputDriveLevel(pin.OutputDriveLevel) : "";
                perPinSettings.SioRefSel[i] = 
                    (pin.IsFirstSIO()) ? HwConvertSioVref(pin.VRefSource) : "";
                perPinSettings.SioVtrip[i] = pin.IsFirstSIO() ? HwConvertSioVtrip(pin.InputThreshold, pin.InputMultiplier) : "";
                perPinSettings.SioHiFreq[i] = 
                    pin.IsFirstSIO() && (deviceQuery.IsPSoC3 || deviceQuery.IsPSoC5) ? HwConvertSioHiFreq(pin.OutputSlewRate) : "";
                perPinSettings.SioVohSel[i] = pin.IsFirstSIO() ? HwConvertSioMultiplier(pin) : "";
        ...
        private static string HwConvertSioVref(CyVrefSource refSource)
        {
            // AMUXBUSA/AMUXBUSB/INTERNAL designation will be populated by the fitter
            switch (refSource)
            {
                case CyVrefSource.INTERNAL:
                case CyVrefSource.ROUTED:
                    return CyPinData.SIORefSelValue_VOHREF;
                case CyVrefSource.DONTCARE:
                default:
                    return CyPinData.SIORefSelValue_VCC_IO;
           }
        }

        private static string HwConvertSioHiFreq(CyOutputSlewRate slewRate)
        {
            // Per CDT 65312 - set SIO HiFreq to 1 for fast slew rate
            switch (slewRate)
            {
                case CyOutputSlewRate.FAST:
                    return CyPinData.ParamValue_ENABLED;
                default:
                    return CyPinData.ParamValue_DISABLED;
            }
        }

        private static string HwConvertSioOutputDriveLevel(CyOutputDriveLevel driveLevel)
        {
            switch(driveLevel)
            {
                case CyOutputDriveLevel.VDDIO:
                    return CyPinData.OutputDriveLevelValue_VDDIO;
                case CyOutputDriveLevel.VREF:
                case CyOutputDriveLevel.VREF_INTERNAL: // Vrev vs. Vref internal is handled by vtrip
                    return CyPinData.OutputDriveLevelValue_VREF;
                default:
                    Debug.Fail("Unknown drive level");
                    return string.Empty;
            }
        }

        private static string HwConvertSioVtrip(CyInputThreshold threshold, double multiplier)
        {
            switch (threshold)
            {
                ...
                case CyInputThreshold.VDDIO:
                    if (multiplier == CyPinData.VddioMultiplierValues[0])
                        return CyPinData.SIOVTripValue_PT4_OR_ONE;
                    else
                        return CyPinData.SIOVTripValue_PT5;
                default:
        ...
        private static string HwConvertSioMultiplier(CyPinData.PerPin pin)
        {
            return Convert.ToString(pin.MultiplierIndex, 2).PadLeft(3, '0');
        }
        ...
        private static string HwConvertSlewRate(CyOutputSlewRate slewRate)
        {
            switch (slewRate)
            {
                ...
                case CyOutputSlewRate.I2CHS_FAST: // they will be differentiated by the value in ovt_slew_control
        ...
        private static string HwConvertOvtSlew(CyOutputSlewRate rate, string supply)
        {
            ...
            const double OVT_SLEW_HS_FAST_THRESHOLD = 3.0; 
            const double OVT_SLEW_HS_SLOW_THRESHOLD = 2.8; 
            switch(rate)
            {
                case CyOutputSlewRate.FAST:
                ...
                case CyOutputSlewRate.I2CHS_SLOW:
                    if(supplyVoltage <= OVT_SLEW_HS_SLOW_THRESHOLD)
                        return CyPinData.OVTSlewRate_HSSlowLowVDDIO;
                    else
                        return CyPinData.OVTSlewRate_HSSlowHighVDDIO;
                case CyOutputSlewRate.I2CHS_FAST:
                    if(supplyVoltage <= OVT_SLEW_HS_FAST_THRESHOLD)
                        return CyPinData.OVTSlewRate_HSFastLowVDDIO;
                    else
                        return CyPinData.OVTSlewRate_HSFastHighVDDIO;
                default:
                    Debug.Fail("Unknown input threshold");
                    return CyPinData.SlewRateValue_FAST;
            }
    ...
    public class CyDeviceSupport
    {
        ...
        private const string UDBIF_BLOCK = "m0s8udbif";
        private const string UDB_BLOCK = "m0s8udb";
        ...
        public readonly bool HasAllPortsInterrupt;
        ...
        public CyDeviceSupport(ICyDeviceQuery_v1 deviceQuery)
        {
            ...
            HasAllPortsInterrupt = GetHasAllPortsInterrupt(deviceQuery);
        }
        ...
        public static bool GetHasPortAdapter(ICyDeviceQuery_v1 deviceQuery)
        {
            return (deviceQuery.IsPSoC4 || deviceQuery.IsTSG4)
                ? (GetUdbCount(deviceQuery) > 0)
                : false;
        }

        public static int GetUdbCount(ICyDeviceQuery_v1 deviceQuery)
        {
            return Math.Max(deviceQuery.GetFeatureCountDie(UDB_BLOCK),
                            deviceQuery.GetFeatureCountDie(UDBIF_BLOCK));
        }

        public static bool GetHasAllPortsInterrupt(ICyDeviceQuery_v1 deviceQuery)
        {
            if (deviceQuery.IsPSoC4 && deviceQuery.PSoC4DeviceInfoQueryV1 != null)
            {
                bool result;
                deviceQuery.PSoC4DeviceInfoQueryV1.HasAllPortsInterrupt(out result);
                return result;
            }
            else
                return false;
